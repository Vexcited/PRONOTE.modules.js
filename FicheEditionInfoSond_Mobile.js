exports.FicheEditionInfoSond_Mobile = void 0;
...
class FicheEditionInfoSond_Mobile extends ObjetFenetre_1.ObjetFenetre {
	constructor() {
		super(...arguments);
		this.id = {
			...
			nbIndividus: GUID_1.GUID.getId(),
			nbProfs: GUID_1.GUID.getId(),
			nbPersonnels: GUID_1.GUID.getId(),
			nbResponsables: GUID_1.GUID.getId(),
		};
		this.forcerAR = false;
	}
	getControleur(aInstance) {
		return $.extend(true, super.getControleur(aInstance), {
			...
			getEntiteClasse: {
				getLibelle() {
					return `${ObjetTraduction_1.GTraductions.getValeur("infoSond.parClasses")}<span class="strNumber" id="${aInstance.id.nbEntiteClasse}"> (0) </span> ${ObjetTraduction_1.GTraductions.getValeur("infoSond.ouGroupes")}<span class="strNumber" id="${aInstance.id.nbEntiteGpe}"> (0) </span>`;
				},
				getIcone() {
					return `icon_group`;
				},
			...
			nodeSelectDestParEntite: function () {
				$(this.node).on("click", function () {
					const lFenetre = ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
						FenetreEditionDestinatairesParEntites_1.FenetreEditionDestinatairesParEntites,
						{
							...
							evenement: function (aNumeroBouton, aDonnees) {
								if (aNumeroBouton === 1) {
									aInstance.donnee = aDonnees.donnee;
									aInstance.updateCompteursDestinataires();
								}
							},
							initialiser: function (aInstanceFenetre) {
								...
								aInstanceFenetre.setOptions({
									avecCBElevesRattaches: aInstance.options
										? aInstance.options.avecCBElevesRattaches
										: false,
								});
					...
					lFenetre.setDonnees({ donnee: aInstance.donnee });
				});
			},
			getIndividu: {
				getLibelle() {
					return `${ObjetTraduction_1.GTraductions.getValeur("infoSond.aTitreIndiv")}<span class="strNumber" id="${aInstance.id.nbIndividus}"> (0)</span>`;
				},
				getIcone() {
					return `icon_user`;
				},
					...
					const lFenetre = ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
						FenetreEditionDestinatairesParIndividus_1.FenetreEditionDestinatairesParIndividus,
						{
							...
							initialiser: function (aInstanceFenetre) {
								aInstanceFenetre.setUtilitaires(aInstance.utilitaires);
								aInstanceFenetre.setOptions({
									avecGestionEleves: aInstance.options
										? aInstance.options.avecGestionEleves
										: true,
									avecGestionPersonnels: aInstance.options
										? aInstance.options.avecGestionPersonnels
										: true,
									avecGestionStages: aInstance.options
										? aInstance.options.avecGestionStages
										: true,
									avecGestionIPR: aInstance.options
										? aInstance.options.avecGestionIPR
										: true,
								});
								aInstanceFenetre.avecListeDiffusion =
									aInstance.avecListeDiffusion;
			...
			nodeSelectDest_RespToResp: function () {
				$(this.node).on("click", function () {
					aInstance.openModaleSelectionPublic({
						genreRessource:
							aInstance.utilitaires.genreRessource.getRessourceClasse(),
						listePublicDonnee: aInstance.donnee.listePublicIndividu,
						clbck: function (aParam) {
							aInstance.donnee.listePublicIndividu = aParam.listePublicDonnee;
			...
			nodeSelectDestPrimaire_RespClasses: function (aGenreRessource) {
				$(this.node).on("click", function () {
					aInstance.openModaleSelectRespClasses({
						genreRessource: aGenreRessource,
						donnee: aInstance.donnee,
						clbck: () => {
							aInstance.utilitaires.moteurDestinataires.surSelectEntitesPrimaireRespClasses(
								{ donnee: aInstance.donnee },
							);
							aInstance.updateCompteursDestinataires();
						},
			...
			cbPrimaire_Directeur: {
				getValue: function () {
					return aInstance.donnee.avecDirecteur;
				},
				setValue: function (aValue) {
					aInstance.donnee.avecDirecteur = aValue;
					aInstance.donnee.avecModificationPublic = true;
				},
			...
			nodeTitre: function () {
				$(this.node).on("focusin", function () {
					ObjetHtml_1.GHtml.setSelectionEdit(aInstance.id.titre);
				});
			},
			titre: {
				getValue: function () {
					return aInstance.donnee !== null && aInstance.donnee !== undefined
						? aInstance.donnee.getLibelle()
						: "";
				},
				setValue: function (aValue) {
					aInstance.donnee.setLibelle(aValue);
				},
				getDisabled: function () {
					return aInstance.donnee.estVerrouille;
				},
				node: function () {
					$(this.node).attr(
						"placeholder",
						ObjetTraduction_1.GTraductions.getValeur("infoSond.redigerTitre"),
					);
				},
			},
			contenu: {
				getValue: function () {
					return aInstance.getValueContenu();
				},
				setValue: function (aValue) {
					aInstance.setValueContenu(aValue);
				},
				getDisabled: function () {
					return aInstance.donnee.estVerrouille;
				},
			...
			cbAR: {
				getValue: function () {
					let lResult = false;
					if (
						aInstance.donnee !== null &&
						aInstance.donnee !== undefined &&
						aInstance.donnee.estInformation &&
						aInstance.donnee.listeQuestions.getNbrElementsExistes() > 0
					) {
						const lContenu =
							aInstance.donnee.listeQuestions.getPremierElement();
						...
						aInstance._evntSurEditPJ({
							...
							validation: function (
								...
								if (
									...
									if (aListeFichiers) {
										aInstance.listePJCree.add(aListeFichiers);
									}
									const lContenu =
										aInstance.donnee.listeQuestions.getPremierElement();
									lContenu.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
								}
								aInstance.actualiserListePJ();
							},
						});
					}
				});
			...
			btnPJ: {
				...
				getLibelle() {
					return ObjetTraduction_1.GTraductions.getValeur("infoSond.ajouterPJ");
				},
			},
			chipsPJ: {
							...
							GApplication.getMessage().afficher({
								...
								callback: function (aAccepte) {
									...
									ObjetHtml_1.GHtml.setHtml(
										aInstance.id.listePJ,
										aInstance._construireHtmlListePJ(),
			...
			radioPublication: {
				getValue: function (aBool) {
					return !!aInstance.donnee && aInstance.donnee.publie === aBool;
				},
				setValue: function (aBool) {
					aInstance.donnee.publie = aBool;
					aInstance.instanceCalendrierDebPublication.setActif(
						!!aInstance.donnee.publie,
					);
					aInstance.instanceCalendrierFinPublication.setActif(
						!!aInstance.donnee.publie,
					);
				},
			},
	...
	setUtilitaires(aUtilitaires) {
		...
		this.moteurDestinataires = new MoteurDestinataires_1.MoteurDestinataires(
			this.utilitaires,
		);
	}
	construireInstances() {
		this.instanceCalendrierDebPublication = this.instancierCalendrier(
			(aDate) => {
				if (
					...
					!ObjetDate_1.GDate.estJourEgal(aDate, this.donnee.dateDebut)
				) {
		...
		this.instanceCalendrierFinPublication = this.instancierCalendrier(
			(aDate) => {
				if (
					this.donnee.publie === true &&
					!ObjetDate_1.GDate.estJourEgal(aDate, this.donnee.dateFin)
				) {
		...
		this.instanceSelectCategorie = this.instancierSelecteur(
			...
			this,
			{
				...
				labelWAICellule:
					ObjetTraduction_1.GTraductions.getValeur("infoSond.categorie"),
			},
		);
	}
	composeDate(aParam) {
	...
	setDonnees(aParam) {
		Promise.resolve()
			...
			.then((aDonnees) => {
				this.listePJCree = new ObjetListeElements_1.ObjetListeElements();
				this.listePJ =
				...
				this.donnee = MethodesObjet_1.MethodesObjet.dupliquer(
					this.donneeOrigine,
				);
				this.afficher(this.composeContenu());
				this.updateContenu();
				if (this._estCasParentsDelegues()) {
	...
	updateCompteursDestinataires() {
		...
		if (this.utilitaires.genreEspace.estPourPrimaire()) {
			...
			ObjetHtml_1.GHtml.setHtml(
				...
				this._construireHtmlNb(lNbResponsables),
			);
			...
			ObjetHtml_1.GHtml.setHtml(
				this.id.nbProfs,
				this._construireHtmlNb(lNbProfs),
			);
			...
			ObjetHtml_1.GHtml.setHtml(
				...
				this._construireHtmlNb(lNbPersonnels),
			);
		} else if (this._estCasParentsDelegues()) {
			const lNbResponsables = this.donnee.listePublicIndividu
				.getListeElements((D) => {
					return (
						D.getGenre() ===
			...
			ObjetHtml_1.GHtml.setHtml(
				this.id.nbIndividus,
				this._construireHtmlNb(lNbIndiv),
			);
		}
	}
	construireHtml() {
		...
		if (this.donnee !== null && this.donnee !== undefined) {
			...
			H.push(this._construireHtmlDestinataires());
			H.push("</div>");
			if (!this.donnee.estVerrouille) {
				...
				H.push("</div>");
				H.push('<div class="field-contain">');
				const lLibelleTitre = ObjetTraduction_1.GTraductions.getValeur(
					"infoSond.libelleTitre",
				);
				H.push('<label class="ie-titre-petit">', lLibelleTitre, "</label>");
				H.push(
				...
				if (
					...
					H.push(
						'<label class="ie-titre-petit">',
						ObjetTraduction_1.GTraductions.getValeur("infoSond.Contenu"),
						"</label>",
					);
					H.push(
						(0, tag_1.tag)(
				...
				H.push(
					'<div class="taille-m item msgContenuNonModifiable">',
					ObjetTraduction_1.GTraductions.getValeur(
			...
			H.push(`<div class="field-contain flex-gap-l">`);
			H.push(
				'<ie-radio ie-model="radioPublication(false)" class="itemPublication">',
				ObjetTraduction_1.GTraductions.getValeur("actualites.NonPubliee"),
				"</ie-radio>",
			);
			H.push(
			...
			H.push(`<div class="flex-contain flex-center flex-gap m-top">`);
			H.push(
				"<span>",
	...
	surValidation(aGenreBouton) {
		...
		if (lEstValidation) {
			const lControleContenu = this._controlerContenu();
			if (!lControleContenu) {
	...
	finaliserValidation(aGenreBouton, aJSONReponse, aJSONRapport) {
		if (
			!!aJSONReponse &&
			aJSONReponse === ObjetRequeteJSON_1.EGenreReponseSaisie.succes
		) {
			Toast_1.Toast.afficher({
	...
	_evntSurEditPJ(aParam) {
		...
		if (lAvecCloud) {
			lTabActions.push({
				...
				icon: "icon_cloud",
				event: () => {
					this.utilitaires.moteurGestionPJ.ouvrirFenetreChoixListeCloud(aParam);
				},
			...
			if (GEtatUtilisateur.avecCloudENEJDisponible()) {
				const lActionENEJ =
					ObjetFenetre_ActionContextuelle_1.ObjetFenetre_ActionContextuelle.getActionENEJ(
						() => {
							this.utilitaires.moteurGestionPJ.ouvrirFenetreChoixFichierCloud(
								Object.assign(Object.assign({}, aParam), {
	...
	jsxModelBoutonSignature() {
		return {
			event: () => {
				const lSignature = this.options.strSignature;
				const lContent = this.getValueContenu();
				this.setValueContenu(lContent ? lContent + lSignature : lSignature);
			},
			getDisabled: () => {
	...
	_construireHtmlDestinataires() {
		...
		H.push(
			...
			ObjetTraduction_1.GTraductions.getValeur("infoSond.destinataires"),
			"</label>",
		);
		if (this.utilitaires.genreEspace.estPourPrimaire()) {
			H.push(
	...
	_controlerContenu() {
		...
		if (
			...
			const lContenuEscape = $(
				ObjetHtml_1.GHtml.htmlToDOM((0, tag_1.tag)("div", lStr)),
			)
				.text()
				.trim();
	...
	avecBtnSignature() {
		var _a;
		return (
			!!this.options.strSignature &&
			((_a = this.donnee) === null || _a === void 0
				? void 0
				: _a.estInformation)
		);
	}
...
exports.FicheEditionInfoSond_Mobile = FicheEditionInfoSond_Mobile;
