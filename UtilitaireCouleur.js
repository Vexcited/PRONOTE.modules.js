...
const UtilitaireCouleur = {
	...
	getContrastedColor(aColor) {
		...
		const lBlack = "#000000";
		if (
			this.contrastRatio(aColor, lWhite) > this.contrastRatio(aColor, lBlack)
		) {
			return lWhite;
		}
		return lBlack;
	...
	contrastRatio(aColor1, aColor2) {
		let ratio =
			(0.05 + this.relativeLum(aColor1)) / (0.05 + this.relativeLum(aColor2));
		if (ratio < 1) {
			ratio = 1 / ratio;
		}
		return ratio;
	...
	relativeLum(aColor) {
		...
		let lum =
			transformed.r * 0.2126 + transformed.g * 0.7152 + transformed.b * 0.0722;
		return lum;
	},
};
