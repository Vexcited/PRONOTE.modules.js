...
class ObjetNavigateur {
	...
	initialiser() {
		...
		if (
			...
			this.IsIE9 =
				parseInt(this.UserAgent.substring(LIndex + 5, LIndex + 6), 10) === 9;
		}
		try {
			this.IsIE = LIndex !== -1;
		} catch (e) {
		...
		this.withContentEditable =
			...
			this.IsOpera ||
			this.IsGecko ||
			(this.IsWebKit && !this.isAndroid && !this.isIphone && !this.isIpad) ||
			(this.IsWebKit &&
	...
	isToucheShiftTab() {
		return (
			this.CodeTouche === ToucheClavier_1.ToucheClavier.Tab && this.ShiftTouche
		);
	}
	isToucheOnlyTab() {
		return (
	...
	debDeplacement(APere, AId) {
		...
		this.DecalageX =
			this.pointerX - ObjetPosition_1.GPosition.getLeft(this._idDeplacement);
		this.DecalageY =
...
function _emulationContextMenu() {
	...
	function _touchstart(event) {
		...
		uGestionTouch = { x: lPosition.x, y: lPosition.y, timeStart: Date.now() };
	}
		...
		if (lGestionTouch && Date.now() - lGestionTouch.timeStart > lDelai) {
			...
			const lAccepterContextMenu =
				...
				(!ObjetPosition_1.GPosition ||
					ObjetPosition_1.GPosition.positionDansZone(lPosition, aEvent.target));
...
function _uniformisationTouch() {
	...
	function _eventDansMargesLongTouch(aEvent) {
		return (
			...
			Math.abs(aEvent.clientY - uLongTouch.y) < const_margesDeplacementLongTouch
		);
	}
	...
	function _eventDansMargesDoubleTap(aEvent) {
		return (
			...
			Math.abs(aEvent.clientY - uLastTap.y) < const_margesDeplacementDoubleTap
		);
	}
	$(document).on({
