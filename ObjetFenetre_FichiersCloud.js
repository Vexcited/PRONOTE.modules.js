...
class ObjetFenetre_FichiersCloud extends ObjetFenetre_1.ObjetFenetre {
			...
			(aInstance) => {
				aInstance.setOptionsListe({
					...
					avecCBToutCocher: this.optionsFenetre
						...
						? false
						: ObjetListe_1.ObjetListe.typeLibelleCocheCBEntete.compteurSelec,
					boutons: [
						{
							class: "icon_refresh",
							event: () => {
	...
	setDonnees(aParam) {
		...
		if (!aParam || !MethodesObjet_1.MethodesObjet.isNumber(aParam.service)) {
			return;
		}
			...
			btnUpload: {
				...
				getClass() {
					return aInstance.param &&
						...
						aInstance._estIdPartageValide(aInstance.idPartageDossierSelection)
						? ""
						: "hide";
				},
			},
		...
		Promise.resolve()
			.then(() => {
				if (lBouton && lBouton.estValider && this.param.racineFichier) {
					...
					if (
						this.listeCloud.getElementParNumeroEtGenre(null, this.param.service)
							.avecChoixFormat &&
			...
			.then((aListeSelection) => {
				if (aListeSelection && aListeSelection.count) {
					...
					if (lBouton && lBouton.estValider && aListeSelection.count() > 0) {
	...
	_surReponseCloudAttente(aParamCloud, aEnAuthCloud = false) {
		if (
			...
			aParamCloud.status ===
				TypeStatutInterrogationCloud_1.TypeStatutInterrogationCloud.sic_OK &&
			!aEnAuthCloud
		) {
			if (!this.param.inscriptionSeule) {
				...
				if ("idPartage" in aParamCloud && aParamCloud.idPartage) {
					...
					if (lDossier && aParamCloud.racineFichier) {
						const lElementService = this.listeCloud.getElementParNumeroEtGenre(
							null,
							aParamCloud.service,
						);
					...
					this.idPartageDossierSelection = aParamCloud.racineFichier.idPartage;
					this._setDonneesListe(
						new DonneesListe_FichiersCloud_1.DonneesListe_FichiersCloud(
					...
					new ObjetRequeteCloudAttente_1.ObjetRequeteCloudAttente(this)
						...
						.then((aParamsReponse) => {
							...
							if (aParamCloud.partageFichier) {
								...
								this._surReponseCloudAttente(aParamsReponse, aParamCloud.oauth);
							}
						});
				},
				() => {
						...
						return this._requeteRecupFichiersDIdPartage(
							...
							this._parcoursDossiersRacine((aFichierSource) => {
								if (
									...
									aParams.racineFichier.listeFichier.parcourir((aFichier) => {
