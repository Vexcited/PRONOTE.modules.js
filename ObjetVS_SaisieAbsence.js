...
class ObjetVS_SaisieAbsence extends ObjetIdentite_1.Identite {
	constructor() {
		...
		this.commentaireAbsenceObligatoire = false;
	}
	construireInstances() {
			...
			if (this.parametres.afficherNew) {
				return IE.jsx.str(
					...
					IE.jsx.str(
						...
						this.avecInfoJustificationObligatoire
							? IE.jsx.str(
									"div",
									{ class: [Texte_css_1.StylesTexte.fontSize11] },
									"*",
				...
				H.push(
					' <div class="p-top  p-bottom-l flex-contain flex-end justify-between">',
					'     <div class="',
					!IE.estMobile ? "flex-contain flex-center flex-gap" : "fix-bloc",
					'">',
					"     <label>",
				...
				if (this.parametres.avecHoraire) {
					H.push(
						'<ie-combo style="flex:none;',
						IE.estMobile ? "width:9.5rem;" : "",
						'" ie-model="comboHeureDebut"></ie-combo>',
					);
				} else {
					H.push(
						'<div class="fix-bloc',
						IE.estMobile ? " p-bottom" : "",
						'">',
				...
				H.push(
					' <div class="p-top p-bottom-l flex-contain flex-end justify-between">',
					'<div class="fix-bloc',
					!IE.estMobile ? " flex-contain flex-center flex-gap" : "",
					'">',
					"  <label>",
					IE.estMobile && this.parametres.avecHoraire
				...
				H.push(" </div>");
				H.push(
					' <ie-combo class="m-top-l separateur-haut',
					IE.estMobile ? " p-y-xl" : " p-y-l",
					'" ie-model="comboMotif"></ie-combo>',
				);
				H.push(
					' <div class="pj-global-conteneur m-top-l p-y separateur-haut">',
					'    <ie-btnselecteur ie-model="telechargerJustificatif" role="button" ie-selecfile class="pj" ie-tooltiplabel="',
					ObjetTraduction_1.GTraductions.getValeur(
						"AbsenceVS.TelechargerUnJustifcatif",
					),
					'"></ie-btnselecteur>',
					'     <div class="pj-liste-conteneur" ie-html="getLibellePJ"></div>',
					"  </div>",
				);
				H.push(
					` <div class="m-top-l separateur-haut ${IE.estMobile ? " p-y-xl" : " p-y-l"}"><label class="m-bottom" for="${this.idCommentaire}">${ObjetTraduction_1.GTraductions.getValeur("AbsenceVS.commentaire")}</label><ie-textareamax ie-model="commentaire" ie-autoresize maxlength="200" class="txt-comment fluid-bloc" ${!IE.estMobile ? 'style="min-height:6rem;"' : ""} ie-class="getClassCommentaire" placeholder="${ObjetTraduction_1.GTraductions.getValeur("AbsenceVS.AjouterUnCommentaire")}" id="${this.idCommentaire}"></ie-textareamax></div>`,
				);
				H.push("</div>");
	...
	getControleur(aInstance) {
		return $.extend(true, super.getControleur(aInstance), {
			...
			btnValider: {
				...
				getDisabled: function () {
					return (
						...
						(aInstance.commentaireAbsenceObligatoire &&
							(!aInstance.absenceTraitee.justification ||
								aInstance.absenceTraitee.justification.trim() === "")) ||
						(!!aInstance.absenceTraitee.motifParent.justificationObligatoire &&
							(!aInstance.absenceTraitee.justification ||
								aInstance.absenceTraitee.justification.trim() === "") &&
			...
			telechargerJustificatif: {
				...
				getLibelle() {
					return (
						ObjetTraduction_1.GTraductions.getValeur(
							"AbsenceVS.TelechargerUnJustifcatif",
						) + (aInstance.avecInfoJustificationObligatoire ? "*" : "")
					);
				},
			},
			...
			comboHeureFin: {
				...
				getIndiceSelection: function () {
					const lElement = aInstance.absenceTraitee;
					if (!lElement || !lElement.fin || !lElement.fin.heure) {
	...
	setDonnees(aDonnees, aParams) {
		...
		this.commentaireAbsenceObligatoire = aParams.commentaireAbsenceObligatoire;
		if (!this.absenceTraitee.documents) {
			this.absenceTraitee.documents =
				new ObjetListeElements_1.ObjetListeElements();
		}
		if (this.parametres.avecHoraire && this.absenceTraitee) {
			if (this.absenceTraitee.debut) {
				if (
					!this.absenceTraitee.debut.heure &&
					this.parametres.listeHeuresDebut &&
					this.parametres.listeHeuresDebut.count() > 0
				) {
					this.absenceTraitee.debut.heure =
						this.parametres.listeHeuresDebut.get(0);
				}
			}
			if (this.absenceTraitee.fin) {
				if (
					!this.absenceTraitee.fin.heure &&
					this.parametres.listeHeuresFin &&
					this.parametres.listeHeuresFin.count() > 0
				) {
					this.absenceTraitee.fin.heure =
						this.parametres.listeHeuresFin.getDernierElement();
				}
			}
		}
		if (aParams) {
	...
	_composeDebut() {
		return IE.jsx.str(
			"div",
			{
				...
				role: "group",
				"aria-labelledby": this.ids.labelDebut,
			},
			IE.jsx.str(
				"label",
	...
	_composeFin() {
		return IE.jsx.str(
			"div",
			{
				class: ["field-contain", "border-bottom", "p-bottom-xl"],
				role: "group",
				"aria-labelledby": this.ids.labelFin,
			},
			IE.jsx.str(
				"label",
