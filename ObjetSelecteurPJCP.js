...
class ObjetSelecteurPJCP extends ObjetInterface_1.ObjetInterface {
	...
	setDonnees(aParam) {
		this.avecInputFile =
			this._options.genrePJ !== Enumere_DocumentJoint_1.EGenreDocumentJoint.Url;
	...
	evntEditionPJ() {
		...
		for (let I = 0; I < alisteDocumentsJoints.count(); I++) {
			...
			const lActif =
				alisteDocumentsJoints.get(I).Actif && alisteDocumentsJoints.existe(I);
			if (lDocumentJoint) {
				if (!lActif) {
