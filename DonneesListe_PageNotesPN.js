exports.DonneesListe_PageNotesPN = void 0;
...
var EGenreCommandeMenuCtxPageNotesPN;
(function (EGenreCommandeMenuCtxPageNotesPN) {
	EGenreCommandeMenuCtxPageNotesPN["modifierCommentaireSurNote"] =
		"modifierCommentaireSurNote";
	EGenreCommandeMenuCtxPageNotesPN["accederCopieKiosque"] =
		"accederCopieKiosque";
})(EGenreCommandeMenuCtxPageNotesPN || (EGenreCommandeMenuCtxPageNotesPN = {}));
class DonneesListe_PageNotesPN extends DonneesListe_PageNotes_1.DonneesListe_PageNotes {
	constructor(aDonnees, aParam) {
		aParam.moteurNotesCP = new MoteurNotesCP_1.MoteurNotesCP(
			new MoteurNotes_1.MoteurNotes(),
		);
		super(aDonnees, aParam);
	}
	composeEleveProjetAcc(aArticle, aSurExportCSV, aPourImpression) {
		if (
			this.param.optionsAffichage.afficherProjetsAccompagnement &&
			!aSurExportCSV &&
			!aPourImpression
		) {
			const lHintProjetAcc = aArticle.projetsAccompagnement || "";
			const lAvecPiecesJointes = !!aArticle.avecDocsProjetsAccompagnement;
	...
	getTraductionMoyenneClasse() {
		return this.param.avecColonneClasse
			? ObjetTraduction_1.GTraductions.getValeur("Notes.MoyenneGroupe") + " :"
			: ObjetTraduction_1.GTraductions.getValeur("Notes.MoyenneClasse") + " :";
	}
	...
	getTitrePeriodes() {
		...
		return (
			!!lMoteurNotesProduit &&
			lMoteurNotesProduit.getTitrePeriodes({
				avecSemestre: this.avecSemestre,
				avecTrimestre: this.avecTrimestre,
			})
		);
	}
	getNoteMaxDevoir() {
		...
		return new TypeNote_1.TypeNote(lValeurNoteMax);
	}
	...
	jsxGetClassBoutonLigneTitre(aIndex) {
		...
		return {
			event: () => {
				...
				if (lDevoir) {
					...
					if (lDevoir.avecCommentaireSurNoteEleve) {
						this.surCommentaireSurNote(aIndex);
						return false;
					}
					if (lDevoir.evaluation) {
						this.surCompetences(aIndex);
	...
	getTitreBouton(aDevoir, aIndex, aStrIeNodeEditionDevoir, aAriaLabel) {
		return {
			getLibelleHtml: () => {
				return [
					`<div ${aStrIeNodeEditionDevoir ? `${aStrIeNodeEditionDevoir} aria-label="${aAriaLabel}" data-tooltip` : ""} class="${Saisie_Notes_css_1.StylesSaisieNotes.devoir} ${Saisie_Notes_css_1.StylesSaisieNotes.devoirPN} AvecMain ${ObjetListe_1.ObjetListe.StyleElementInteractifTitreSansTri}">&nbsp;</div>`,
					aDevoir.evaluation || aDevoir.avecCommentaireSurNoteEleve
						? IE.jsx.str("ie-btnicon", {
								"ie-model": this.jsxBoutonLigneTitre.bind(this, aIndex),
								"ie-class": this.jsxGetClassBoutonLigneTitre.bind(this, aIndex),
								class: [
									Saisie_Notes_css_1.StylesSaisieNotes.devoir,
									Saisie_Notes_css_1.StylesSaisieNotes.btnDevoirSuperpose,
	...
	getSuffixe(aEleveDevoir) {
		if (
			...
			return IE.jsx.str("i", {
				role: "img",
				class: "icon_comment_vide",
				style: "position:absolute; left:0px; bottom:0px;",
				title: `${ObjetTraduction_1.GTraductions.getValeur("Notes.remarque")}: ${aEleveDevoir.commentaire}`,
			});
		}
		return;
	}
	...
	initialisationObjetContextuel(aParams) {
			...
			if (
				...
				const lLibelleCommande =
					lEleveDeDevoir.commentaire && lEleveDeDevoir.commentaire.length > 0
						? ObjetTraduction_1.GTraductions.getValeur(
								"Notes.ActionEditerCommentaireSurnote",
							)
						: ObjetTraduction_1.GTraductions.getValeur(
		...
		if (lEleve && lDevoir && this._estUnNoteDevoirExecKiosque(aParams)) {
			aParams.menuContextuel.add(
				ObjetTraduction_1.GTraductions.getValeur("AfficherCopieEleve"),
				true,
				() => this.surEvntKiosque(lEleve, lDevoir),
			);
		}
		if (aParams.menuContextuel.getListeLignes().count() > 0) {
	...
	surEvntKiosque(aEleve, aDevoir) {
		...
		if (!!lEleveDevoir && !!lEleveDevoir.execKiosque) {
			let lUrl;
			if (lEleveDevoir.execKiosque.noteDEleve) {
				lUrl = ObjetChaine_1.GChaine.creerUrlBruteLienExterne(
					lEleveDevoir.execKiosque.noteDEleve,
				);
			} else {
				lUrl = ObjetChaine_1.GChaine.creerUrlBruteLienExterne(
			...
			if (lUrl) {
				UtilitairePartenaire_1.TUtilitairePartenaire.ouvrirUrl(lUrl, true);
			}
		}
	}
...
exports.DonneesListe_PageNotesPN = DonneesListe_PageNotesPN;
