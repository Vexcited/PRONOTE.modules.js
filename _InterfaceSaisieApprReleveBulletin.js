exports._InterfaceSaisieApprReleveBulletin = void 0;
const InterfacePage_1 = require("InterfacePage");
const InterfacePageAvecMenusDeroulants_1 = require("InterfacePageAvecMenusDeroulants");
const Enumere_Ressource_1 = require("Enumere_Ressource");
const ObjetRequeteApprBulletin_1 = require("ObjetRequeteApprBulletin");
...
class _InterfaceSaisieApprReleveBulletin extends InterfacePage_1.InterfacePage {
	constructor(...aParams) {
		super(...aParams);
	...
	envoyerRequete(aParam) {
		new ObjetRequeteApprBulletin_1.ObjetRequeteApprBulletin(
			this,
	...
	clbckSaisieApprClasse(aParamSucces) {
		...
		this._afficherPiedPage(this.donnees);
	}
	...
	getListeAnnotationsPourAvisReligion() {
		...
		this.listeAnnotations.insererElement(
			ObjetElement_1.ObjetElement.create({
				Libelle: "",
				Numero: 0,
				Position: 0,
				description: ObjetTraduction_1.GTraductions.getValeur("Aucun"),
			}),
			0,
		);
	}
	construireInstances() {
				...
				aListe.setOptionsListe({
					ariaLabel: () => {
						...
						return [
							...
							((_a = this.getClasse()) === null || _a === void 0
								? void 0
								: _a.getLibelle()) || "",
							this.avecComboPeriode === true
								? ((_b = this.getPeriode()) === null || _b === void 0
										? void 0
										: _b.getLibelle()) || ""
								: "",
							((_c = this.getService()) === null || _c === void 0
								? void 0
								: _c.getLibelle()) || "",
						]
							.join(" ")
		...
		this.fenetreDetailJauges = this.addFenetre(
			...
			() => {
				this.etatUtilisateurSco.Navigation.fenetre_BarreNiveauxDAcquisitions_estAffiche = false;
			},
			(aInstance) => {
				const lColonnes = [];
				lColonnes.push({
					id: DonneesListe_BarreNiveauxDAcquisitionsDePilier_1
						.DonneesListe_BarreNiveauxDAcquisitionsDePilier.colonnes.domaine,
				...
				lColonnes.push({
					id: DonneesListe_BarreNiveauxDAcquisitionsDePilier_1
						.DonneesListe_BarreNiveauxDAcquisitionsDePilier.colonnes.jauge,
					titre: ObjetTraduction_1.GTraductions.getValeur(
	...
	construireStructureAffichageAutre() {
		...
		H.push(
			IE.jsx.str("div", {
				class: [Divers_css_1.StylesDivers.pAll],
	...
	_initTripleCombo(aInstance) {
		...
		lCombos.push(Enumere_Ressource_1.EGenreRessource.Service);
		aInstance.setParametres(lCombos, true);
	}
	_evntSurDernierMenuDeroulant() {
		if (
			this.etatUtilisateurSco.Navigation
	...
	_evntSurListe(aParametres) {
		...
		switch (aParametres.genreEvenement) {
			case Enumere_EvenementListe_1.EGenreEvenementListe.Selection:
				...
				switch (lIdCol) {
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						.jaugeEval:
						this._ouvrirFenetreDetailJauges(lLigne);
						break;
				}
				break;
			case Enumere_EvenementListe_1.EGenreEvenementListe.Edition:
				switch (lIdCol) {
								...
								if (aEvolution) {
									this.moteur.saisieEvolution({
										...
										clbckSucces: (aParamSucces) => {
											...
											const lLignes = lDonneesListe.getListeElements(
												(aLigne) => {
													return (
					...
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						...
						aParametres.ouvrirMenuContextuel({
							...
							clbackMenuPositionnement: (aNiveauDAcquisition) => {
								this._saisiePositionnementSurLigneEleve(
									lLigne,
									aNiveauDAcquisition,
								);
							},
					...
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						...
						const lAvisReligion =
							lIdCol ===
							DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
								.avisReligionPropose
						...
						this.moteur.surEditionAvisReligion({
							...
							surValiderEditionDonnee: (aParam) => {
								const lArticle = aParam.article;
								switch (aParam.idColonne) {
									case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin
										.colonnes.avisReligionPropose:
										...
										break;
									case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin
										.colonnes.avisReligionDeliberee:
								...
								GApplication.getMessage().afficher({
									...
									callback: (aGenreAction) => {
										if (
											...
											this._basculerMoyNRDEleve(lLigne);
										}
									},
								});
							} else {
					...
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						.appreciationC: {
						const lAppreciation =
							DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.getAppreciationDeColonne(
								aParametres,
							);
				...
				switch (lIdCol) {
					...
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						...
						if (
							...
							this._envoyerSaisiePropDelib({ article: lLigne, idCol: lIdCol });
						}
						break;
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						.moyDeliberee:
					...
					case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
						...
						this.moteur.saisieAvisProfesseurParcoursup({
							...
							clbckSucces: (aParamSucces) => {
								...
								const lLigne = lLignes.get(0);
								lLigne.appAvisProfParcoursup =
									aParamSucces.appAvisProfParcoursup;
							},
							paramCellSuivante: {
								orientationVerticale: this.estOrientationVerticale,
							},
	...
	_modifierNiveauDeSelectionCourante(aNiveau) {
		...
		this._saisiePositionnementSurLigneEleve(lLigneEleve, aNiveau);
	}
	_saisiePositionnementSurLigneEleve(
		aLigneEleve,
		aNiveauDAcquisition,
		...
		if (aNiveauDAcquisition && aLigneEleve.niveauAcquEditable) {
			...
			if (lAvecChangement) {
				const lClasse = this._getInfoClasseEleve(aLigneEleve);
				const lParamsMoteurSaisie = {
					...
					paramCellSuivante: aAvecPassageLigneSuivante
						? lParamsCallSuivante
						: null,
				};
	...
	_basculerMoyNRDEleve(aLigneEleve) {
	...
	_ouvrirFenetreDetailJauges(aLigne) {
	...
	_getModeValidationAuto(aListeCols, aCol) {
	...
	_getListeElevesPourValidationAuto(aDonnees, aCol) {
			...
			switch (aCol) {
				...
				case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
					...
					if (aLigne.estPere && aLigne.ECTSEditable === true) {
						...
						lElt.classe = this._getInfoClasseEleve(aLigne);
	...
	_afficherBulletin(aParam) {
		...
		const lParamDonneesListe = {
			...
			clbckJauge: () => {
				...
				if (
					...
					const lLignes = lDonneesListe.getListeElements((aLigne) => {
						return (
							aLigne.eleve.getNumero() === lEleve.getNumero() &&
							aLigne.estService
						);
					});
					if (lLignes.count() > 0) {
			...
			clbckSigmaECTS: () => {
				this.moteur.surBoutonCalculAutoECTS(
					this._getParamBtnCalculAutoECTS(lDonneesListe, aParam.listeColonnes),
				);
			},
			clbckSigmaLSU: () => {
				this.moteur.surBoutonCalculAuto(
					this._getParamBtnCalculAuto({
						donneesListe: lDonneesListe,
						col: DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
							.noteLSU,
		...
		this.getInstance(this.identListe).setDonnees(
			new DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin(
				lDonneesListe,
				lParamDonneesListe,
			),
		);
		...
		switch (lCol) {
			...
			case DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes
				...
				return {
					...
					modeCalculAuto: this._getModeValidationAuto(
						aParam.listeColonnes,
						lCol,
					),
					clbckCalculAuto: this._surClbckCalculAutoLSU.bind(this),
	...
	_getParamBtnCalculAutoECTS(aLignesApprBulletin, aListeColonnes) {
		const lCol =
			DonneesListe_ApprBulletin_1.DonneesListe_ApprBulletin.colonnes.ects;
		const lListeEleves = this._getListeElevesPourValidationAuto(
			aLignesApprBulletin,
			lCol,
		);
		const lInfosCol = this.moteur.getInfosCol(aListeColonnes, lCol);
		return {
		...
		if (lListe !== null && lListe !== undefined) {
			...
			switch (aParam.cmd) {
				case EBoutonFenetreAssistantSaisie_1.EBoutonFenetreAssistantSaisie
					.Valider:
					lClbck = () => {
						this._validerAppreciation(aParam, { contexte: lContexte });
					};
					break;
				case EBoutonFenetreAssistantSaisie_1.EBoutonFenetreAssistantSaisie
					.PasserEnSaisie:
	...
	_envoyerSaisiePropDelib(aParam) {
		const lArticle = aParam.article;
...
exports._InterfaceSaisieApprReleveBulletin = _InterfaceSaisieApprReleveBulletin;
