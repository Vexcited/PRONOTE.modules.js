...
class ObjetFicheEtablissement extends ObjetFiche_1.ObjetFiche {
	...
	setDonnees(aParams) {
		...
		this.listeEtablissements = new ObjetListeElements_1.ObjetListeElements();
		if (lListeEtab) {
	...
	getOptionsParDefaut() {
		...
		const lEstEspaceAvecMembre = [
			...
			(_a =
				this === null || this === void 0 ? void 0 : this.etatUtilisateurPN) ===
				null || _a === void 0
				? void 0
				: _a.GenreEspace,
		);
	...
	composeInformations(aEtablissement) {
		...
		if (
			...
			if (lEtablissement.Coordonnees.LibellePostal) {
				H.push("<span>", lEtablissement.Coordonnees.LibellePostal, "</span>");
			}
		...
		if (
			...
			H.push(
				...
				"</p>",
				"<p>",
				this._composeEmail(lEtablissement.Coordonnees.EMailPersonnalise1.Mail),
				"</p></div></div>",
			);
		}
		if (
			...
			H.push(
				...
				"</p>",
				"<p>",
				this._composeEmail(lEtablissement.Coordonnees.EMailPersonnalise2.Mail),
				"</p></div></div>",
			);
		}
		let lFichier;
