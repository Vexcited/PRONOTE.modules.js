...
class ObjetForumVisuPosts extends ObjetIdentite_1.Identite {
	...
	jsxVisibilityObserverNodeLoader(aTypeLoader, aVisible) {
		if (aVisible) {
			this._actualiserListePostsPromise({
				sujet: this.donnees.sujet,
				avecSujet: false,
	...
	jsxVisibilityObserverPostNonLu(aNumeroPost, aVisible) {
		if (aVisible) {
			const lListe = this.donnees.listePosts;
			const lIndice = lListe.getIndiceParNumeroEtGenre(aNumeroPost);
			const lPost = lListe.get(lIndice);
			if (lPost) {
				this.moteurForum.setIndicePostLu(lIndice);
			}
			return false;
		}
	}
	jsxVisibilityObserverPost(aIndicePost, aVisible) {
		if (aVisible) {
			this.ensemblePostsVisibles.add(aIndicePost);
		} else {
			this.ensemblePostsVisibles.remove(aIndicePost);
		}
	}
	...
	_construireHeaderSujet() {
		...
		if (lTabLigneSupp.length > 0) {
			H.push(
				IE.jsx.str("div", { class: "ligne-supp" }, lTabLigneSupp.join("")),
			);
		}
	...
	_construireMessageInformatifDeSujet(aSujet) {
		...
		if (H.length > 0) {
			return IE.jsx.str("div", { class: "message-sujet" }, H.join("<br />"));
		}
		return "";
	}
		...
		H.push(
			(0, tag_1.tag)("div", { class: "commandes" }, (H) => {
				...
				if (aPost.actions.count() > 0) {
					...
					H.push(IE.jsx.str("div", { class: "gabarit-icon" }));
				}
			}),
		);
		return H.join(" ");
	...
	_construireTitreAuteurPost(aPost) {
		return (0, tag_1.tag)("div", { class: "titre-auteur" }, () => {
			const H = [];
			H.push(IE.jsx.str("label", { class: "auteur" }, aPost.strAuteur));
			if (aPost.exclu) {
				H.push(
					IE.jsx.str(
						IE.jsx.fragment,
			...
			H.push(IE.jsx.str("label", { class: "date" }, aPost.strDate));
			if (aPost.editeAuteur || aPost.editeModeration) {
				const lStrEditeAuteur = aPost.editeAuteur
					? ObjetTraduction_1.GTraductions.getValeur("ForumPeda.ModifieAuteur")
					: ObjetTraduction_1.GTraductions.getValeur(
							"ForumPeda.ModifieModeration",
						);
				H.push(
					IE.jsx.str("label", { class: "edite" }, "(", lStrEditeAuteur, ")"),
				);
			}
			return H.join(" ");
		});
	...
	_construireTitrePost(aPost) {
		...
		if (lEtat) {
			return IE.jsx.str(
				IE.jsx.fragment,
				null,
				IE.jsx.str("div", { class: "principal" }, lEtat, lCommandes),
				IE.jsx.str("div", { class: "secondaire" }, lTitreAuteur),
			);
		}
		return IE.jsx.str("div", { class: "principal" }, lTitreAuteur, lCommandes);
	}
	_construireListePosts() {
		...
		if (this.donnees.loaderPostBefore) {
			H.push(
				IE.jsx.str(
					...
					{
						class: "posts-load",
						"ie-visibility-observer": this.jsxVisibilityObserverNodeLoader.bind(
							this,
							"before",
						),
					},
		...
		lListePosts.parcourir((aPost, aIndex) => {
			...
			let lContenuSource = "";
			if (aPost.contenuSource) {
				lContenuSource = IE.jsx.str(
					"div",
					{ class: "quotation" },
					IE.jsx.str("label", null, aPost.strAuteurSource),
					IE.jsx.str("div", null, aPost.contenuSource),
				);
			}
			let lMarqueurLuComplet = "";
			if (aPost.nonLu) {
				lMarqueurLuComplet = IE.jsx.str("div", {
					"ie-visibility-observer": this.jsxVisibilityObserverPostNonLu.bind(
						this,
						aPost.getNumero(),
					),
				});
			}
			const lIdArticle = "post_" + aPost.getNumero();
			const lClassesArticlePost = ["post", aPost.etatHeader];
			if (aPost.estAuteur) {
				lClassesArticlePost.push("auteur");
			}
			H.push(
				IE.jsx.str(
					"article",
					{
						id: lIdArticle,
						class: lClassesArticlePost.join(" "),
						"ie-visibility-observer": this.jsxVisibilityObserverPost.bind(
							this,
							aIndex,
						),
						tabindex: "0",
	...
	_construireMessageVide(aPourSujet) {
		...
		return IE.jsx.str(
			...
			IE.jsx.str("div", { class: "message card-content" }, lMessage),
	...
	_construireAffichage() {
		...
		if (this.donnees.sujet) {
			lContenu.push(this._construirePosts());
		} else if (!IE.estMobile) {
			lContenu.push(this._construireMessageVide(true));
		}
		const H = [];
		H.push(
			IE.jsx.str("div", { class: "ObjetForumVisuPosts" }, lContenu.join("")),
		);
		return H.join("");
	}
	_peutPoster() {
	...
	_estVisiteurDuSujet() {
		...
		return (
			((_b =
				(_a = this.donnees.sujet) === null || _a === void 0
					? void 0
					: _a.roles) === null || _b === void 0
				? void 0
				: _b.count()) === 1 &&
			((_d =
				(_c = this.donnees.sujet) === null || _c === void 0
					? void 0
					: _c.roles) === null || _d === void 0
				? void 0
				: _d.contains(
