...
class TypeNote {
		...
		if (typeof aParametre === "string") {
			...
			if (
				...
				if (lAvecFelicitationAutorisee) {
					...
					try {
						const lSurplusBareme = TypeNote.getValeurSurplusBaremeAAppliquer(
							this.bareme,
						);
	...
	static estCaractereNote(
		...
		aAvecLettre =
			aAvecLettre === null || aAvecLettre === undefined ? true : aAvecLettre;
		aAvecVirgule =
			aAvecVirgule === null || aAvecVirgule === undefined ? true : aAvecVirgule;
		aAvecMoins =
	...
	static validerNote(aValeur, aOptions = {}) {
		...
		if (
			...
			const lMessage = ObjetChaine_1.GChaine.format(lOptions.messageMinMax, [
