...
class ObjetDiscussion_Mobile extends ObjetIdentite_Mobile_1.ObjetIdentite_Mobile {
		...
		if (this.options.estDiscussionEnFenetre) {
			...
			if (
				...
				this._fenetreWrapper.afficher(
					IE.jsx.str("div", { id: this.getNom(), style: "height: 100%" }),
				);
	...
	jsxModeleBoutonRepondre(aGenre) {
		return {
			event: () => {
				this._surBoutonRepondre(aGenre);
			},
			getDisabled: () => {
	...
	jsxModeleChipsDestinataire(aGenre) {
		return {
			event: (aEvent) => {
				this._popupDestinataires(aEvent, aGenre);
			},
			eventBtn: (aEvent) => {
				aEvent.stopPropagation();
				this.newMsg.listeDestinataires =
					this.newMsg.listeDestinataires.getListeElements((aDest) => {
	...
	getControleur(aInstance) {
		return $.extend(true, super.getControleur(aInstance), {
				...
				event: function (aParametres, aInstanceCombo) {
					if (
						...
						if (aParametres.element.getNumero() === -1) {
							...
							new UtilitaireContactVieScolaire_1.ObjetRequeteSaisieContactVieScolaire(
								{},
							)
			...
			destList: {
				event(aEvent) {
					aInstance._popupDestinataires(aEvent, undefined);
				},
				getIcone() {
					return "icon_user";
				},
							...
							if (lNb > 0) {
								lHtml.push(
									IE.jsx.str(
										"ie-chips",
										{
											"ie-model": aInstance.jsxModeleChipsDestinataire.bind(
												aInstance,
												aEle.getGenre(),
											),
											class: "chips-dest",
	...
	static creerCarnetDeLiaisonPrimParent() {
		...
		const lAffDiscussion = new ObjetDiscussion_Mobile({
	...
	_initNewMsg(aBrouillon, aRecupererModeleAlerte) {
		...
		if (this.donnees.tableauDeBord) {
			...
			this.newMsg.listeDestinataires.parcourir((D) => {
		...
		if (this.donnees.estCreationRechercheDeStage) {
			this.newMsg.estCreationRechercheDeStage = true;
			this.newMsg.rechercheDeStage = this.donnees.rechercheDeStage;
			this.newMsg.objet = this.donnees.message.objet;
		}
	...
	_addCommandesMenuPiedSaisie(
			...
			if (lAvecCloud) {
				aMenu.add(
					...
					() => {
						...
						if (this.etatUtilisateurSco.avecCloudENEJDisponible()) {
							const lActionENEJ =
								ObjetFenetre_ActionContextuelle_1.ObjetFenetre_ActionContextuelle.getActionENEJ(
									() => {
										UtilitaireGestionCloudEtPDF_1.UtilitaireGestionCloudEtPDF.ouvrirFenetreChoixFichierCloud(
											{
												service: this.etatUtilisateurSco.getCloudENEJ(),
												instance: this,
											},
										);
									},
		...
		if (aAvecSignature) {
			aMenu.add(
				...
				async () => {
					var _a;
					this._setValeurMessage(
						this.newMsg.contenu +
							this.etatUtilisateurSco.messagerieSignature.signature,
						this.options.avecHtml,
					);
					if (this.options.avecHtml) {
						const lParamsTiny = await TinyInit_1.TinyInit.onLoadEnd(
							this.idTiny,
						);
						(_a =
							lParamsTiny === null || lParamsTiny === void 0
								? void 0
								: lParamsTiny.tiny) === null || _a === void 0
							? void 0
							: _a.setContent(this.newMsg.contenu);
					}
					this.$refreshSelf();
				},
				{ icon: "icon_signature" },
	...
	_surBoutonRepondre(aGenre) {
		...
		if (this.options.avecHtml) {
			lContenuEscape = $(
				ObjetHtml_1.GHtml.htmlToDOM("<div>" + lContenuEscape + "</div>"),
			)
				.text()
				.trim();
		}
		...
		if (
			(lListeDest === null || lListeDest === void 0
				? void 0
				: lListeDest.count()) > 0
		) {
			this.newMsg.listeDestinataires = lListeDest;
		}
				...
				if (aIndex === this.donnees.listeBoutons.count() - 1) {
					H.push(
						IE.jsx.str("ie-bouton", {
							...
							"ie-model": this.jsxModeleBoutonRepondre.bind(
								this,
								aBouton.getGenre(),
							),
						}),
	...
	_composer() {
				...
				IE.jsx.str(
					...
					this.options.estDiscussionEnFenetre
						...
						: IE.jsx.str("ie-btnicon", {
								...
								class: "icon_retour_mobile retour",
							}),
					this.options.estDiscussionEnFenetre
						? ""
						: IE.jsx.str(
								"span",
								{ class: "odm_header_libelle ellipsis-multilignes mdr2" },
								lLibelle,
							),
		...
		if (
			...
			H.push(
				IE.jsx.str(
					"div",
					{ style: "color:var(--color-red-moyen);" },
		...
		if (lAvecZoneSaisie && !lDiscussionDesactiveeSelonHoraire) {
			...
			const lAvecModifDestinataires =
				this.options.avecChoixDestinataires &&
				!lMessageNonEditable &&
			...
			if (!this.options.avecChoixDestinataires) {
				H.push(
					IE.jsx.str(
						"div",
						{
						...
						IE.jsx.str("div", {
							...
							class: "fluid-bloc p-bottom-l m-left-xl m-top-l",
						}),
					),
				);
			}
	...
	_formatterDonneesParGenreRessource() {
		function _infosDest(aNb, aHint) {
			return { nbDest: aNb, arrHint: aHint, hintDest: () => aHint.join("\r") };
		}
		let lNb = 0;
		const lHint = [];
		switch (this.options.genreRessource) {
			case Enumere_Ressource_1.EGenreRessource.Eleve: {
				...
				let lHintResp1 = [],
					...
					lHintEquipePeda = [];
				for (
					let i = 0, lNbr = this.newMsg.listeDestinataires.count();
					i < lNbr;
					i++
				) {
					const lElt = this.newMsg.listeDestinataires.get(i);
				...
				return {
					eleves: _infosDest(lNb, lHint),
					resp1: _infosDest(lNbResp1, lHintResp1),
					resp2: _infosDest(lNbResp2, lHintResp2),
					pp: _infosDest(lNbPP, lHintPP),
					tuteurs: _infosDest(lNbTuteurs, lHintTuteurs),
					equipePeda: _infosDest(lNbEquipePeda, lHintEquipePeda),
				};
			}
			...
			case Enumere_Ressource_1.EGenreRessource.Personnel:
				...
				return { destinataires: _infosDest(lNb, lHint) };
			default:
				break;
		}
	}
	...
	_setDestinatairesParGenreRessource() {
		function _getTraducGenreRessource(aGenreRessource) {
			switch (aGenreRessource) {
				...
				case Enumere_Ressource_1.EGenreRessource.Responsable:
					return ObjetTraduction_1.GTraductions.getValeur("Messagerie.Parents");
				case Enumere_Ressource_1.EGenreRessource.Enseignant:
					return ObjetTraduction_1.GTraductions.getValeur("Messagerie.Profs");
				case Enumere_Ressource_1.EGenreRessource.Personnel:
					return ObjetTraduction_1.GTraductions.getValeur(
				...
				case Enumere_Ressource_1.EGenreRessource.Personnel:
					if (this.destinataires.destinataires.nbDest > 1) {
						T.push(
							...
							'">',
							_getTraducGenreRessource(this.options.genreRessource),
							" (",
	...
	_getListeTypeDestinatairesPossiblesPourEleve() {
		...
		if (
			this.options.genreRessource === Enumere_Ressource_1.EGenreRessource.Eleve
		) {
			...
			if (
				...
				if (this.destinataires.tuteurs.nbDest > 1) {
					...
					lLabelTuteur =
						...
						" (" +
						ObjetTraduction_1.GTraductions.getValeur("Messagerie.abbrTuteur") +
						")";
				}
				lAutresDestinataires.push({
					hint: lHintTuteur,
		...
		if (!this.options.avecChoixDestinataires) {
			...
			switch (this.options.genreRessource) {
				case Enumere_Ressource_1.EGenreRessource.Eleve: {
					...
					const lAvecEquipePeda = this._estDestinatairePossibleEstCocheEtActif(
				...
				case Enumere_Ressource_1.EGenreRessource.Personnel:
					if (
						...
						lListe.add(this.newMsg.listeDestinataires);
					}
					break;
				default:
					break;
