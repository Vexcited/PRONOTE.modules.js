...
class FicheCours extends ObjetFiche_1.ObjetFiche {
	constructor(...aParams) {
		...
		this._options = {
			hauteurDecroche: ObjetNavigateur_1.Navigateur.isTactile ? 20 : 15,
			largeurContenu: 220,
	...
	jsxModeleBoutonMemo(aIndiceCours) {
		return {
			event: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				if (this._parametres.ouvrirFenetreMemo) {
					this._parametres.ouvrirFenetreMemo({
						cours: lCours,
						nonEditable: !this._editionMemoPossible(lCours),
					});
				}
			},
			getDisabled: () => {
				return !this._parametres.ouvrirFenetreMemo;
			},
		};
	}
	jsxModeleMemoPublic(aIndiceCours) {
		return {
			getValue: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				return lCours.memo;
			...
			setValue: (aValue) => {
				...
				ControleSaisieEvenement_1.ControleSaisieEvenement.addSaisieEnCours(
					this._parametres.callbackSaisieMemo.bind(this, lCours, lCours.memo),
				);
			},
			getDisabled: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				return (
					!this._parametres.callbackSaisieMemo ||
					!this._editionMemoPossible(lCours)
				);
			},
			exitChange: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
	...
	jsxModeleMemoPrive(aIndiceCours) {
		return {
			getValue: () => {
				return this._parametres.listeCours.get(aIndiceCours).memoPrive;
			},
		};
	}
	jsxModeleBoutonListeVisiosCours(aIndiceCours) {
		return {
			event: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				UtilitaireVisiosSco_1.UtilitaireVisios.ouvrirFenetreEditionVisiosCours(
					lCours,
					this._parametres.callbackSaisieVisios,
				);
			},
			getDisabled: () => {
	...
	jsxModeleBoutonCDT(aIndiceCours) {
		return {
			event: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				if (lCours && this._parametres.callbackCDT) {
					this._parametres.callbackCDT(lCours, lCours.numeroSemaine);
				}
			},
		};
	}
	jsxModeleBoutonSuppression(aIndiceCours) {
		return {
			event: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				this._parametres.callbackSuppressionCours(lCours, lCours.numeroSemaine);
			},
			getDisabled: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				return (
					!lCours ||
					!this._parametres.suppressionCoursActif ||
					!this._parametres.suppressionCoursActif(lCours)
				);
			},
		};
	}
	jsxModeleBoutonDeplacerAutreSemaine(aIndiceCours) {
		return {
			event: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				this._parametres.callbackDeplacerCoursAutreSemaine(lCours);
			},
			getDisabled: () => {
				const lCours = this._parametres.listeCours.get(aIndiceCours);
				return (
					!lCours ||
					(aIndiceCours === 0 && this._parametres.listeCours.count() > 1) ||
					!this._parametres.rechercheCreneauLibreActif ||
					!this._parametres.rechercheCreneauLibreActif(lCours)
				);
			},
		};
	}
	jsxModeleBoutonAfficherLegende() {
		return {
			event: () => {
	...
	_composeDecrocheTop() {
		...
		if (
			...
			H.push(
				IE.jsx.str("ie-btnimage", {
					class: "btnImageIcon icon_info_sign",
					"ie-model": this.jsxModeleBoutonAfficherLegende.bind(this),
					style:
						"position:relative; top:-1px; font-size:" +
						lFontSizeIcon +
						"px;margin-right:5px;",
		...
		H.push(
			IE.jsx.str("ie-btnimage", {
				...
				style: { position: "relative", top: 0, "font-size": lFontSizeIcon + 1 },
				title: ObjetTraduction_1.GTraductions.getValeur("WAI.FermerFiche"),
			}),
		);
		H.push("</div>");
		H.push("</div>");
	...
	_composeCours(aIndiceCours, aAvecScrollH, aEtatListeCours) {
		...
		if (this._parametres.afficherMemo) {
			if (lAvecEditionMemo || lCours.memo) {
				H.push(
					IE.jsx.str("div", null, this._construireMemo(aIndiceCours, true)),
				);
			}
			if (lCours.memoPrive) {
				H.push(
	...
	_construireMemo(aIndiceCours, aMemoPublic) {
			...
			H.push(
				IE.jsx.str(
					...
					IE.jsx.str("ie-btnicon", {
						"ie-model": this.jsxModeleBoutonMemo.bind(this, aIndiceCours),
						class: "icon_post_it_rempli color-neutre",
						title: ObjetTraduction_1.GTraductions.getValeur("EDT.AfficherMemo"),
		...
		H.push("</div>");
		let lJsxTextAreaMemo;
		if (aMemoPublic) {
			lJsxTextAreaMemo = this.jsxModeleMemoPublic.bind(this, aIndiceCours);
		} else {
			lJsxTextAreaMemo = this.jsxModeleMemoPrive.bind(this, aIndiceCours);
		}
		H.push(
			IE.jsx.str("ie-textareamax", {
				"ie-model": lJsxTextAreaMemo,
				"aria-labelledby": lId,
				style:
	...
	_composeBoutonsBarreOutils(aCours, aIndiceCours) {
		...
		if (
			...
			HGauche.push(
				IE.jsx.str(
					...
					{ class: "BoutonOutils" },
					IE.jsx.str("ie-btnimage", {
						"ie-model": this.jsxModeleBoutonListeVisiosCours.bind(
							this,
							aIndiceCours,
						),
						class: [
		...
		if (this._parametres.afficherCDT && aCours.avecCDT) {
			HGauche.push(
				IE.jsx.str(
					...
					{ class: "BoutonOutils" },
					IE.jsx.str("ie-btnimage", {
						"ie-model": this.jsxModeleBoutonCDT.bind(this, aIndiceCours),
						class: "Image_CahierDeTexte6Etats",
						style: "width:19px;",
						"ie-tooltiplabel":
		...
		if (
			...
			HDroit.push(
				IE.jsx.str(
					...
					{ class: "BoutonOutils" },
					IE.jsx.str("ie-btnimage", {
						class: "Image_FicheCoursBtnDeplacerFiche",
						style: "width:18px;",
						"ie-model": this.jsxModeleBoutonDeplacerAutreSemaine.bind(
							this,
							aIndiceCours,
						),
						title: ObjetTraduction_1.GTraductions.getValeur(
		...
		if (
			...
			const lClasseImage = !lAnnulation
				? "Image_FicheCoursBtnSupprimerFiche"
				: "Image_FicheCoursBtnAnnulerFiche";
			const lTitleBouton = !lAnnulation
				? ObjetTraduction_1.GTraductions.getValeur("SaisieCours.SupprimerCours")
				: ObjetTraduction_1.GTraductions.getValeur(
			...
			HDroit.push(
				IE.jsx.str(
					...
					IE.jsx.str("ie-btnimage", {
						class: lClasseImage,
						style: "width:18px;",
						"ie-model": this.jsxModeleBoutonSuppression.bind(
							this,
							aIndiceCours,
						),
						title: lTitleBouton,
		...
		const H = [HGauche.join("")];
		if (HDroit.length > 0) {
			H.push('<div style="flex:2 0 auto;"></div>');
		}
		H.push(HDroit.join(""));
		return H.join("");
	}
	_construireBlocEntete(aCours, aIndiceCours, alargeur, aEtatListeCours) {
		...
		return IE.jsx.str(
			"div",
			{
				class: ["EnteteCours", lAvecClickEntete ? "AvecMain" : ""],
				"ie-node": lAvecClickEntete
					? "getNodeEntete(" + aIndiceCours + ")"
					: "",
			...
			(H) => {
				...
				if (
					(aCours.Statut || aEtatListeCours.avecStatut) &&
					!lEstConseilDeClasse
				) {
					if (!aCours.Statut) {
				...
				if (aCours.estGAEV) {
					...
					HImagesLigne.push(
						IE.jsx.str(
							"div",
							{ style: "margin-left:3px; font-size: 1.6rem;" },
							IE.jsx.str("i", {
						...
						IE.jsx.str(
							"span",
							{
								...
								style:
									...
									aCours.libelleCours.couleur +
									"; --color-text-edt-libellecours: " +
									lColor +
									";",
								"ie-tooltiplabel": aCours.libelleCours.getLibelle(),
							},
							aCours.libelleCours.abbr,
						),
					);
			...
			lParametres.ressources[Enumere_Ressource_1.EGenreRessource.LibelleCours] =
				{
					...
					avecAjout: lAvecModifLibelleCours,
					avecAjoutSeulementSiVide: true,
					avecEdition: lAvecModifLibelleCours,
				};
		}
