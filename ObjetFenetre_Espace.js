...
class ObjetFenetreEspace extends _ObjetFenetre_1._ObjetFenetre {
	...
	construireBouton(aBoutonRepeat, aMargin) {
		return [
			...
			aBoutonRepeat.element.title
				? ObjetHtml_1.GHtml.composeAttr("title", aBoutonRepeat.element.title)
				: "",
			">",
			"</ie-bouton>",
		].join("");
	...
	_initialiserOptionsFenetre() {
		this.setOptionsFenetre({
			...
			couleurFondBandeau: (0, AccessApp_1.getApp)().getCouleur().fenetre.bandeau
				.fond,
	...
	getDragFenetre() {
		return {
			start: (aParamsDrag) => {
				...
				aParamsDrag.rectFenetreStart = ObjetPosition_1.GPosition.getClientRect(
					lJFenetre.get(0),
				);
	...
	getDragResize(aNode) {
		let lCoin = {
			...
			droit:
				aNode.classList.contains("right") || aNode.classList.contains("coin"),
			haut: aNode.classList.contains("top"),
			bas:
				aNode.classList.contains("bottom") || aNode.classList.contains("coin"),
		};
		return {
			start: (aParamsDrag) => {
				const lClassCurseur =
