...
class ObjetListe extends ObjetIdentite_1.Identite {
	...
	getControleur(aInstance) {
		const lControleur = {
			...
			getNodeAferToolbar() {
				const lTabElements = ObjetHtml_1.GHtml.getElementsFocusablesDElement(
					this.node,
				);
				...
				node(
					...
					$(this.node).on({
						keydown: function (aEvent) {
							...
							if (
								aInstance.Donnees.options.avecNavigationClavierFlechesEnEdition
							) {
			...
			getClassCellulePere(aLigne, aColonne) {
				const lParamsCellule = aInstance._getParamsCellule(aColonne, aLigne);
				return (
				...
				switch (aBouton.genre) {
					...
					case ObjetListe.typeBouton.deployer:
						...
						lBouton.getClass = (aParam) => {
							...
							return lEstDeploye ? "icon_fleche_num_bas" : "icon_fleche_num";
						};
	...
	composeWAICommun() {
		return IE.jsx.str(
			...
			IE.jsx.str(
				...
				ObjetTraduction_1.GTraductions.getValeur("liste.wai.DescriptionTri"),
			),
			IE.jsx.str(
				"span",
