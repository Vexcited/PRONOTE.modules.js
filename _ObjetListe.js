...
class ObjetListe extends ObjetIdentite_1.Identite {
	...
	actualiser(aObjetOuConserverSelection, aSansTriDonnees) {
		const lParametres = { ignorerFocusListe: false, ignorerCalculTri: true };
	...
	selectionnerCellule(aParametres) {
		this._cache.lignesSelectionnees_prec = [...this._cache.lignesSelectionnees];
		this._selectionner(
			Object.assign({ avecEvenementModificationSelection: true }, aParametres),
		);
	}
	getSelection() {
	...
	construireCopieCSV() {}
	getPositionScrollV() {
		const lBackupScroll = this._backupScroll({ conserverPositionScroll: true });
							...
							if (this.Donnees && this.Donnees.Donnees) {
								...
								this.Donnees.Donnees.parcourir((aElement, aLigne) => {
									...
									if (
										...
										if (
											this.Donnees._estDeploye(aLigne) === aParam.bouton.deploye
										) {
											return;
										}
										lAvecModif = true;
	...
	composeWAICommun() {
			...
			if (this._cache.avecWAICocheLigne) {
				T.push(
					IE.jsx.str(
						"span",
						null,
						ObjetTraduction_1.GTraductions.getValeur("liste.wai.ModifierCoche"),
					),
				);
			}
			return T.join("");
		...
		return IE.jsx.str(
			...
			IE.jsx.str(
				"span",
				{ class: Divers_css_1.StylesDivers.srOnly, id: this.ids.WAILigneLue },
		...
		return LDonneesListe.getEtatCocheSelonFils(
			...
			(aParamsCellule) => {
				return (
					...
					(!aIgnorerCBDisabled || !LDonneesListe.getDisabledCB(aParamsCellule))
				);
			},
			aLigne === -1,
		);
	...
	_setValueCBFlatDesign(aParams, aValue, aParentCocheTout) {
		...
		let lLigneCacheeRecherche = !!(
			aParams.article && this._cache.rechercheTexte.lignesCachees[aParams.ligne]
		);
		if (
			aParams.article &&
	...
	async _clickSurCocheTitre(aColonne) {
		...
		const lEtatCoche = lDonneesTableau.getEtatCocheSelonFils(
			...
			lValeur =
				lEtatCoche !== ObjetDonneesListe_1.ObjetDonneesListe.EGenreCoche.Verte,
		...
		if (this.ListeTailles && this.Donnees) {
			...
			lCache.lignesVisibles.forEach((aLigneDonnees) => {
				...
				if (
					...
					(lProfondeur > 1 && lStructPrec && lProfondeur > lStructPrec.level)
				) {
					lInfosProfondeurParent = {
						count: 0,
									...
									iLigne === 0
										? lValeur !== undefined && lValeur !== null
											...
											: lNumeroColonne === 0
												...
												: TypeFusionTitreListe_1.TypeFusionTitreListe
														.FusionGauche
										: _completerCelluleVide.call(this, iLigne, lNumeroColonne);
							}
							if (
		...
		lStructure.forEach((aStructure, aIndiceRange) => {
			...
			try {
				...
				for (
					...
					lInfos = { ligne: lCache.lignesVisibles[lIndiceLigne], texte: "" };
					...
					lCache.infosZonesColonnes.forEach((aInfosZoneColonnes, aIndex) => {
						...
						lInfos.texte +=
							" " +
							ComparateurChaines_1.ComparateurChaines.normalize(lTab.join(" "));
					});
				}
			} finally {
	...
	_setDonnees(ADonnees, aLigneSelectionne, aParams) {
		...
		if (this.Donnees) {
			this.Donnees._setParamsListe({
				...
				jsxGetModelCBLigneFlatDesign: this.jsxGetModelCBLigneFlatDesign.bind(
					this,
					this,
				),
			...
			if (MethodesObjet_1.MethodesObjet.isArray(lOptionsTriParDefaut)) {
				...
				for (i = 0; i < lOptionsTriParDefaut.length; i++) {
					...
					if (MethodesObjet_1.MethodesObjet.isObject(lOptTri)) {
						...
						lNumeroColonne = lOptTri.numero || 0;
						lGenreColonne =
							lOptTri.genre || Enumere_TriElement_1.EGenreTriElement.Croissant;
					} else {
						lNumeroColonne =
				...
				if (MethodesObjet_1.MethodesObjet.isObject(lOptionsTriParDefaut)) {
					...
					lNumeroColonne =
						this._getNumeroColonneSelonNumeroOuId(lOptionsTriParDefaut, true) ||
						0;
					lGenreColonne = Enumere_TriElement_1.EGenreTriElement.Croissant;
				}
				lNumeroColonne = _chercherColonneTriValide.call(
	...
	_getLargeurPartieScrollV() {
		return Math.max(
			...
			this._avecBoutonsListeHautScroll() ? this._options.tailleBoutons + 1 : 0,
		);
	}
	_getLargeurScrollV() {
		return 0;
		...
		for (i = 0; i < this.ListeTailles.length; i++) {
			if (this._estColonneVisible(i)) {
				...
				if (lTailleCol && lTailleCol.valeur && !lTailleREM.estREM) {
					...
					Object.assign(lTaillesColonne[i], lElement, { estPourcent: true });
	...
	surSuppressionEvenement(aParams) {
		...
		if (
			this.Donnees &&
			(aParams.ligne > -1 ||
				(aParams.listeSuppressions && aParams.listeSuppressions.count() > 0))
		) {
			if (this.estDonneeListeFlatDesign(this.Donnees)) {
				return false;
			}
		...
		if (this.Donnees) {
			if (this.Donnees.avecEvenementApresErreurCreation()) {
				if (this.Pere && this.Evenement) {
					const lResult = this.callback.appel(
						this._getParamsCallback(
							Enumere_EvenementListe_1.EGenreEvenementListe.ApresErreurCreation,
							null,
							null,
						),
						Enumere_EvenementListe_1.EGenreEvenementListe.ApresErreurCreation,
	...
	entrerTexte(
		...
		let LValeur = this.Donnees._getValeur(lParamsCellule),
			lClass = "";
		const lEstNote =
			aTypeCellule === ObjetDonneesListe_1.ObjetDonneesListe.ETypeCellule.Note;
		if (!lEstNote) {
		...
		ObjetHtml_1.GHtml.setHtml(this.IdCellule, H.join(""), { instance: this });
	}
	_getInfosZoneColonneDeNumeroColonne(aNumeroColonne) {
		...
		return (
			lInfosZone ||
			this._cache.infosZonesColonnes[this._cache.infosZonesColonnes.length - 1]
		);
	}
	...
	entrerZoneTexte(
		...
		const lPositionnement = {
			...
			top:
				lOffset.top + (lEstLigneCreationTitre ? 0 : -1) - lOffsetConteneur.top,
		};
		if (
	...
	_surSelection(aNumeroColonne, aNumeroLigne, aParams) {
		...
		if (lParams.avecFocus) {
			...
			if (this._estLigneDansRangeNonConstruitDyn(aNumeroLigne)) {
				this._scrollSurLigne({ ligne: aNumeroLigne, colonne: lNumeroColonne });
			}
	...
	setCadreSelectionInterne(aParams, aArrayTraitements) {
		...
		const lIdSelecCellule =
			this.ids.cadreSelection + "_sel_" + aParams.ligne + "_" + aParams.colonne;
		...
		if (aParams.selection) {
			...
			const lFusionLigneIdentique = function (aColonne) {
				if (
					!this._cache.lignesFusionParColonne[aColonne + "_" + aParams.ligne] &&
			...
			if (
				...
				aParams.posCellulePrec.selColSuivante === lPositionCache.selColSuivante
			) {
				return;
			}
			const lStyle =
		...
		if (arguments.length > 1) {
			if (aSelectionne) {
				...
				lParametres.cacheSelection[lParametres.ligne][lParametres.colonne] =
					false;
				if (
					lParametres.cacheSelection[lParametres.ligne].every((aColonne) => {
				...
				if (this._estLigneVisible(aLigne, { contexteSelection: true })) {
					aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
						const lPosCellule =
							...
							this._cache.positionsCelluleCadreSelection[aLigne][aNumeroColonne]
								? this._cache.positionsCelluleCadreSelection[aLigne][
										aNumeroColonne
									]
								: null;
	...
	_selectionner(aParametres) {
		...
		const lSelectionsPrecedente = lParametres.avecEvenementModificationSelection
			? this._getCellulesSelection(true)
			: null;
			...
			for (
				...
				if (lEstSelectionParCellule) {
					for (
						...
						if (
							this._etatSelectionCellule({ ligne: lLigne, colonne: lColonne })
						) {
				...
				this.Donnees.Donnees.parcourir((aElementLigne, aLigne) => {
					if (aElementLigne) {
						...
						aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
							...
							if (
								lParametres.ligne === aLigne &&
								(lNumeroColonne === -1 || lNumeroColonne === aNumeroColonne)
							) {
								this._etatSelectionCellule(
								...
								!lEstSelectionParCellule &&
								(!lDonneesTableau ||
									lDonneesTableau.options.avecMultiSelectionSurLigneFusion) &&
								lParametres.ligne !== aLigne &&
								lParametres.colonne >= 0
							) {
								let lAvecModif = false;
	...
	_surLigneCreationUnique(aSurClick, aLigne, aSurToucheEdition) {
		const lParams = this._getParamsCellule(-1, aLigne, { surCreation: true }),
	...
	_surCreation(aValeur) {
		...
		if (
			lValeur !== null &&
			lValeur !== undefined &&
			(lValeur !== "" || this.Donnees.autoriserChaineVideSurEdition(lParams))
		) {
				...
				((lValeur !== null && lValeur !== undefined) ||
					this.Donnees.getTypeValeur(aParams) ===
						ObjetDonneesListe_1.ObjetDonneesListe.ETypeCellule
							.DateCalendrier) &&
				(lValeur !== "" || this.Donnees.autoriserChaineVideSurEdition(aParams))
			) {
		...
		this._finaliserEdition(
			Object.assign(aParams, {
				ignorerActualisation:
					!this._cache.editionEnCours && !this._cache.editionEnCoursEvenement,
			}),
		);
	}
	_surSuppression() {
	...
	surSuppressionApresConfirmation(aListeSelections, aParamsCellule, AAccepte) {
		...
		if (AAccepte === Enumere_Action_1.EGenreAction.Valider) {
			...
			if (aListeSelections) {
				aParamsCellule.listeSuppressions = this.Donnees.getListeSupprimables(
					aListeSelections,
					aParamsCellule,
				);
			}
		...
		lFuncs.forEach((aParam) => {
			if (aParam.hashColonnesSelectionnees) {
				aParam.colonnes.forEach((aColonne) => {
					_gererSelection(
						lParams,
						aParam.elementTrouve && aParam.hashColonnesSelectionnees[aColonne],
						aParam.numeroLigne,
						aColonne,
	...
	_selectionnerCelluleSuivante(aParametres) {
		...
		if (
			...
			(lParametres.entrerEdition === true ||
				(MethodesObjet_1.MethodesObjet.isFunction(lParametres.entrerEdition) &&
					lParametres.entrerEdition(
						this._getParamsCellule(
							this._cache.selectionCellule.colonne,
							this._cache.selectionCellule.ligne,
	...
	_estColonneVisible(aNumeroColonne, aCache) {
		return (
			...
			(aCache || this._cache).tableauColonnesCachees[aNumeroColonne] !== true
		);
	}
	...
	_deploiementSurCellule(aParams) {
		...
		return (
			...
			lTypeValeur !== ObjetDonneesListe_1.ObjetDonneesListe.ETypeCellule.Coche
		);
	}
							...
							aTab.push(
								IE.jsx.str("ie-checkbox", {
									...
									style: this._options.paddingContenu_LR
										? "padding-left:" + this._options.paddingContenu_LR + "px;"
										: false,
								}),
							);
						}
		...
		switch (this._options.avecCBToutCocher) {
			...
			case ObjetListe.typeLibelleCocheCBEntete.toutCocher: {
				return $(aNode).hasClass("is-checked")
					? ObjetTraduction_1.GTraductions.getValeur("liste.toutDecocher")
	...
	_annulerRechercheTexte() {
		...
		ObjetHtml_1.GHtml.setFocus(
			lCacheBouton && lCacheBouton.id ? lCacheBouton.id : this.IdPremierElement,
		);
			...
			if (lRangeConstruit.node_start && lRangeConstruit.node_end) {
				lStructure.height =
					ObjetPosition_1.GPosition.getClientRect(lRangeConstruit.node_end)
						.top -
					ObjetPosition_1.GPosition.getClientRect(lRangeConstruit.node_start)
						.bottom;
	...
	_refreshContenuDynamique(aParams) {
		...
		if (!lParams.surInitialisation) {
			if (lParams.avecBackupScroll) {
				lBackupScroll = this._backupScroll({ conserverPositionScroll: true });
			}
		...
		if (lLigneAConstruire) {
			...
			let lJCellule =
				aParams.colonne >= 0
					? $(
							"#" +
								this.getIdCellule(aParams.colonne, aParams.ligne).escapeJQ(),
						)
					: null;
			...
			lTopCelluleDansContenu =
				lRectCellule.top - lRectContenuScroll.top - (aParams.ecartForce || 0);
		}
	...
	avecFiltresVisibles() {
		return (
			!!this._options.avecFiltresVisibles &&
			this.estDonneeListeFlatDesign(this.Donnees) &&
			(this.Donnees.getNbrLignes() > 0 || !this.Donnees.estFiltresParDefaut())
		);
	}
	_construireFiltres() {
		if (
	...
	_surlignageRechercheTexte() {
		...
		const lRangeLignes = { debut: 0, fin: this._cache.lignesVisibles.length };
	...
	getAriaLabelListe(aAvecDefaut = true) {
		...
		if (this._options.ariaLabel) {
			lLabel = MethodesObjet_1.MethodesObjet.isFunction(this._options.ariaLabel)
			...
			$(aNode).on({
				"keyup.listetoolbar"(aEvent) {
					if (
						...
						const lNextNode = ObjetWAI_1.GObjetWAI.getNodeRechercheNavigation(
							lTabElements,
							this,
							lNav,
						);
	...
	jsxIfBoutonListe(aBouton, aNode) {
		return aBouton.getVisible({ bouton: aBouton, node: aNode, liste: this });
	}
					...
					keyup: function (aEvent) {
						if (
							...
							if (aEvent.which === ToucheClavier_1.ToucheClavier.Echap) {
								...
								const lFuncSuite = () => {
									if (
										aInstance.Donnees.options.avecCelluleSuivanteSurFinEdition
									) {
		...
		if (aLigne < 0) {
			...
			lLabel =
				...
				ObjetDonneesListe_1.ObjetDonneesListe.EGenreCoche.Verte
					? ObjetTraduction_1.GTraductions.getValeur("liste.wai.DecocherLigne")
					: ObjetTraduction_1.GTraductions.getValeur("liste.wai.CocherLigne");
		}
	...
	getAttrsWAICellule(aParamsCellule, aParams) {
		...
		const lAttrs = {
			...
			"aria-expanded": aParamsCellule._estDeploiement
				...
				: false,
			"aria-describedby":
				lAriaDescribedByTab.length > 0 ? lAriaDescribedByTab.join(" ") : false,
									...
									(aNumeroColonne) => {
										...
										aTabTri.push(
											IE.jsx.str("div", {
												...
												"aria-owns": this._cache.colonnesTri[aNumeroColonne]
													? this.getIdBtnTri(
