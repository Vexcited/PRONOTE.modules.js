...
class ObjetGrilleGabarit {
	constructor(aOptions) {
		...
		this.options = {
			...
			styleTrait: "solid",
			couleurTrait: "red",
			couleurBordure: (0, AccessApp_1.getApp)().getCouleur().blanc,
	...
	_construire() {
		...
		const lSurNodeCentre = (aNode) => {
			$(aNode).on({
				pointerdown: function (aEvent) {
					const lPosition = lThis.options.grille.getPositionGrilleEvent(aEvent),
						lPlaceOrigine = lThis.options.grille.getPlaceDePosition(lPosition);
					lThis._ajouterEventsGabarit({
				...
				contextmenu: function (event) {
					if (lThis.options.callbackMenuContextuel) {
						lThis.options.callbackMenuContextuel(lThis._getParamsCallback());
					}
				},
				dblclick: function () {
		...
		const lTailleTrait = this._getTailleTrait();
		const lZIndex = this.options.zIndex,
			...
			lClassResize = lGrilleInverse
				...
				: "AvecResizeVertical",
			lTailleOreilles = 10;
		const lFuncTooltip = MethodesObjet_1.MethodesObjet.isFunction(
			this.options.getHintHtml,
		)
			? () => {
					return { "data-tooltip-html": this.options.getHintHtml(this) };
				}
			: false;
		const lHint =
			!lFuncTooltip &&
			MethodesObjet_1.MethodesObjet.isFunction(this.options.getHint)
		...
		const lHtml = IE.jsx.str(
			"div",
			{
				...
				"data-tooltip":
					lHint || lFuncTooltip ? Tooltip_1.Tooltip.Type.default : false,
				"data-tooltip-text": lHint || false,
				"ie-attr": lFuncTooltip,
			},
			() => {
				let lHtmlDeplacement = "";
				if (this.options.avecDeplacement) {
					lHtmlDeplacement = IE.jsx.str(
						"div",
						{
							class: this.options.class || "",
							style: "position:absolute;top:0; left:0; right:0; bottom:0;",
							"ie-node": lSurNodeCentre,
						},
						IE.jsx.str(
							"table",
							{
								class: "Table",
								role: "presentation",
								style: "font-size:1px;" + ObjetStyle_1.GStyle.composeOpacite(0),
							},
							IE.jsx.str("tr", null, IE.jsx.str("td", null)),
						),
					);
				}
				...
				return lHtmlDeplacement;
			},
			ObjetSupport_1.Support.tactile && this.options.avecRetaillageHoraire
				? IE.jsx.str(
						IE.jsx.fragment,
						null,
						IE.jsx.str("div", {
				...
				: "",
			this.options.avecRetaillageHoraire
				? IE.jsx.str("div", {
						class: lClassResize,
						style:
							"position:absolute;z-index:1;" +
		...
		ObjetHtml_1.GHtml.setHtml(this._getIdConteneur(), lHtml, {
			ignorerScroll: true,
		});
	}
}
