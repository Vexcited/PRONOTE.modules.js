...
const GestionnaireBlocSaisieTAF_1 = require("GestionnaireBlocSaisieTAF");
const GestionnaireBlocSaisieContenu_1 = require("GestionnaireBlocSaisieContenu");
const GestionnaireBlocSaisieEltPgm_1 = require("GestionnaireBlocSaisieEltPgm");
const TypeOptionPublicationCDT_1 = require("TypeOptionPublicationCDT");
const ObjetPanelEditionContenu_1 = require("ObjetPanelEditionContenu");
const ObjetPanelEditionTAF_1 = require("ObjetPanelEditionTAF");
...
class ObjetVisuCdT extends ObjetIdentite_Mobile_1.ObjetIdentite_Mobile {
	constructor(...aParams) {
		super(...aParams);
		this.ids = {
			page: GUID_1.GUID.getId(),
			zoneTAFSeance: GUID_1.GUID.getId(),
			zoneContenuSeance: GUID_1.GUID.getId(),
			zoneEltPgmSeance: GUID_1.GUID.getId(),
			zoneTAFProchaineSeance: GUID_1.GUID.getId(),
			boutonPlus: GUID_1.GUID.getId(),
			zonePublicationCdT: GUID_1.GUID.getId(),
			imgNonPub: GUID_1.GUID.getId(),
			zonePub: GUID_1.GUID.getId(),
			btnCmd: GUID_1.GUID.getId(),
		};
		this.data = null;
		this.dimensions = { largeurMenuCtx: 250 };
		this.applicationSco = (0, AccessApp_1.getApp)();
		this.interfaceMobile = this.applicationSco.getInterfaceMobile();
		this.genreTabSelectionne = ObjetVisuCdT.genreTab.tafSeance;
		this.moteurCDT = new ObjetMoteurCahierDeTextes_1.ObjetMoteurCDT();
		this._instancierGestionnaires();
		this._instancierPanelsEdition();
		const lThis = this;
		this.identDatePublication = ObjetIdentite_1.Identite.creerInstance(
			ObjetCelluleDate_1.ObjetCelluleDate,
			{
				pere: this,
				evenement: function (aDate) {
					const lChangement = !ObjetDate_1.GDate.estDateEgale(
						lThis.datePublication,
						aDate,
					);
					if (lChangement) {
						lThis.CdT.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
						lThis.CdT.datePublication = aDate;
						lThis.callback.appel({
							evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.publierDate,
							data: lThis.CdT,
							ctx: {},
						});
					}
	...
	getControleur(aInstance) {
		return $.extend(true, super.getControleur(aInstance), {
			btnOnglet: {
				event(aGenre) {
					aInstance.genreTabSelectionne = aGenre;
					const lId = aInstance._getIdDeGenre(aGenre);
			...
			publierCdt: {
				getValue() {
					return aInstance._estCdTPublie();
				},
				setValue: function (aValue) {
					if (aInstance._estPublicationCdTEditable()) {
						aInstance.CdT.publie = aValue;
						aInstance.CdT.datePublication = aValue
							? aInstance.CdT.datePublication
								? aInstance.CdT.datePublication
								: ObjetDate_1.GDate.getDateCourante()
							: null;
						aInstance.CdT.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
						aInstance._actualiserImgPublication();
						aInstance.callback.appel({
							evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.publierCdT,
							data: aInstance.CdT,
							ctx: {},
						});
					}
				...
				getDisabled: function () {
					return !aInstance._estPublicationCdTEditable();
				},
			},
			verrouCdt: {
				getValue: function () {
					return aInstance._estCdTVerrouille();
				},
				setValue: function () {},
				getDisabled: function () {
					return true;
			...
			getNodePage: function () {
				if (
					!aInstance._estCdTVerrouille() ||
					aInstance.applicationSco.parametresUtilisateur.get(
						"CDT.Commentaire.ActiverSaisie",
					)
				) {
					$("#" + aInstance.interfaceMobile.idZonePrincipale).ieHtmlAppend(
						IE.jsx.str("div", {
							id: aInstance.ids.boutonPlus,
							class: [
								"is-sticky btn-float primary messagerieNouveauMessage",
								IEHtml_Ripple_css_1.StylesIEHtmlRipple.ieRipple,
								IEHtml_Ripple_css_1.StylesIEHtmlRipple.ieRippleClaire,
							],
							"ie-event": "click->btnPlus.surCreer()",
							"ie-display": "btnPlus.estVisible",
						}),
			...
			btnPlus: {
				surCreer: function () {
					if (
							...
							initCommandes: function (aInstanceMenu) {
								aInstanceMenu.add(
									ObjetTraduction_1.GTraductions.getValeur(
										"CahierDeTexte.SaisirContenu",
									),
									!aInstance._estCdTVerrouille(),
									() => {
										aInstance._ouvrirCreationSelonTab(
											ObjetVisuCdT.genreTab.contenuSeance,
										);
									},
								...
								aInstanceMenu.add(
									...
									true,
									() => {
										aInstance._ouvrirCreationNoteProchaineSeance();
									},
									{ icon: "icon_comment" },
								);
							},
						...
						aInstance._ouvrirCreationSelonTab(aInstance.genreTabSelectionne);
					}
				},
				estVisible: function () {
					return (
						aInstance._estTabAvecCreation(aInstance.genreTabSelectionne) &&
						!aInstance._estCdTVerrouille()
					);
				},
			},
			btnCommandes: {
				event: function () {
					ObjetMenuContextuel_1.ObjetMenuContextuel.afficher({
						...
						id: {
							x:
								ObjetPosition_1.GPosition.getLeft(aInstance.ids.btnCmd) -
								aInstance.dimensions.largeurMenuCtx -
								5,
							y: ObjetPosition_1.GPosition.getTop(aInstance.ids.btnCmd),
						},
						evenement: function () {},
						initCommandes: function (aInstanceMenu) {
							aInstance.setOptions({
								largeurMin: aInstance.dimensions.largeurMenuCtx,
							});
							aInstanceMenu.add(
								ObjetTraduction_1.GTraductions.getValeur(
									"CahierDeTexte.CopierCDT",
								),
								!!(aInstance.cours.utilisable && aInstance.cours.AvecCdT),
								() => {
									aInstance.cahierDeTextesCopie = aInstance.CdT;
									aInstance.callback.appel({
										evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.copierCdT,
										data: aInstance.CdT,
										ctx: {},
									});
								},
							...
							aInstanceMenu.add(
								ObjetTraduction_1.GTraductions.getValeur(
									"CahierDeTexte.CollerCDT",
								),
								!(
									!aInstance.cahierDeTextesCopie ||
									!aInstance.cours.utilisable ||
									(aInstance.CdT && aInstance.CdT.verrouille)
								),
								() => {
									if (
										aInstance.cahierDeTextesCopie !== null &&
										aInstance.cahierDeTextesCopie !== undefined &&
										aInstance.CdT !== null &&
										aInstance.CdT !== undefined &&
										aInstance.cahierDeTextesCopie.getNumero() !==
											aInstance.CdT.getNumero()
									) {
										if (
											aInstance.cours &&
											aInstance.cours.utilisable &&
											aInstance.cours.AvecCdT
										) {
											aInstance.applicationSco.getMessage().afficher({
												...
												callback: function (aGenreAction) {
													if (
														...
														aInstance.callback.appel({
															evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT
																.collerCdT,
															data: aInstance.CdT,
															ctx: {},
														});
							...
							aInstanceMenu.add(
								ObjetTraduction_1.GTraductions.getValeur(
									"CahierDeTexte.SupprimerCDT",
								),
								!!(
									aInstance.cours.AvecCdT &&
									!aInstance.CdT.verrouille &&
									aInstance.cours.utilisable
								),
								() => {
									aInstance.applicationSco.getMessage().afficher({
							...
							if (
								...
								aInstanceMenu.add(
									...
									!!aInstance.cours.utilisable,
									() => {
										aInstance._ouvrirCreationCommentairePrive();
									},
	...
	actualiserDataCdT(aParam) {
		...
		this._actualiserPublication();
		switch (aParam.evnt) {
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.createContenu:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.editContenu:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.deleteContenu:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.editNoteProchaineSeance:
				this._actualiserContenus();
				break;
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.createTAF:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.editTAF:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.deleteTAF:
				this._actualiserTafs();
				break;
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.deleteCdT:
			case ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.collerCdT:
				this._actualiserContenus();
				this._actualiserTafs();
				break;
		}
	...
	setDonnees(aParam) {
		...
		this._actualiserOptionsGestionnaires();
		this.afficher();
		this._refreshBlocs();
	...
	construireAffichage() {
		...
		if (this.data !== null && this.data !== undefined) {
			...
			H.push(this._construireZoneCdT());
			H.push(this._construireTabsScrollable({ onglets: lTabOnglets }));
			H.push(
				'<section class="content content--active" id="',
				this.ids.zoneTAFSeance,
				'">',
				this._construireTAFSeance(),
				"</section>",
			);
			H.push(
			...
			H.push(
				...
				'">',
				this._construireTAFProchaineSeance(),
				"</section>",
			);
			H.push("</div>");
		}
	...
	actualiserFicheTAFDuCours(aParam) {
		this.tafsSeance = aParam.tafs;
		this._actualiserTafsSeance();
	}
	actualiserDatePublication() {
		if (
			this.identDatePublication &&
			this._estPublicationVisible() &&
			!this._estPublieAuto()
		) {
			...
			if (this.CdT) {
				const lDate = this.CdT.datePublication
					...
					: ObjetDate_1.GDate.getDateCourante();
				this.identDatePublication.setActif(this._estCdTPublie());
				this.identDatePublication.setOptionsObjetCelluleDate({
					domaineValide: this.applicationSco.droits.get(
	...
	async _ouvrirCreationSelonTab(aGenre) {
		switch (aGenre) {
			case ObjetVisuCdT.genreTab.contenuSeance:
				this._ouvrirEditionContenu({
					estCreation: true,
			...
			case ObjetVisuCdT.genreTab.eltPgmSeance:
				break;
			case ObjetVisuCdT.genreTab.tafProchaineSeance: {
				const lAvecAutorisationCreationTaf =
					await UtilitaireSaisieCDT_1.UtilitaireSaisieCDT.autoriserCreationTAF(
						this.data,
					);
				if (lAvecAutorisationCreationTaf) {
					const lTaf = this.moteurCDT.creerTAFParDefaut({
					...
					if (
						...
						const lListeTousEleves = MethodesObjet_1.MethodesObjet.dupliquer(
							this.moteurCDT.getListeTousEleves({
								listeClassesEleves: this.data.listeClassesEleves,
							}),
						);
						const lListeElevesDetachesEtEnCours =
							UtilitaireSaisieCDT_1.UtilitaireSaisieCDT.getListeElevesDetachesEtEnCours(
								lListeTousEleves,
							);
						const lEstPourTous =
						...
						lTaf.estPourTous = lEstPourTous;
						lTaf.pourTousLesEleves = lEstPourTous;
						lListeElevesDetachesEtEnCours.listeEnCours.parcourir((D) => {
							D.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
							lTaf.listeEleves.addElement(D);
						});
					}
					this._ouvrirEditionTAF({ estCreation: true, taf: lTaf });
				}
				break;
			}
		}
	}
	_actualiserPublication() {
		ObjetHtml_1.GHtml.setHtml(
			this.ids.zonePublicationCdT,
			this._construirePublication(
				this._estCdTPublie(),
				this._estPublicationCdTEditable(),
			),
			{ controleur: this.controleur },
		);
		this.actualiserDatePublication();
	...
	_construireZoneCdT() {
		...
		if (this._estCdTVerrouille()) {
			H.push(this._construireVerrouVisa());
		}
		H.push("</div>");
		H.push(
		...
		H.push(
			...
			this.ids.btnCmd,
			'"><ie-btnicon ie-model="btnCommandes" class="icon_ellipsis_vertical btnCmd" title="',
			ObjetTraduction_1.GTraductions.getValeur("liste.BtnAction"),
			'" aria-haspopup="menu"></ie-btnicon></div>',
		);
		H.push("</section>");
		return H.join("");
	}
	_construireSectionDonnees(aParam) {
	...
	_construireTAFSeance() {
		return this._construireSectionDonnees({
			...
			msgAucun: ObjetTraduction_1.GTraductions.getValeur(
				"CahierDeTexte.AucunTAFPourSeance",
			),
		});
	}
	_construireContenuSeance() {
		const H = [];
		H.push(
		...
		if (
			...
			H.push(this._constuireNoteProchaineSeance());
		}
		return H.join("");
	}
	_construireEltPgmeSeance() {
		if (
	...
	_instancierPanelsEdition() {
		this.instancePanelEditionContenu = ObjetIdentite_1.Identite.creerInstance(
			ObjetPanelEditionContenu_1.ObjetPanelEditionContenu,
			{ pere: this, evenement: this._evntSurEditionContenu.bind(this) },
		);
		this.instancePanelEditionTAF = ObjetIdentite_1.Identite.creerInstance(
			ObjetPanelEditionTAF_1.ObjetPanelEditionTAF,
			{ pere: this, evenement: this._evntSurEditionTAF.bind(this) },
		);
	}
	_ouvrirEditionContenu(aParam) {
		const lParametrePanelContenu = {
		...
		this.instancePanelEditionContenu.setOptionsPanelEditionContenu({
		...
		this.instancePanelEditionContenu.setDonnees(lParametrePanelContenu);
		const lHtmlPanel = this.instancePanelEditionContenu.getHtmlPanel();
		this.interfaceMobile.openPanel(lHtmlPanel, {
	...
	_ouvrirEditionEltPgme(...aParam) {}
	_evntSurEditionContenu(aParam) {
		switch (aParam.commande) {
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.annuler:
				break;
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.valider:
				...
				this.CdT.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
				this.callback.appel({
					evnt: aParam.estCreation
						? ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.createContenu
						: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.editContenu,
					data: this.CdT,
					ctx: { element: aParam.data },
				});
				break;
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.supprimer:
				this.CdT.listeContenus.addElement(
					aParam.data,
					this.CdT.listeContenus.getIndiceParElement(aParam.data),
				);
				this.CdT.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
				this.callback.appel({
					evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.deleteContenu,
					data: this.CdT,
					ctx: {},
				});
				break;
		...
		this.interfaceMobile.closePanel();
	}
	_evntSurEditionTAF(aParam) {
		switch (aParam.commande) {
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.annuler:
				break;
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.valider:
				...
				this.callback.appel({
					evnt: aParam.estCreation
						? ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.createTAF
						: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.editTAF,
					data: this.CdT,
					ctx: { element: aParam.data },
				});
				break;
			case ObjetMoteurFormSaisieMobile_1.EGenreEvntForm.supprimer:
				...
				this.CdT.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
				this.callback.appel({
					evnt: ObjetMoteurCahierDeTextes_1.EGenreEvntCdT.deleteTAF,
					data: this.CdT,
					ctx: {},
				});
				break;
	...
	_ouvrirEditionTAF(aParam) {
		const lParametresPanelTAF = {
			taf: aParam.taf,
			estCreation: aParam.estCreation,
			listeClassesEleves: this.data.listeClassesEleves,
			listeDocumentsJoints: this.data.ListeDocumentsJoints,
			listePeriodes: this.general.ListePeriodes,
			dateCoursDeb: this.data.DateCoursDeb,
			CDTPublie: this._estCdTPublie(),
			matiere: this.data.CahierDeTextes.Matiere,
		};
		this.instancePanelEditionTAF.setOptionsPanelEditionTAF({
			avecSaisiePJ: this.general.PublierDocuments,
			avecSaisieSitesWeb: this.general.PublierUrl,
			avecCloud: this.general.PublierCloud,
			avecKiosque: this.general.PublierKiosque,
		});
		this.instancePanelEditionTAF.setDonnees(lParametresPanelTAF);
	...
	_instancierGestionnaires() {
		this.gestionnaireBlocsTAFsSeance = ObjetIdentite_1.Identite.creerInstance(
			GestionnaireBlocSaisieTAF_1.GestionnaireBlocSaisieTAF,
			{
				pere: this,
				evenement: (aDataBloc) => {
					this._ouvrirSuiviRendusTAFs({ taf: aDataBloc });
				},
			},
		);
		this.gestionnaireBlocsTAFsProchaineSeance =
			ObjetIdentite_1.Identite.creerInstance(
				GestionnaireBlocSaisieTAF_1.GestionnaireBlocSaisieTAF,
				{
					pere: this,
					evenement: (aDataBloc, aGenreEvnt) => {
						switch (aGenreEvnt) {
							case BlocCard_1.EGenreEvntBlocCard.edition:
								this._ouvrirEditionTAF({ estCreation: false, taf: aDataBloc });
								break;
						}
					},
				},
			);
		this.gestionnaireBlocsContenus = ObjetIdentite_1.Identite.creerInstance(
			GestionnaireBlocSaisieContenu_1.GestionnaireBlocSaisieContenu,
			{
				pere: this,
				evenement: (aDataBloc, aGenreEvnt) => {
					switch (aGenreEvnt) {
		...
		this.gestionnaireBlocsEltPgm = ObjetIdentite_1.Identite.creerInstance(
			GestionnaireBlocSaisieEltPgm_1.GestionnaireBlocSaisieEltPgm,
			{
				pere: this,
				evenement: (aDataBloc, aGenreEvnt) => {
					switch (aGenreEvnt) {
						case BlocCard_1.EGenreEvntBlocCard.edition:
							this._ouvrirEditionEltPgme({
								estCreation: false,
								eltPgme: aDataBloc,
							});
							break;
	...
	_actualiserOptionsGestionnaires() {
		...
		this.gestionnaireBlocsTAFsProchaineSeance.setOptions({
			editable: !this._estCdTVerrouille(),
			avecDonneLe: false,
			avecPourLe: true,
	...
	_construirePublication(aCoche, aActif) {
		const H = [];
		if (this._estPublicationVisible()) {
			H.push(
				'<div id="',
				this.ids.imgNonPub,
				'" class="',
				this._getImgPublicationSelonEtat(aCoche, aActif),
				'"></div>',
			);
			H.push(
				'<ie-checkbox id="',
	...
	_estCdTPublie() {
		return (
			this.CdT.existeNumero() && (this.CdT.publie || this._estPublieAuto())
		);
	}
	_estPublicationCdTEditable() {
		return !this._estPublieAuto() && !this._estCdTVerrouille();
	}
	_getImgPublicationSelonEtat(aPublie, aActif) {
		return aActif === true
			? aPublie === true
				? "Image_Publie"
				: "Image_NonPublie"
	...
	_actualiserImgPublication() {
		const lCoche = this._estCdTPublie();
		const lActif = this._estPublicationCdTEditable();
	...
	_construireVerrouVisa() {
		const H = [];
		H.push('<div class="flex-contain flex-center flex-gap">');
		H.push('<i class="icon_lock verrou" role="presentation"></i>');
		H.push(
	...
	_constuireNoteProchaineSeance() {
		const H = [];
		H.push(
			`<div class="m-all-xl" ie-node="editionNoteProchaineSeance">`,
			`<header><h3>${ObjetTraduction_1.GTraductions.getValeur("CahierDeTexte.postIt.seanceSuivante.titre")}</h3></header>`,
			`<p>${ObjetChaine_1.GChaine.replaceRCToHTML(this.CdT.noteProchaineSeance)}</p>`,
			`</div>`,
		);
		return H.join("");
	}
	_ouvrirCreationNoteProchaineSeance() {
		ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
	...
	_ouvrirCreationCommentairePrive() {
		ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
			ObjetFenetre_PostIt_1.ObjetFenetre_PostIt,
			{
				pere: this,
...
exports.ObjetVisuCdT = ObjetVisuCdT;
(function (ObjetVisuCdT) {
	let genreTab;
	(function (genreTab) {
		genreTab[(genreTab["tafSeance"] = 0)] = "tafSeance";
		genreTab[(genreTab["contenuSeance"] = 1)] = "contenuSeance";
		genreTab[(genreTab["eltPgmSeance"] = 2)] = "eltPgmSeance";
		genreTab[(genreTab["tafProchaineSeance"] = 3)] = "tafProchaineSeance";
	})((genreTab = ObjetVisuCdT.genreTab || (ObjetVisuCdT.genreTab = {})));
