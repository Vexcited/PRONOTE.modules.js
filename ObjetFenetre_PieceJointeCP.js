			...
			switch (aParams.colonne) {
				...
				case 1:
					return aParams.surEdition &&
						this.Genre === Enumere_DocumentJoint_1.EGenreDocumentJoint.Fichier
						? ObjetChaine_1.GChaine.extraireNomFichier(aParams.article.Libelle)
						: aParams.article.Libelle;
				case 2: {
					const lValeur = [];
					if (
						...
						if (
							aParams.article.getEtat() !== Enumere_Etat_1.EGenreEtat.Creation
						) {
...
class ObjetFenetre_PieceJointeCP extends ObjetFenetre_1.ObjetFenetre {
	...
	initialiserListe(AInstance) {
		...
		if (this.avecGestionLibelleSurLiens()) {
			lColonnes.push(
				{ taille: 20, titre: { classeCssImage: "Image_CocheVerte as-icon" } },
				{
					taille: "100%",
					titre: {
			...
			if (this.Genre === Enumere_DocumentJoint_1.EGenreDocumentJoint.Fichier) {
	...
	ajouterPiecesJointesAvecAppelCallback(
		...
		if (aListeNouveauxFichiers) {
			...
			if (aGenreDocJoint === Enumere_DocumentJoint_1.EGenreDocumentJoint.Url) {
	...
	setDonnees(aParametres) {
		...
		ObjetHtml_1.GHtml.setDisplay(this.getNomInstanceListeFlat(), lEstModeFlat);
		if (lEstModeFlat) {
	...
	composeContenu() {
		const T = [];
		T.push(
			'<div id="' + this.getNomInstanceListe() + '" class="full-height"></div>',
		);
		T.push(
			'<div id="' +
	...
	composeBas() {
		const T = [];
		T.push(`<div class="compose-bas flex-contain flex-center justify-end">`);
							...
							if (lUrlConforme) {
								...
								if (aParam.estCreation) {
									...
									if (!lEstUnDoublon) {
										...
										lMsg = aParam.donneesListe.getMessageDoublon(lNouvelleUrl);
									}
								} else {
									let lEltEnEdition = aParam.data;
									let lEltTempPourControle = new ObjetElement_1.ObjetElement(
										lDataOut.libelle,
