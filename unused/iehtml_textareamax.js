...
IEHtml_1.default.addBalise("ie-textareamax", (aContexteCourant, aOutils) => {
	...
	const lOptions = {
		...
		couleurTexte: (0, AccessApp_1.getApp)().getCouleur().themeNeutre.sombre,
	...
	const lFLogSpeech = function (aMsg) {
		...
		if (!!lMsg) {
			Toast_1.Toast.afficher({ msg: lMsg, type: Toast_1.ETypeToast.erreur });
		}
	...
	const lEstAutoSize =
		...
		(IE.estMobile && !$(aContexteCourant.node).hasClass("ie-no-autoresize"));
	let H;
	if (lAvecWrapperCompteurMobile) {
		H = IE.jsx.str(
			"div",
			{
				tabindex: "-1",
				class:
					IEHtml_TextareaMax_css_1.StylesIEHtmlTextareaMax.textareamaxWrapper,
			},
			IE.jsx.str("textarea", {
		...
		H = IE.jsx.str(
			...
			lAvecCompteur
				? IE.jsx.str(
						...
						{
							class: [
								IEHtml_TextareaMax_css_1.StylesIEHtmlTextareaMax.tam_counter,
								lSpeechRecognizer
