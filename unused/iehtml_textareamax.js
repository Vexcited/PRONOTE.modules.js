...
IEHtml.addBalise("ie-textareamax", (aContexteCourant, aOutils) => {
	...
	const lFLogSpeech = function (aMsg) {
		...
		switch (aMsg) {
			case "no-speech":
				...
				break;
			case "aborted":
				lMsg = ObjetTraduction_1.GTraductions.getValeur("SaisieVocale.Aborted");
				break;
			case "audio-capture":
				lMsg = ObjetTraduction_1.GTraductions.getValeur(
				...
				break;
			case "network":
				lMsg = ObjetTraduction_1.GTraductions.getValeur("SaisieVocale.Network");
				break;
			case "not-allowed":
				lMsg = ObjetTraduction_1.GTraductions.getValeur(
	...
	if (lSpeechRecognizer && lJSpeechReco.length > 0) {
		(lEstAutoSize ? lJSpeechReco.children("i") : lJSpeechReco).eventValidation(
			() => {
				if (!lFuncEstNonEditable()) {
					if (!lInRecognition) {
						try {
