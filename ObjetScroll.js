...
class ObjetScroll extends ObjetIdentite_1.Identite {
  ...
  setDonnees(...aParams) {
    ...
    if (this.TailleContenu > this.TailleZone) {
      if (this.pas === 0) {
        while (
          this.callback.appel(EGenreScrollEvenement.Deplacement, ++this.pas) ===
            0 &&
      ...
      if (this.Genre === EGenreScroll.Vertical) {
        for (let I = 0; I < this.Zones.length; I++) {
          $("#" + this.getIdZone(this.Zones[I]).escapeJQ())
            ...
            .on("wheel.ObjetScrollV", { aObjet: this }, this.surMouseWheelV);
          if (
            this._estTactile() &&
        ...
        for (let I = 0; I < this.Zones.length; I++) {
          $("#" + this.getIdZone(this.Zones[I]).escapeJQ())
            ...
            .on("wheel.ObjetScrollH", { aObjet: this }, this.surMouseWheelH);
          if (
            this._estTactile() &&
            this.zonesSansScrollTouch.indexOf(this.Zones[I]) === -1
          ) {
    ...
    return IE.jsx.str(
      ...
      IE.jsx.str(
        "div",
        {
          ...
          class: [
            this.Genre === EGenreScroll.Horizontal ? "horizontal" : "vertical",
          ],
          style: LStyleScroll,
          tabindex: "-1",
          role: "presentation",
  ...
  estScrollSurBorne(aSurBorneDebut) {
    ...
    if (lElement) {
      ...
      return lVert
        ? lElement.offsetHeight + lElement.scrollTop >= lElement.scrollHeight
        : lElement.offsetWidth + lElement.scrollLeft >= lElement.scrollWidth;
    }
    return true;
  }
  ...
  scrollToElement(aElement) {
    ...
    if (MethodesObjet_1.MethodesObjet.isNumber(lPositionDestination)) {
      this.scrollTo(
        this._getTailleScrollDeTailleContenu(Math.max(0, lPositionDestination)),
      );
    }
    return true;
  }
