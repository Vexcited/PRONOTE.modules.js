...
class MoteurMessagerie {
	constructor() {
		...
		this.options = {
			...
			avecTiny: UtilitaireMessagerie_1.UtilitaireMessagerie.avecEditeurTiny(),
	...
	addCommandesMenuContextuelDiscussion(aParams) {
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{ genre: TypeCommandeMessagerie_1.TypeCommandeMessagerie.desarchiver },
				lParams,
			),
		);
		this._addCommandeMenuContextuel(
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.modifierCategories,
				},
				lParams,
			),
		);
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.discussionEnFenetre,
				},
				lParams,
			),
		);
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.fermerDiscussion,
				},
				lParams,
			),
		);
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.signalerSuppression,
				},
				lParams,
			),
		);
	...
	addCommandesMenuContextuelVisuMessage(aParams) {
		const lParams = Object.assign(
			{
				...
				messageVisu: null,
				listeMessages: new ObjetListeElements_1.ObjetListeElements().addElement(
					aParams.messageVisu,
				),
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.afficherDiscussion,
				},
				lParams,
			),
		);
		...
		this._addCommandeMenuContextuel(
			Object.assign(
				{
					genre:
						TypeCommandeMessagerie_1.TypeCommandeMessagerie.copierContenuVisu,
				},
				lParams,
			),
		);
		...
		if (aMessage.estUneDiscussion) {
			if (aMessage.public) {
				...
				lSousTitre = ObjetChaine_1.GChaine.format(
					ObjetTraduction_1.GTraductions.getValeur("Messagerie.NParticipants"),
					[aMessage.nbPublic || 0],
				);
	...
	_addCommandeMenuContextuel(aParams) {
		...
		const lEstBrouillon =
			...
			(aParams.message.estUneDiscussion &&
				(aParams.message.nbBrouillons || 0) >= aParams.message.nombreMessages);
		switch (aParams.genre) {
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.desarchiver: {
				...
				if (
					...
					_add(
						lArchiver
							? ObjetTraduction_1.GTraductions.getValeur("Messagerie.Archiver")
							: ObjetTraduction_1.GTraductions.getValeur(
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.signalerSuppression:
				if (
					!lEnConsult &&
					lDiscussionAvancee &&
					!lEstBrouillon &&
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.restaurer:
				if (!lEnConsult && aParams.message && aParams.message.poubelle) {
					_add(
						...
						() => {
							lSaisie({ commande: "restauration", message: aParams.message });
						},
						{ icon: "icon_undo" },
					);
				}
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.purger:
				if (!lEnConsult && aParams.message && aParams.message.messagePurge) {
					_add(
						ObjetTraduction_1.GTraductions.getValeur("Messagerie.Menu_Purger"),
						() => {
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.nonLu: {
				const lLu =
					aParams.genre === TypeCommandeMessagerie_1.TypeCommandeMessagerie.lu;
				if (
					!lEnConsult &&
					aParams.message &&
					!aParams.message.estNonPossede &&
					...
					_add(
						lLu
							? ObjetTraduction_1.GTraductions.getValeur("Messagerie.Menu.Lu")
							: ObjetTraduction_1.GTraductions.getValeur(
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.ouvrirDiscussion: {
				const lOuvrir =
					aParams.genre ===
					TypeCommandeMessagerie_1.TypeCommandeMessagerie.ouvrirDiscussion;
				if (
			...
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.discussionEnFenetre:
				...
				break;
			case TypeCommandeMessagerie_1.TypeCommandeMessagerie.copierContenuVisu:
				if (
					aParams.messageVisu &&
		...
		if (!lMessageBrouillon && aListeMessagerie) {
			...
			aListeMessagerie.parcourir((aMessage) => {
				if (
					...
					aMessage.listePossessionsMessages.getNumero(0) === lNumeroPossession
				) {
	...
	async _getCommandePurgerMessagesPromise(aPurgeSelection, aListeMessages) {
		const lListeMessagesPurge = new ObjetListeElements_1.ObjetListeElements();
						...
						case TypeCommandeAvertissementsMessage.purger:
							lThis
								...
								.then((aCommandePurger) => {
									if (aCommandePurger) {
										return lThis.requeteSaisieMessage(aCommandePurger, true);
									}
								});
							break;
		...
		this.applicationSco
			...
			.then((aAccepte) => {
				...
				const lListePossessionsMessages =
					new ObjetListeElements_1.ObjetListeElements();
				this.donneesListeMessagerie.listeMessagerie.parcourir((aMessage) => {
					if (
						aMessage &&
						...
						lListePossessionsMessages.add(aMessage.listePossessionsMessages);
					}
				});
		...
		if (
			...
			const lNbResponsables = aMessage.listeDestinataires
				.getListeElements((D_1) => {
					return (
						D_1.getGenre() === Enumere_Ressource_1.EGenreRessource.Responsable
					);
				})
				.count();
	...
	_traiterListeEtiquette(aListeEtiquettes, aAvecCategories) {
		...
		if (lAvecCategories) {
			lCumulEtiquettesPerso = new ObjetElement_1.ObjetElement(
				ObjetTraduction_1.GTraductions.getValeur("Messagerie.Titre.Categories"),
				lGetNumeroBidon(),
			);
...
(function (TypeCommandeAvertissementsMessage) {
	...
	TypeCommandeAvertissementsMessage["corbeilleInactif"] = "corbeilleInactif";
})(
	TypeCommandeAvertissementsMessage || (TypeCommandeAvertissementsMessage = {}),
);
