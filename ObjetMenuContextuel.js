...
class ObjetMenuContextuel extends ObjetIdentite_1.Identite {
	constructor(...aParams) {
		...
		this.options = {
			...
			couleurBordureDroiteImage: (0, AccessApp_1.getApp)().getCouleur().bordure,
			heightLigne: 16,
	...
	surValidation(aPositionLigne) {
		...
		if (this.ligne.getGenre() !== EGenreLigneMenu.Deploiement) {
			...
			if (
				this.ligne &&
				MethodesObjet_1.MethodesObjet.isFunction(this.ligne.callbackValidation)
			) {
			...
			"li",
			{
				...
				"ie-model":
					aLigne.estSelecFile && lActif
						? this.jsxModeleSelecFile.bind(this, aLigne, aPositionLigne, lActif)
						: false,
