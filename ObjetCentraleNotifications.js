exports.ObjetCentraleNotifications = exports.TradObjetCentraleNotifications =
	void 0;
...
const TradObjetCentraleNotifications =
	ObjetTraduction_2.TraductionsModule.getModule("ObjetCentraleNotifications", {
		AucuneNotification: "",
		Notification_S: "",
		Notifications_S: "",
		TitreNotifications: "",
		AucuneNouvelleNotification: "",
		HistoSectionTitre: "",
		HistoNotifLueLe: "",
		HistoNotifSuppr: "",
		MarquerLu: "",
...
exports.TradObjetCentraleNotifications = TradObjetCentraleNotifications;
class ObjetRequeteCentraleNotifications extends ObjetRequeteJSON_1.ObjetRequeteConsultation {}
CollectionRequetes_1.Requetes.inscrire(
	"CentraleNotifications",
	ObjetRequeteCentraleNotifications,
);
class ObjetCentraleNotifications extends ObjetIdentite_1.Identite {
	...
	recupererDonnees() {
		if (this.options.simulerFenetre && !this.elementFocusPrec) {
	...
	static getMessagNbNotifications(aNbs) {
		return aNbs === 0
			? TradObjetCentraleNotifications.AucuneNotification
			: aNbs > 1
				? TradObjetCentraleNotifications.Notifications_S.format([aNbs])
				: TradObjetCentraleNotifications.Notification_S.format([aNbs]);
	}
	construireAffichage() {
		this.idsDescrTitre = [];
		const lGetNodeRacine = (aNode) => {
			$(aNode).on({
				keyup: (aEvent) => {
		...
		const lavecCompteurGeneral = () => {
			return this.donneesCentraleNotifications.nbNotifs > 0;
		};
		const lGetAttrHeader = () => {
			return {
				"aria-label": ObjetCentraleNotifications.getMessageNotifications(
					this.donneesCentraleNotifications,
				),
				"aria-describedby": this.idsDescrTitre.join(" "),
			};
		};
		const lgetHtmlNbNotifs = () => {
			return this.donneesCentraleNotifications.nbNotifs > 99
				? "99+"
				: this.donneesCentraleNotifications.nbNotifs;
		};
		const lafficherAucuneNotif = () => {
			return this.donneesCentraleNotifications.nbNotifs === 0;
		};
		const lafficherPapillion = () => {
			return this.donneesCentraleNotifications.nbNotifs > 0;
		};
		return IE.jsx.str(
			IE.jsx.fragment,
			null,
			this.options.simulerFenetre
				? IE.jsx.str("span", {
			...
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						...
						TradObjetCentraleNotifications.TitreNotifications,
					),
					IE.jsx.str(
						"div",
						{
							class: "cn_compteurTotal",
							"ie-if": lavecCompteurGeneral,
							"aria-hidden": "true",
						},
						IE.jsx.str(
							"div",
							null,
							IE.jsx.str("label", { "ie-html": lgetHtmlNbNotifs }),
						),
					),
					IE.jsx.str(
						"div",
				...
				IE.jsx.str("i", {
					"ie-if": lafficherPapillion,
					class: "image_centrale_notification",
					role: "presentation",
				}),
				IE.jsx.str(
					"div",
					{ "ie-if": lafficherAucuneNotif, class: "cn_imgAucuneNouvelleNotif" },
					IE.jsx.str("div", null),
					TradObjetCentraleNotifications.AucuneNouvelleNotification,
				),
	...
	_notificationSaisie(aNotification, aParams) {
		...
		const lDonneesCentraleNotif = (0, AccessApp_1.getApp)()
			.donneesCentraleNotifications;
		if (
			!aNotification.modalites.contains(
				TypeCentraleNotifications_1.TypeModaliteAffichage.affHistorique,
			)
		) {
			lDonneesCentraleNotif.simulerSuppressionNotif();
		}
		return new ObjetRequeteCentraleNotifications(this)
			.lancerRequete(lParams)
			.then((aDonnees) => {
				if (aDonnees.ok) {
					lDonneesCentraleNotif.finSimulationSuppressionNotif(false);
			...
			.finally(() => {
				lDonneesCentraleNotif.finSimulationSuppressionNotif(true);
			});
	}
	_requete(aSurDemarrage) {
		...
		const lPromises = [
			new ObjetRequeteCentraleNotifications(this)
				.lancerRequete()
				.then(async (aDonnees) => {
	...
	_construireListeNotifications(aSection, aIndexSection, aAfficherHistorique) {
		...
		aSection.liste.parcourir((aNotification, aIndexNotif) => {
			...
			const lbtnNotifCoin = () => {
				return {
					event: () => {
						if (aNotification.onBtnClick) {
							this._onBtnCoinClick(aNotification);
						}
					},
				};
			};
				...
				case TypeCentraleNotifications_1.TypeActionBoutonNotif.abnLu:
					if (!aAfficherHistorique) {
						lBoutonCoin = IE.jsx.str("ie-btnimage", {
							...
							title: TradObjetCentraleNotifications.MarquerLu,
							"ie-model": lbtnNotifCoin,
			...
			const lgetNodeNotif = (aNode) => {
				$(aNode).eventValidation(() => {
					this._navigation(aNotification);
				});
			};
			const lgetNodeMessage = (aNode) => {
				$(aNode)
					.find("*")
					.each(function () {
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						"div",
						{
							...
							"ie-node": lAvecActionArticle ? lgetNodeNotif : false,
						},
						lAvecActionArticle
							? IE.jsx.str(
									"p",
						...
						IE.jsx.str(
							...
							{
								class: [
									ObjetCentraleNotifications_css_1
										.StylesObjetCentraleNotifications.sc_article_contenu,
								],
							},
							aNotification.titre
								? IE.jsx.str(
										"div",
										{
											class: [
												ObjetCentraleNotifications_css_1
													.StylesObjetCentraleNotifications
													.sc_article_contenu_titre,
												lBoutonCoin
													? ObjetCentraleNotifications_css_1
															.StylesObjetCentraleNotifications
															.sc_article_contenu_titre_avecBtn
													: "",
											],
										},
										aNotification.titre,
		...
		return H.length > 0 ? IE.jsx.str("ul", null, H.join("")) : "";
	}
	getIdSection(aIdent) {
	...
	_actualiser(aSurDemarrage) {
		...
		this.donnees.liste.parcourir((aSection, aIndexSection) => {
			...
			if (lNbLus > 0) {
				lLabelTitre += ` - ${TradObjetCentraleNotifications.Lues_S.format([ObjetCentraleNotifications.getMessagNbNotifications(lNbLus)])}`;
			}
			...
			const lbtnSectionLu = () => {
				return {
					event: () => {
						this._afficherSectionLue(aSection);
					},
				};
			};
			H.push(
						...
						lNbLus > 0
							? IE.jsx.str(
									...
									IE.jsx.str("ie-btnimage", {
										...
										title:
											TradObjetCentraleNotifications.HistoSectionTitre.format([
												aSection.getLibelle(),
											]),
										"ie-model": lbtnSectionLu,
									}),
...
function _contruireEtiquetteArticle(aNotification) {
	switch (aNotification.type) {
		case TypeCentraleNotifications_1.TypeNotification.nCompteur:
			return IE.jsx.str("div", null, aNotification.compteur);
		default: {
			let lJour = "";
			let lEllipsis = false;
			...
			return IE.jsx.str(
				...
				IE.jsx.str(
					"div",
					{ class: "cn_etiquette_jour", "ie-ellipsis": lEllipsis || false },
					" ",
					lJour,
				),
				IE.jsx.str("div", { class: "cn_etiquette_sep" }),
				IE.jsx.str(
					"div",
					null,
...
function _construireDateLue(aNotification) {
	return IE.jsx.str(
		"div",
		{ class: "sc_datePied" },
		IE.jsx.str("i", { class: "icon_eye_open", role: "presentation" }),
		TradObjetCentraleNotifications.HistoNotifLueLe.format([
...
function _construireDateExpiration(aNotification) {
	return IE.jsx.str(
		"div",
		{ class: "sc_datePied" },
		IE.jsx.str("i", { class: "icon_time", role: "presentation" }),
		TradObjetCentraleNotifications.HistoNotifSuppr.format([
