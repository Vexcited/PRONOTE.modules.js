exports.ObjetFenetre_SelectionPublic = exports.TypeGenreCumulSelectionPublic =
	void 0;
...
(function (TypeGenreCumulSelectionPublic) {
	TypeGenreCumulSelectionPublic[(TypeGenreCumulSelectionPublic["sans"] = 0)] =
		"sans";
	TypeGenreCumulSelectionPublic[(TypeGenreCumulSelectionPublic["classe"] = 1)] =
		"classe";
	...
	TypeGenreCumulSelectionPublic[(TypeGenreCumulSelectionPublic["groupe"] = 9)] =
		"groupe";
	TypeGenreCumulSelectionPublic[
...
class ObjetFenetre_SelectionPublic extends ObjetFenetre_SelectionRessource_1.ObjetFenetre_SelectionRessource {
	...
	_majListeSelection() {
		...
		for (let i = 0; i < lNb; i++) {
			...
			let lCle =
				this.getCleHashElementAvecInformationEleveDansResponsable(lRessource);
			if (
			...
			if (lEleveConcerne) {
				if (lListeEleves.has(lEleveConcerne.getNumero())) {
					...
					const lRessourceSelectionnee = ObjetElement_1.ObjetElement.create({
						...
						responsables: new ObjetListeElements_1.ObjetListeElements().add(
				...
				if (!lRessource.classes || lRessource.classes.count() === 0) {
					...
					for (let j = 0, lNbr2 = lRessource.classes.count(); j < lNbr2; j++) {
						...
						let lNiveau = lListeCumulClasses.get(
							lListeCumulClasses.getIndiceParNumeroEtGenre(lClasse.getNumero()),
						);
						if (!lNiveau) {
				...
				if (
					...
					aRessource.listeServicesPeriscolaire.parcourir(
						(aServicePeriscolaire) => {
							...
							let lCumulSP =
								this.listeRessources.getElementParElement(aServicePeriscolaire);
							if (!lCumulSP) {
								lCumulSP =
									MethodesObjet_1.MethodesObjet.dupliquer(aServicePeriscolaire);
			...
			aListeRessources.parcourir((aRessource) => {
				...
				if (lRessource.listeFamilles) {
					const lFamilleTrouvee =
						lRessource.listeFamilles.getElementParElement(aFamilleConcernee);
					if (
						lFamilleTrouvee &&
