exports.ObjetFenetre_Correspondance = void 0;
...
const DonneesListe_Simple_FlatDesign_1 = require("DonneesListe_Simple_FlatDesign");
...
var EnvoiReception;
(function (EnvoiReception) {
	EnvoiReception[(EnvoiReception["envoi"] = 0)] = "envoi";
	EnvoiReception[(EnvoiReception["reception"] = 1)] = "reception";
})(EnvoiReception || (EnvoiReception = {}));
class ObjetFenetre_Correspondance extends ObjetFenetre_1.ObjetFenetre {
	constructor(...aParams) {
		super(...aParams);
		this.avecDocuments = [
	...
	construireInstances() {
		...
		this.identFenetreAuteur = this.addFenetre(
			...
			this._evntFenetreAuteur,
			this._initFenetreAuteur,
		);
	}
	jsxModeleBoutonChoixInterlocuteur() {
		return {
			event: () => {
				this._evenementBtnSelectionInterlocuteur();
			},
			getTitle: () => {
			...
			getDisabled: () => {
				return (
					...
					this.donneesSaisie.listeInterlocuteurs.count() <= 1
				);
			},
		};
	}
	jsxModelInputInterlocuteur() {
		return {
			getValue: () => {
				return !!this.correspondance
					? this.correspondance.interlocuteur || ""
					: "";
			},
			setValue: (aValue) => {
				if (!!this.correspondance) {
					this.correspondance.interlocuteur = aValue;
				}
			},
		};
	}
	jsxModelCommentaire() {
		return {
			getValue: () => {
				return !!this.correspondance
					? this.correspondance.commentaire || ""
					: "";
			},
			setValue: (aValue) => {
				if (!!this.correspondance) {
					this.correspondance.commentaire = aValue;
				}
			},
		};
	}
	...
	redessinerDocumentsFournis(aListe) {
		...
		if (aListe && aListe.count()) {
			lHTML = UtilitaireUrl_1.UtilitaireUrl.construireListeUrls(aListe, {
				separateur: " ",
		...
		ObjetHtml_1.GHtml.setHtml(this.idListeDocuments, lHTML, {
	...
	setDonnees(aCorrespondance, aDonneesSaisieCorrespondance, aListePJEleve) {
		...
		this.correspondance =
			MethodesObjet_1.MethodesObjet.dupliquer(aCorrespondance);
		this.listePJEleve = MethodesObjet_1.MethodesObjet.dupliquer(aListePJEleve);
		$("#" + this.idGroupeEnvoiReception.escapeJQ() + " > input")
			.off("change")
			.on("change", this._evenementRadioDemiJournee.bind(this));
		...
		if (aCorrespondance.getEtat() === 1) {
			this.correspondance.date = ObjetDate_1.GDate.getDateCourante();
		...
		$("#" + this.idGroupeEnvoiReception.escapeJQ())
			.find("input")
			.removeAttr("checked");
		if (this.correspondance.getEtat() !== Enumere_Etat_1.EGenreEtat.Creation) {
			$("#" + this.idGroupeEnvoiReception.escapeJQ())
				.find("input")
				.inputDisabled(true);
			this.getInstance(this.identType).setActif(false);
		}
		if (!this.correspondance.listePJ) {
			this.correspondance.listePJ =
				new ObjetListeElements_1.ObjetListeElements();
		}
		this.redessinerDocumentsFournis(this.correspondance.listePJ);
		let lTypeCorrespondance = (
			(_a = this.correspondance) === null || _a === void 0
				? void 0
				: _a.avecReponseCourrier
		)
			? EnvoiReception.reception
			: EnvoiReception.envoi;
		$("#" + this.idGroupeEnvoiReception.escapeJQ())
	...
	evenementComboType(aParams) {
		if (
			...
			this.correspondance.type = MethodesObjet_1.MethodesObjet.dupliquer(
				aParams.element,
			);
		}
	}
	composeContenu() {
		const lGroupeRadiosBoutons = {
			...
			listeRadios: [
				{
					libelle: ObjetTraduction_1.GTraductions.getValeur(
						"FenetreCorrespondance.Envoi",
					),
					value: EnvoiReception.envoi,
				},
				{
					libelle: ObjetTraduction_1.GTraductions.getValeur(
						"FenetreCorrespondance.Reponse",
					),
					value: EnvoiReception.reception,
				},
			],
			selectedValue: EnvoiReception.envoi,
		};
		return IE.jsx.str(
			"div",
			{
				class:
					ObjetFenetre_Correspondance_css_1.StylesObjetFenetre_Correspondance
						.FenetreCorrespondance,
			},
			IE.jsx.str(
				"div",
				{
				...
				UtilitaireHtml_1.UtilitaireHtml.composeGroupeRadiosBoutons(
					lGroupeRadiosBoutons,
				),
			),
			IE.jsx.str(
				"div",
					...
					"label",
					{
						class:
							ObjetFenetre_Correspondance_css_1
								.StylesObjetFenetre_Correspondance.ZoneLibelleChamp,
					},
					ObjetTraduction_1.GTraductions.getValeur(
				...
				IE.jsx.str(
					"div",
					{
						class:
							ObjetFenetre_Correspondance_css_1
								.StylesObjetFenetre_Correspondance.ZoneChamp,
					},
					IE.jsx.str("div", { id: this.getNomInstance(this.identType) }),
				),
			),
			IE.jsx.str(
				"div",
			...
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					IE.jsx.str("div", { id: this.getNomInstance(this.identAuteur) }),
				),
			),
			IE.jsx.str(
				"div",
				...
				IE.jsx.str(
					"label",
					{
						for: "inputInterlocuteur",
						class:
							ObjetFenetre_Correspondance_css_1
								.StylesObjetFenetre_Correspondance.ZoneLibelleChamp,
					},
					ObjetTraduction_1.GTraductions.getValeur(
						"FenetreCorrespondance.Interlocuteur",
					),
				),
				IE.jsx.str(
					"div",
					...
					IE.jsx.str("input", {
						...
						class: [
							"style-input",
							ObjetFenetre_Correspondance_css_1
								.StylesObjetFenetre_Correspondance.inputInterlocuteur,
						],
						"ie-model": this.jsxModelInputInterlocuteur.bind(this),
					}),
					IE.jsx.str(
						"div",
						{
							class:
								ObjetFenetre_Correspondance_css_1
									.StylesObjetFenetre_Correspondance
									.btnSelectInterlocuteurWrapper,
						},
						IE.jsx.str("ie-btnicon", {
							"ie-model": this.jsxModeleBoutonChoixInterlocuteur.bind(this),
							class: "bt-activable icon_plus_cercle",
						}),
					),
				),
			),
			...
			IE.jsx.str(
				...
				IE.jsx.str("textarea", {
					class:
						ObjetFenetre_Correspondance_css_1.StylesObjetFenetre_Correspondance
							.textareaCommentaire,
					"ie-model": this.jsxModelCommentaire.bind(this),
			...
			IE.jsx.str(
				"div",
				{
					...
					"ie-if": this.jsxIfAvecDocuments.bind(this),
				},
	...
	_evenementRadioDemiJournee(aEvent) {
		var _a;
		if (!!this.correspondance) {
			const lNum =
				(_a = parseInt($(aEvent.target).val())) !== null && _a !== void 0
					? _a
					: EnvoiReception.envoi;
			this.correspondance.avecReponseCourrier =
				lNum === EnvoiReception.reception;
		}
	}
	_initAuteur(aInstance) {
		aInstance.setOptionsObjetCelluleBouton({
			ariaDescribedBy: this.idLabelAuteur,
	...
	_initFenetreAuteur(aInstance) {
		const lParamsListe = {
			optionsListe: {
				...
				arialabel: ObjetTraduction_1.GTraductions.getValeur(
					"FenetreCorrespondance.SelectionnerAuteur",
				),
			},
		};
		const lInstance = this;
		aInstance.verifierActivationBtnValider = (aBouton) => {
	...
	_evntAuteur(aGenreEvent) {
		if (
			(aGenreEvent === Enumere_Event_1.EEvent.SurKeyUp &&
				ObjetNavigateur_1.Navigateur.isToucheSelection()) ||
	...
	_evntFenetreAuteur(aGenreBouton, aSelection) {
		if (aGenreBouton === 1 && this.correspondance) {
			...
			if (this.correspondance.element.respAdmin) {
				this.correspondance.element.respAdmin.setEtat(
	...
	evenementSelecteurDate(aDate) {
		if (!ObjetDate_1.GDate.estJourEgal(this.correspondance.date, aDate)) {
			this.correspondance.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
	...
	_evenementBtnSelectionInterlocuteur() {
		...
		if (!!lListeInterlocuteurs && !!this.correspondance) {
			...
			lFenetreListeInterlocuteurs.setDonnees(
				new DonneesListe_Simple_FlatDesign_1.DonneesListe_Simple_Fd(
...
exports.ObjetFenetre_Correspondance = ObjetFenetre_Correspondance;
