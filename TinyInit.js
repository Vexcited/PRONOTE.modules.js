...
class ObjetTinyInit {
		...
		if (lParam.filePickerOpener && lParam.filePickerTypes) {
			...
			lParam.filePickerReturn = function (aUrl) {
				...
				if (
					...
					MethodesObjet_1.MethodesObjet.isFunction(lEditor.callbackFilePicker)
				) {
	...
	getPlugins(aParam) {
		return this.plugins
			.concat(
				aParam && aParam.plugins && aParam.plugins.length ? aParam.plugins : [],
				...
				aParam && aParam.clientLourd !== true && !IE.estMobile ? ["help"] : [],
			)
			.join(" ");
	}
	getToolbar(aParam) {
	...
	_detruireInstanceTiny(aTinymce, aID) {
		...
		if (lInstanceTiny) {
			if (!lInstanceTiny.initialized) {
				IE.log.addLog(
					"tiny id " + aID + " doit etre detruit, mais n'est pas initialized",
				);
			}
			try {
