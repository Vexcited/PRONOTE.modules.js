...
class DonneesListe_CDTPlanning extends ObjetDonneesListe_1.ObjetDonneesListe {
	...
	getValeur(aParams) {
		...
		switch (aParams.idColonne) {
			...
			case DonneesListe_CDTPlanning.colonnes.categories: {
				...
				const lLibelle = aParams.article.contenu.categorie.getLibelle();
				const lImage =
				...
				if (lImage) {
					return IE.jsx.str(
						"div",
						{ class: "cdtplanning_dl_cdt_contenu_cat" },
						IE.jsx.str("div", null, " ", lLibelle),
						IE.jsx.str(
							"i",
							{ class: lImage, role: "presentation" },
							aParams.article.contenu.categorie.libelleIcone || "",
						),
					);
				}
				return lLibelle;
			...
			case DonneesListe_CDTPlanning.colonnes.eltProg: {
				...
				const H = [];
				H.push(
					'<div class="cdtplanning_dl_cdt_eltProg" ',
			...
			case DonneesListe_CDTPlanning.colonnes.publie: {
				const lClassesIcone = ["icon_info_sondage_publier"];
				if (
					aParams.article.cours.publie ||
					aParams.article.cdt.datePublication
				) {
					lClassesIcone.push("mix-icon_ok", "i-green");
				} else {
					lClassesIcone.push("mix-icon_remove", "i-red");
				}
				return IE.jsx.str("i", {
	...
	getTri(aColonneDeTri, aGenreTri) {
		const lTris = [];
		const lUniquementTitre = this.applicationSco.parametresUtilisateur.get(
		...
		switch (this.getId(aColonneDeTri)) {
			case DonneesListe_CDTPlanning.colonnes.cours:
				...
				break;
			case DonneesListe_CDTPlanning.colonnes.absences:
				lTris.push(ObjetTri_1.ObjetTri.init("cours.nbAbsences", aGenreTri));
				break;
			case DonneesListe_CDTPlanning.colonnes.categories:
				lTris.push(
	...
	_construireContenu(aParams) {
		...
		const H = [];
		H.push("<div ", this._getAttrConteneurHtml(aParams), ">");
			...
			if (
				...
				H.push(
					IE.jsx.str(
						"div",
						{
							class: "cdtplanning_dl_cdt_contenu_pj",
							"ie-node": "getNodeListePJs",
						},
...
function _construireCours(aParams) {
		...
		H.push(
			...
			aParams.article.cours.notePourCetteSeance
				? IE.jsx.str("i", {
						class: "icon_comment",
						role: "img",
						"ie-tooltiplabel": aParams.article.cours.notePourCetteSeance,
					})
				: "",
			lAvecTafAttendus
	...
	H.push(
		...
		aParams.article.cours.annulation
			? [
					'<div class="cdtplanning_dl_cours_annulation color-red-foncee">',
					aParams.article.cours.annulation,
					"</div>",
				].join("")
			: "",
