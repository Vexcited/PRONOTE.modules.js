exports.InterfaceAbsencesGrille = void 0;
const ObjetDroitsPN_1 = require("ObjetDroitsPN");
const TypeFusionTitreListe_1 = require("TypeFusionTitreListe");
const ObjetAbsencesGrille_1 = require("ObjetAbsencesGrille");
const ObjetRequeteAbsencesGrille_1 = require("ObjetRequeteAbsencesGrille");
...
const DonneesListe_AbsencesGrille_1 = require("DonneesListe_AbsencesGrille");
...
class InterfaceAbsencesGrille extends InterfacePage_1.InterfacePage {
	constructor(...aParams) {
		super(...aParams);
		...
		let lInfosAffichage = this.etatUtilSco.getOnglet().absencesGrille;
		if (!lInfosAffichage) {
			lInfosAffichage = this.etatUtilSco.getOnglet().absencesGrille = {
	...
	construireInstances() {
		...
		this.IdentGrille = this.add(
			ObjetAbsencesGrille_1.ObjetAbsencesGrille,
			null,
			this._initialiserGrille,
		);
		this.identListe = this.add(
			ObjetListe_1.ObjetListe,
			this._evenemnentSurListe,
	...
	setParametresGeneraux() {
		...
		const lJsxHtml = () =>
			this.parametres.eleve ? this.parametres.eleve.titre : "";
		this.AddSurZone.push({
			html: IE.jsx.str("div", { "ie-html": lJsxHtml, class: "NoWrap" }),
		});
		...
		this.AddSurZone.push({
			html: UtilitaireBoutonBandeau_1.UtilitaireBoutonBandeau.getHtmlBtnAfficherCoursAnnules(
				() => {
					return {
						event: () => {
							this.etatUtilSco.setAvecCoursAnnule(
								!this.etatUtilSco.getAvecCoursAnnule(),
							);
							this._actualiserGrille();
						},
						getSelection: () => {
							return this.etatUtilSco.getAvecCoursAnnule();
						},
						getTitle: () => {
							return this.etatUtilSco.getAvecCoursAnnule()
				...
				() =>
					UtilitaireBoutonBandeau_1.UtilitaireBoutonBandeau.getClassesMixIconAfficherCoursAnnules(
						this.etatUtilSco.getAvecCoursAnnule(),
					),
			),
		});
		...
		this.AddSurZone.push({
			html: UtilitaireBoutonBandeau_1.UtilitaireBoutonBandeau.getHtmlBtnAfficherMasquerZone(
				this.jsxModeleBoutonAfficherListeAnnees.bind(this),
			),
		});
	}
	...
	jsxModeleBoutonAfficherListeAnnees() {
		return {
			event: () => {
				this.parametres.affichage.listeAnneeVisible =
					!this.parametres.affichage.listeAnneeVisible;
				if (this.parametres.affichage.listeAnneeVisible) {
					this.parametres.eleve.listeAbsences = null;
					this._actualiserAbsences();
				} else {
					this._actualiserGrille();
				}
			},
			...
			getSelection: () => {
				return !!this.parametres.affichage.listeAnneeVisible;
			},
		};
	}
		...
		return IE.jsx.str(
			...
			IE.jsx.str(
				"div",
				{
					...
					class: "interface_affV interface_affV_padding",
					id: this.idPage,
				},
				IE.jsx.str("div", {
				...
				this._composeSelecteurAbsences(),
				IE.jsx.str(
					"div",
					{
					...
					IE.jsx.str("div", {
						id: this.idConteneurListeAnnee,
						style: "display:none;",
					}),
				),
				IE.jsx.str("div", {
					class: "AlignementHaut EspaceHaut",
					id: this.getInstance(this.identListe).getNom(),
					style: { width: "100%", height: 250 },
				}),
			),
			IE.jsx.str("div", { class: "Espace", id: this.idMessage }),
		);
	}
	...
	_creerColonnesListe() {
		const lDomaine = this.etatUtilSco.getDomaineSelectionne(),
			lColonnes = [
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.date,
					titre: [
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.SemaineDuAu",
							[
					...
					taille: ObjetListe_1.ObjetListe.initColonne(100, 150, 450),
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.duree,
				...
				},
				{
					...
					titre: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.titre.Motif",
					),
				...
				},
				{
					...
					titre: [
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.NbDemiJournee",
						),
						{
				...
				},
				{
					...
					titre: [
						...
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.DJCalc",
						),
					],
					hint: ObjetTraduction_1.GTraductions.getValeur(
				...
				},
				{
					...
					titre: [
						TypeFusionTitreListe_1.TypeFusionTitreListe.FusionGauche,
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.DJBulletin",
						),
					],
					hint: ObjetTraduction_1.GTraductions.getValeur(
				...
				},
				{
					...
					titre: [
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.Statuts",
						),
						{
							libelle: ObjetTraduction_1.GTraductions.getValeur(
							...
							title: ObjetTraduction_1.GTraductions.getValeur(
								"grilleAbsence.liste.hint.Justifie",
							),
						},
					],
					taille: 50,
				},
				{
					...
					titre: [
						TypeFusionTitreListe_1.TypeFusionTitreListe.FusionGauche,
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.HorsEtab",
						),
					],
					hint: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.hint.HorsEtab",
					),
					taille: 50,
				},
				{
					...
					titre: [
						TypeFusionTitreListe_1.TypeFusionTitreListe.FusionGauche,
						ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.titre.Sante",
						),
					],
					taille: 50,
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.regle,
					titre: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.titre.RA",
					),
					hint: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.hint.RA",
					),
					taille: 22,
				},
				{
						...
						getLibelleHtml: () =>
							IE.jsx.str("i", {
								...
								"aria-label": ObjetTraduction_1.GTraductions.getValeur(
									"grilleAbsence.liste.hint.Certificat",
								),
							}),
						title: ObjetTraduction_1.GTraductions.getValeur(
							"grilleAbsence.liste.hint.Certificat",
				...
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.matiere,
					titre: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.titre.Matiere",
					),
					taille: ObjetListe_1.ObjetListe.initColonne(100, 150, 250),
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.accompagnateur,
				...
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.commentaire,
					titre: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.titre.Commentaire",
					),
					taille: ObjetListe_1.ObjetListe.initColonne(100, 150, 250),
				},
				{
				...
				},
				{
					id: DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
						.suivi,
					titre: ObjetTraduction_1.GTraductions.getValeur(
						"grilleAbsence.liste.titre.Suivi",
					),
					taille: ObjetListe_1.ObjetListe.initColonne(100, 170, 250),
				},
			];
		return lColonnes;
	}
	_initialiserListe(aListe) {
		aListe.setOptionsListe({
			colonnes: this._creerColonnesListe(),
			ariaLabel:
				GlossaireAbsencesGrille_1.TradGlossaireAbsencesGrille.labelListe,
			scrollHorizontal: true,
		});
	}
	_actualiserListeDomaine() {
		const lListe = this.getInstance(this.identListe);
		let lColoneesCachees = [];
		const lColonnesAbsences = [
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.heuresCours,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.absenceOuverte,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.DJBrutes,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes.DJCalc,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.DJBulletin,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.horsEtab,
			DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes.sante,
		];
		...
		this.parametres._timerListe = setTimeout(() => {
			...
			if (this.parametres.eleve.listeAbsences) {
				lListe.setDonnees(
					new DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille(
						this.parametres.eleve.listeAbsences.liste,
					).setOptions({
						estSurDomaine: true,
						choixAbsence: this.parametres.affichage.choixAbsence,
						saisie: this._saisieAbsence.bind(this),
					}),
				);
	...
	_colonneToGenreSaisie(aColonne) {
		switch (aColonne) {
			case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille.colonnes
				.absenceOuverte:
	...
	_ouvrirFenetreSelectionMotif(aListeMotifs, aEstunRetard, aCallback) {
	...
	_reponseAbsencesGrillePassageInfirmerie(aPassage, aJSONReponse) {
		if (!aJSONReponse.detailsPassage) {
			return;
		}
		const lPassage = MethodesObjet_1.MethodesObjet.dupliquer(aPassage),
			lDetailsPassage = aJSONReponse.detailsPassage,
			lSelf = this;
		lPassage.Accompagnateur =
			lPassage.accompagnateur || new ObjetElement_1.ObjetElement();
		...
		lFenetre.setDonnees(lDetailsPassage.listeEleves, {
			numeroEleve: this.etatUtilSco.Navigation.getRessource(
				Enumere_Ressource_1.EGenreRessource.Eleve,
			).getNumero(),
			...
			avecCommentaireAutorise: aJSONReponse.avecCommentaireAutorise,
		});
	}
	_evenemnentSurListe(aParams) {
		let lListeMotifs;
		switch (aParams.genreEvenement) {
			case Enumere_EvenementListe_1.EGenreEvenementListe.Edition:
				switch (aParams.idColonne) {
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						.colonnes.motif:
						switch (aParams.article.getGenre()) {
							case TypeRessourceAbsence_1.TypeRessourceAbsence.TR_Exclusion:
								this._ouvrirFenetreMotifsExclusion((aListeMotifs) => {
									this._saisieAbsence({
										genreSaisie:
											TypeHttpSaisieAbsencesGrille_1
												.TypeHttpSaisieAbsencesGrille.sag_Motif,
							...
							case TypeRessourceAbsence_1.TypeRessourceAbsence
								.TR_AbsenceInternat:
							case TypeRessourceAbsence_1.TypeRessourceAbsence
								.TR_RetardInternat:
							case TypeRessourceAbsence_1.TypeRessourceAbsence.TR_Retard: {
								...
								lListeMotifs = MethodesObjet_1.MethodesObjet.dupliquer(
									lEstUnRetard
								...
								this._ouvrirFenetreSelectionMotif(
									lListeMotifs,
									lEstUnRetard,
									(aMotif) => {
										this._saisieAbsence({
					...
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						.colonnes.duree:
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						.colonnes.accompagnateur:
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						.colonnes.commentaire:
						new ObjetRequeteAbsencesGrille_1.ObjetRequeteAbsencesGrille(
							this,
							this._reponseAbsencesGrillePassageInfirmerie.bind(
								this,
								aParams.article,
							),
						).lancerRequete({
							...
							domaine: this.etatUtilSco.getDomaineSelectionne(),
					...
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						...
						this._saisieAbsence({
							genreSaisie: this._colonneToGenreSaisie(aParams.idColonne),
					...
					case DonneesListe_AbsencesGrille_1.DonneesListe_AbsencesGrille
						...
						break;
					default: {
						const lGenre = this._colonneToGenreSaisie(aParams.idColonne);
						if (lGenre >= 0) {
							this._saisieAbsence({
								genreSaisie: lGenre,
								article: aParams.article,
	...
	_evenemnentSurListeAnnee(aParametres) {
		switch (aParametres.genreEvenement) {
			...
			case Enumere_EvenementListe_1.EGenreEvenementListe.SelectionDblClick: {
				...
				if (lArticle && lArticle.semaine) {
					...
					if (!lDomaine.egal(this.etatUtilSco.getDomaineSelectionne())) {
						this.parametres.eleve.listeAbsences = null;
	...
	_getListeCacheDAbsence(aGenre) {
		switch (aGenre) {
			case Enumere_Ressource_1.EGenreRessource.Absence:
				return Cache_1.GCache.listeMotifsAbsenceEleve;
			case Enumere_Ressource_1.EGenreRessource.Retard:
				return Cache_1.GCache.listeMotifsRetards;
		}
	}
	_getAccesseurMotifDAbsence(aGenre) {
		switch (aGenre) {
			case Enumere_Ressource_1.EGenreRessource.Absence:
	...
	_getContenuComboMotif(aLibelle, aCouleur) {
		const H = [];
		H.push(
	...
	_selectionnerMotifAbsence(aMotif) {
		this.parametres.affichage[
			this._getAccesseurMotifDAbsence(this.parametres.affichage.choixAbsence)
		] = aMotif;
		this.getInstance(this.IdentGrille).setOptions({
			motifAbsence:
				this.parametres.affichage[
					this._getAccesseurMotifDAbsence(
						this.parametres.affichage.choixAbsence,
					)
				],
		});
	}
	avecComboMotif(aGenreRessource) {
		return (
			this.parametres.affichage.choixAbsence === aGenreRessource &&
			!!this._getListeCacheDAbsence(aGenreRessource)
		);
	}
	jsxGetModelCombo(aLabel, aGenreRessource) {
		...
		return {
			init: function (aInstance) {
				aInstance.setOptionsObjetSaisie({
					...
					longueur: 200,
					labelWAICellule: aLabel,
					getContenuCellule(aElement) {
						return {
							libelleHtml: lInstance._getContenuComboMotif(
								aElement.getLibelle(),
								aElement.couleur,
							),
						};
					},
					getContenuElement(aLigne) {
						return lInstance._getContenuComboMotif(
							aLigne.element.getLibelle(),
							aLigne.element.couleur,
						);
					},
				});
			},
			getDonnees: function (aDonnees) {
				if (!aDonnees) {
					if (!lInstance.avecComboMotif(aGenreRessource)) {
						return null;
					}
					const lListeModele = lInstance._getListeCacheDAbsence(
						lInstance.parametres.affichage.choixAbsence,
					);
					if (!lListeModele) {
						return new ObjetListeElements_1.ObjetListeElements();
					}
					const lListe = MethodesObjet_1.MethodesObjet.dupliquer(lListeModele);
					lListe.setTri([
						ObjetTri_1.ObjetTri.init((D) => {
							return !D.nonConnu;
						}),
					...
					lListe.trier();
					const lAccesseur = lInstance._getAccesseurMotifDAbsence(
						lInstance.parametres.affichage.choixAbsence,
					);
					if (!lInstance.parametres.affichage[lAccesseur]) {
						lInstance.parametres.affichage[lAccesseur] = lListe.get(0);
						const lGrille = lInstance.getInstance(lInstance.IdentGrille);
						lGrille.setOptions({
							motifAbsence: lInstance.parametres.affichage[lAccesseur],
						});
			...
			getIndiceSelection: function (aInstance) {
				return aInstance
					.getListeElements()
					.getIndiceParElement(
						lInstance.parametres.affichage[
							lInstance._getAccesseurMotifDAbsence(
								lInstance.parametres.affichage.choixAbsence,
							)
						],
					);
			...
			event: function (aParametres, aCombo) {
				if (
					...
					lInstance._selectionnerMotifAbsence(aParametres.element);
				}
			},
		};
	}
	_demandeMemos() {
		ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
			...
			{
				pere: this,
				evenement: (aNumeroBouton, aEstValorisation) => {
	...
	_construireGroupe(aLibelle, aContenu) {
		const H = [];
	...
	jsxModelRadioChoixAbsence(aGenre) {
		return {
			getValue: () => {
				return this.parametres.affichage.choixAbsence === aGenre;
			},
			setValue: (aValue) => {
				this.parametres.affichage.choixAbsence = aGenre;
				const lCalendrier = this.getInstance(this.IdentCalendrier),
					lAvecMultiSelection =
						aGenre === Enumere_Ressource_1.EGenreRessource.Absence,
					lDomaine = this.etatUtilSco.getDomaineSelectionne();
			...
			getName: () => {
				return `${this.Nom}_rbChoixAbsence`;
			},
		};
	}
	_composeSelecteurAbsences() {
		const H = [];
		let lContenu;
		...
		lContenu.push(
			IE.jsx.str(
				"div",
				{ class: "InlineBlock GrandEspaceDroit AlignementMilieuVertical" },
				IE.jsx.str(
					"ie-radio",
					{
						"ie-model": this.jsxModelRadioChoixAbsence.bind(
							this,
							Enumere_Ressource_1.EGenreRessource.Absence,
						),
					},
					" ",
					ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.absence"),
				),
			),
		);
		lContenu.push(
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					" ",
					ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.retard"),
				),
			),
		);
		if (
			...
			lContenu.push(
				IE.jsx.str(
					...
					IE.jsx.str(
						...
						" ",
						ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.exclusion"),
					),
				),
			);
		}
		...
		lContenu.push(
			IE.jsx.str(
				...
				IE.jsx.str("ie-combo", {
					"ie-model": this.jsxGetModelCombo.bind(
						this,
						ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.absence"),
						Enumere_Ressource_1.EGenreRessource.Absence,
					),
					"ie-if": () =>
						this.avecComboMotif(Enumere_Ressource_1.EGenreRessource.Absence),
				}),
				IE.jsx.str("ie-combo", {
					"ie-model": this.jsxGetModelCombo.bind(
						this,
					...
					"ie-if": () =>
						this.avecComboMotif(Enumere_Ressource_1.EGenreRessource.Retard),
				}),
			),
		);
		lContenu.push(
			IE.jsx.str(
				"div",
				{ class: "InlineBlock AlignementMilieuVertical" },
				IE.jsx.str("div", { style: "height:25px;" }, "\u00A0"),
			),
		);
		H.push(
			this._construireGroupe(
				GlossaireAbsencesGrille_1.TradGlossaireAbsencesGrille.typeSaisie,
				lContenu.join(""),
			),
		);
		H.push("</div>");
		if (
			...
			const lJsxEvventMemo = () => {
				this._demandeMemos();
			};
			H.push(
				IE.jsx.str(
					"div",
					{ class: "InlineBlock EspaceGauche AlignementHaut" },
					this._construireGroupe(
						ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.titreMemo"),
						IE.jsx.str(
							"div",
							{
								"ie-eventvalidation": lJsxEvventMemo,
								tabindex: "0",
								role: "button",
							...
							IE.jsx.str("i", {
								class:
									"icon_post_it_rempli InlineBlock AlignementMilieuVertical",
							...
							IE.jsx.str(
								...
								{
									class:
										"InlineBlock EspaceGauche AlignementMilieuVertical SouligneSurvol",
									style: ObjetStyle_1.GStyle.composeCouleurTexte(
										GCouleur.themeCouleur.foncee,
									),
								},
								ObjetTraduction_1.GTraductions.getValeur(
									"grilleAbsence.afficherMemos",
								),
							),
						),
					),
		...
		H.push(
			IE.jsx.str(
				"div",
				{
					class: "InlineBlock GrandEspaceGauche AlignementMilieuVertical",
					style: "padding:23px 0 0 15px;",
				},
				IE.jsx.str("div", {
					class: "InlineBlock AlignementMilieuVertical Image_AppelFait",
				}),
				IE.jsx.str(
					"div",
					{ class: "InlineBlock AlignementMilieuVertical EspaceGauche" },
					ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.AppelFait"),
				),
			),
		);
		H.push("</div>");
	...
	_construireListeAnnee() {
		...
		$("#" + this.idConteneurListeAnnee.escapeJQ())
			...
			.hide();
		if (this.parametres.affichage.listeAnneeVisible) {
			this.listeAnnee = ObjetIdentite_1.Identite.creerInstance(
				ObjetListe_1.ObjetListe,
				{ pere: this, evenement: this._evenemnentSurListeAnnee },
			);
			const lJsxGetTitreListeAnnee = () => {
				return ObjetChaine_1.GChaine.enleverEntites(
					ObjetTraduction_1.GTraductions.getValeur("grilleAbsence.SurLAnnee") +
						" " +
						(this.parametres.eleve.listeAbsences
							? this.parametres.eleve.listeAbsences.titre
							: ""),
				);
			};
			const lHtml = IE.jsx.str(
				...
				null,
				IE.jsx.str("div", {
					"ie-texte": lJsxGetTitreListeAnnee,
					style:
						"position:absolute; top:5px; right:0; width: " +
						this.parametres.largeurListeAnnee +
						"px;",
				}),
				IE.jsx.str("div", {
					id: this.listeAnnee.getNom(),
					style:
						"position:absolute; top:22px; bottom:5px; right:0; width:" +
						this.parametres.largeurListeAnnee +
						"px;",
				}),
			);
			...
			this._initialiserListeAnnee(this.listeAnnee);
			this._actualiserListeAnnee();
		}
	}
	...
	_initialiserGrille(AInstance) {
		AInstance.setOptions({
			decorateurAbsences:
				new UtilitaireAbsencesGrille_1.TDecorateurAbsencesGrille(),
	...
	_ouvrirFenetreMotifsExclusion(aCallback, aListeDejaSelectionnee) {
		const lListeMotifsSelection = MethodesObjet_1.MethodesObjet.dupliquer(
		...
		if (aListeDejaSelectionnee) {
			aListeDejaSelectionnee.parcourir((D) => {
				const lElement = lListeMotifsSelection.getElementParNumero(
					D.getNumero(),
				);
				if (lElement) {
	...
	_reponseSaisie(aDonnees, aJSONReponse) {
		const lInstance = this;
		if (aJSONReponse) {
			if (aJSONReponse.donneesApresSaisie) {
				this._reponseAbsencesGrille(aJSONReponse.donneesApresSaisie);
				return;
			}
			if (aJSONReponse.alert) {
				this.appSco.getMessage().afficher({ message: aJSONReponse.alert });
				this._actualiserAbsences();
				return;
			}
			if (aJSONReponse.confirm || aJSONReponse.confirm_continue) {
				this.appSco.getMessage().afficher({
					...
					callback: function (aAccepte) {
						const lAccepte = aAccepte === Enumere_Action_1.EGenreAction.Valider;
						if (aJSONReponse.confirm_continue) {
							lInstance._saisieAbsence(aDonnees, true, lAccepte);
							return;
						}
						if (lAccepte) {
							lInstance._saisieAbsence(aDonnees, true);
						} else {
							lInstance._actualiserAbsences();
						}
					},
				});
				return;
	...
	_saisieAbsence(
		...
		new ObjetRequeteSaisieAbsencesGrille_1.ObjetRequeteSaisieAbsencesGrille(
			this,
			this._reponseSaisie.bind(this, aDonnees),
		)
	...
	_surMenuContextuelMotifAbsence(aMotif) {
		if (!aMotif) {
			return;
		}
		ObjetMenuContextuel_1.ObjetMenuContextuel.afficher({
			pere: this,
			evenement: () => {
				this._selectionnerMotifAbsence(aMotif);
				this.$refreshSelf();
			},
			initCommandes: (aInstance) => {
				aInstance.addCommande(
					...
					aMotif.getNumero() !==
						this.parametres.affichage.motifAbsenceSelectionne.getNumero(),
				);
			},
		});
	}
	_callbackGabaritFin(aGrille, aParam) {
		if (aParam.gabarit.placeDebut < 0) {
			return;
		}
		this._saisieAbsence({
			genreSaisie:
				TypeHttpSaisieAbsencesGrille_1.TypeHttpSaisieAbsencesGrille.sag_Grille,
	...
	_actualiserGrille() {
		...
		if (
			...
			lGabarit = new ObjetGrilleGabarit_1.ObjetGrilleGabarit({
				...
				callbackFinCreation: function (aParam) {
					lThis._callbackGabaritFin(lGrille, aParam);
				},
			});
		}
		lGrille.setOptions({
			...
			callbackSaisieAbsence: function (aDonnees) {
				lThis._saisieAbsence(aDonnees);
			},
			callbackMenuContextuelMotifAbsence:
				this._surMenuContextuelMotifAbsence.bind(this),
			avecPiedTranche:
				...
				[
					...
					Enumere_Ressource_1.EGenreRessource.Absence,
					Enumere_Ressource_1.EGenreRessource.Retard,
				].includes(this.parametres.affichage.choixAbsence),
			largeurReserve: this.parametres.affichage.listeAnneeVisible
				? this.parametres.largeurListeAnnee
				: 0,
			recreations:
				this.parametres.eleve.recreations || this.parametresSco.recreations,
		});
		const lDomaine = this.etatUtilSco.getDomaineSelectionne();
		lGrille.setDonnees({
			numeroSemaine: lDomaine.getPremierePosition(),
			domaine: lDomaine.getNbrValeurs() > 1 ? lDomaine : null,
			avecCoursAnnule: this.etatUtilSco.getAvecCoursAnnule(),
			listeCours: this.parametres.eleve.listeCours,
			gabarit: lGabarit,
			joursStage: this.parametres.eleve.joursStage,
		});
		this._actualiserListeDomaine();
		this._construireListeAnnee();
		this.afficherBandeau(true);
	}
	_reponseAbsencesGrille(aDonnees) {
	...
	_evenementSurCalendrier(
		...
		if (AIsToucheSelection) {
			...
			if (aDomaine.getNbrValeurs() !== 1) {
				this.parametres.eleve.listeCours =
...
exports.InterfaceAbsencesGrille = InterfaceAbsencesGrille;
