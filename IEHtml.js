...
function _getAccesParametres(
	...
	if (lEstFonction) {
		...
		lResult.getAncre = function () {
			return lTabRempli
				? MethodesObjet_1.MethodesObjet.get(aContexteCourant.controleur, lTab)
				: aContexteCourant.controleur;
		};
...
function _addGetterHtml(aContexteCourant, aInfosParametres, aWrapperHtml) {
	...
	const lGetter = function () {
		const lResult = MethodesObjet_1.MethodesObjet.isFunction(aInfosParametres)
			...
			: aInfosParametres.estFuncJSX
				...
				: aInfosParametres.callback([lNodeDestination, aContexteCourant.data]);
...
function _gererInputModele(
	...
	if (lModele) {
		if (lModele) {
			...
			if (
				!_affecterGetterSetterInputModel(lParametres, aContexteCourant, aEstCB)
			) {
				return;
			}
			...
			if (aEstCB) {
				...
				lMatchPattern =
					lPattern && lPattern.match ? lPattern.match(C_RegExp_Match) : null;
				if (lMatchPattern) {
							...
							if (aContexteCourant.estTelephone) {
								...
								if (lValueInput.length < lSaveValues.inputValue.length) {
									lCaret.begin = Math.max(
										0,
										lCaret.begin -
											(lSaveValues.inputValue.length - lNewInputValue.length) +
											1,
									);
...
function _getDataDeNode(aNode) {
	...
	if (
		...
		try {
			lData = ObjetJSON_1.ObjetJSON.parse(lNodeValue, { ignorerEntites: true });
		} catch (e) {
			lData = null;
		}
	}
...
function _controleAccessibilite(aNode, aNodeName, aContexteCourant) {
	...
	if (
		...
		(aNode.hasAttribute("aria-label") || aNode.hasAttribute("aria-labelledby"))
	) {
	...
	ieHtml(aHtml, aParametres) {
		...
		return this.empty().each(function () {
			if (lParametres.sansCompile) {
				...
				IEHtml.injectHTMLParams(Object.assign({ element: this }, lParametres));
			}
		});
	},
});
