...
exports.StylesIEHtmlComposants = { ellipsisCancelTitle: "ellipsisCancelTitle" };
IEHtml.addAttribut(
	"ie-node",
		...
		aOutils.surInjectionHtml(aContexteCourant, () => {
			...
			function _event(aEvent, ...aParams) {
				if (
					...
					(aEvent.isTrigger || !ObjetHtml_1.GHtml.estDocument(aEvent.target)) &&
					...
					if (lResult && lResult.nodesExclus && lResult.callback) {
						if (
							...
							lResult.nodesExclus.every((aNode) => {
								return (
									!aNode ||
									(aNode !== aEvent.target && !$.contains(aNode, aEvent.target))
								);
							})
						) {
							lResult.callback();
...
IEHtml.addAttribut(
	...
	(aContexteCourant, aNodeName, aAttributValue, aOutils, aComp, aAttrName) => {
		...
		const lGetter = function () {
			const lStyle =
				lInfos.callback([aContexteCourant.node, aContexteCourant.data]) || null;
			return lStyle || {};
		};
