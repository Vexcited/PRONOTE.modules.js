...
class ObjetGrilleCoursPN extends ObjetGrilleCours_1.ObjetGrilleCours {
	...
	getAriaLabelCours(aCours) {
		if (aCours && !aCours.estRetenue) {
			return ObjetChaine_1.GChaine.format(
				ObjetTraduction_1.GTraductions.getValeur("EDT.WAI.CoursDuDeA_SSS"),
				[
					ObjetDate_1.GDate.formatDate(aCours.DateDuCours, `%J %MMMM`),
					ObjetDate_1.GDate.formatDate(
						aCours.DateDuCours,
						`%xh ${GlossaireWAI_1.TradGlossaireWAI.heures.toLowerCase()} %mm`,
					),
					ObjetDate_1.GDate.formatDate(
	...
	actualiserContenuCours(aParams) {
		...
		for (const aLigneContenu of lLignesContenus) {
			...
			if (lEstContenuMasque) {
				LClass = Divers_css_1.StylesDivers.srOnly;
			} else {
			...
			H.push('<div class="NoWrap ' + LClass + '" style="' + LStyle + '">');
			...
			if (lDescriptionImage && lDescriptionImage.estTAF) {
				...
				H.push(aLigneContenu.getLibelle());
			}
			H.push("</div>");
		}
		ObjetHtml_1.GHtml.setHtml(
