...
class WidgetPartenaireCDI extends ObjetWidget_1.Widget.ObjetWidget {
	constructor() {
		super(...arguments);
		this.textRecherche = "";
		this.listeEmprunte = new ObjetListeElements_1.ObjetListeElements();
		this.listeHisto = new ObjetListeElements_1.ObjetListeElements();
	}
	...
	initialiserObjetsPartenaireCDI() {
		...
		const lVisible =
			((_a = this.donnees.listeCDI) === null || _a === void 0
				? void 0
				: _a.count()) > 1;
		this.comboCDI.setVisible(lVisible);
		this.ficheHistorique.initialiser();
	}
	surLienExterne(aCDI) {
		if (aCDI.avecDelegationAuth) {
			UtilitairePartenaire_1.TUtilitairePartenaire.ouvrirURLPartenaireCDI(aCDI);
		} else {
			UtilitairePartenaire_1.TUtilitairePartenaire.ouvrirUrl(aCDI.url, true);
		}
	}
	infosURLExterne() {
		let lCDI = this.CDI;
		if (!lCDI && this.donnees.listeCDI && this.donnees.listeCDI.count() > 0) {
			lCDI = this.donnees.listeCDI.get(0);
		}
		if (!!lCDI && !!lCDI.url) {
			return {
				callbackLien: this.surLienExterne.bind(this, lCDI),
				libelle: ObjetTraduction_1.GTraductions.getValeur(
					"accueil.CDI.lienExterne",
				),
				titre: ObjetTraduction_1.GTraductions.getValeur(
					"accueil.CDI.lienExterneInfobulle",
					[lCDI.getLibelle()],
				),
			};
		}
	...
	construire(aParams) {
		...
		let lCDIParDefaut = null;
		if (this.donnees.listeCDI && this.donnees.listeCDI.count() > 0) {
			lCDIParDefaut = this.donnees.listeCDI.get(0);
		}
		const lTitre =
			((_a = this.donnees.listeCDI) === null || _a === void 0
				? void 0
				: _a.count()) === 1
				? lCDIParDefaut.getLibelle()
				: ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.titre");
		const lWidget = {
			getHtml: this.composeWidgetPartenaireCDI.bind(this, lCDIParDefaut),
	...
	actualiserPartenaireCDI(aCDI) {
		...
		const lTitre =
			((_a = this.donnees.listeCDI) === null || _a === void 0
				...
				: ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.titre");
		const lWidget = {
			getHtml: this.composeWidgetPartenaireCDI.bind(this, aCDI),
			nbrElements: null,
			afficherMessage: false,
			titre: lTitre,
			infosURLExterne: this.infosURLExterne.bind(this),
	...
	composeWidgetPartenaireCDI(aCDI) {
		...
		if (aCDI && aCDI.listePrets && aCDI.listePrets.count() > 0) {
			this.listeEmprunte = aCDI.listePrets.getListeElements((aPret) =>
				[
					TypeGenrePret_1.TypeGenrePret.GP_EnCours,
					TypeGenrePret_1.TypeGenrePret.GP_EnRetard,
				].includes(aPret.getGenre()),
			);
			this.listeHisto = aCDI.listePrets.getListeElements((aPret) =>
				[TypeGenrePret_1.TypeGenrePret.GP_Historique].includes(
					aPret.getGenre(),
				),
			);
		} else {
		...
		H.push(
			IE.jsx.str(
				...
				{
					class: [
						Divers_css_1.StylesDivers.mLeftXl,
						Divers_css_1.StylesDivers.mRightXl,
						Divers_css_1.StylesDivers.mBottomXl,
						Divers_css_1.StylesDivers.fieldContain,
						Divers_css_1.StylesDivers.cols,
						Divers_css_1.StylesDivers.flexStart,
					],
				},
				IE.jsx.str(
					"label",
					{
						for: this.idInputRecherche,
						class: [Divers_css_1.StylesDivers.mBottom],
					},
					ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.rechercher"),
				),
				IE.jsx.str(
					"div",
					{
					...
					IE.jsx.str("input", {
						...
						name: this.idInputRecherche,
						"ie-model": this.jsxInputRecherche.bind(this),
						"ie-eventmap": lMap,
					...
					IE.jsx.str("ie-btnicon", {
						"ie-model": this.jsxBoutonRecherche.bind(this),
						class: [
			...
			H.push(
				IE.jsx.str(
					...
					ObjetTraduction_1.GTraductions.getValeur(
						"accueil.CDI.empruntsEnCours",
						this.listeEmprunte.count(),
					),
				),
			);
			H.push(this.composeListePrets(this.listeEmprunte, false));
		} else {
			H.push(
				IE.jsx.str(
					"div",
					{ class: [Divers_css_1.StylesDivers.mAllXl] },
					ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.aucunEmprunt"),
				),
			);
		}
		if (this.listeHisto.count() > 0) {
			H.push(
				IE.jsx.str(
					"div",
					{
						class: [
							Divers_css_1.StylesDivers.mAllL,
							Divers_css_1.StylesDivers.pBottomXl,
							Divers_css_1.StylesDivers.flexContain,
					...
					IE.jsx.str(
						"ie-bouton",
						{
							"ie-model": this.jsxModeleBoutonHistorique.bind(this),
		...
		return {
			...
			getTitle: () => {
				return ObjetTraduction_1.GTraductions.getValeur(
					"accueil.CDI.rechercher",
				);
			},
			getDisabled: () => {
				return this.textRecherche === "";
			},
		};
	}
	jsxInputRecherche() {
	...
	composeListePrets(aListePret, aPourHistorique) {
		const H = [];
		if (aListePret.count() > 0) {
			if (aPourHistorique) {
				H.push('<div class="partenairecdi" style="min-width: 38rem;">');
			}
			H.push("<dl>");
			for (let i = 0; i < aListePret.count(); i++) {
				const lPret = aListePret.get(i);
				const lEnretard =
					lPret.getGenre() === TypeGenrePret_1.TypeGenrePret.GP_EnRetard;
				const lLibelleRendu = aPourHistorique
					? ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.renduLe")
					: ObjetTraduction_1.GTraductions.getValeur("accueil.CDI.aRendreLe");
				const lDateRetour = aPourHistorique
				...
				H.push(
					IE.jsx.str(
						...
						IE.jsx.str(
							...
							(lPret.url &&
								IE.jsx.str(
									"a",
									{
										href: ObjetChaine_1.GChaine.verifierURLHttp(lPret.url),
										target: "_blank",
										class: [
											Widgets_dashboard_css_1.StylesWidgetsDashboard
												.wrapperLink,
											Divers_css_1.StylesDivers.linkIconAfter,
											Divers_css_1.StylesDivers.withUnderline,
										],
										"ie-tooltiplabel": lPret.commentaire,
									},
									lPret.titre,
								)) ||
								IE.jsx.str(
						...
						IE.jsx.str(
							...
							IE.jsx.str(
								"span",
								{ class: [Divers_css_1.StylesDivers.flexContain] },
								IE.jsx.str(
									"span",
									{
										class: [
											Divers_css_1.StylesDivers.fluidBloc,
											Divers_css_1.StylesDivers.mRight,
										],
									},
									lLibelleRendu,
									" : ",
									ObjetDate_1.GDate.formatDate(lDateRetour, "[%JJ/%MM/%AA]"),
								),
								IE.jsx.str("span", {
									style: "width: 1.6rem",
									"ie-tooltiplabel":
									...
									class:
										(lEnretard && [
											Divers_css_1.StylesDivers.fixBloc,
											Widgets_dashboard_css_1.StylesWidgetsDashboard.dateAlert,
										]) ||
										Divers_css_1.StylesDivers.fixBloc,
								}),
							),
	...
	composeListePretsOld(aCDI, aPourHistorique) {
		const H = [];
	...
	_surHistorique() {
		this.ficheHistorique.setOptionsFenetre({
			...
			largeurMin: 300,
			htmlContenu: this.composeListePrets(this.listeHisto, true),
		});
		this.ficheHistorique.afficher();
	}
	_surRechercheCDI() {
		let lValeurRecherchee = this.textRecherche.trim();
