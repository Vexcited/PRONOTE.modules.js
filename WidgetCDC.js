...
class WidgetCDC extends ObjetWidget_1.Widget.ObjetWidget {
	constructor(...aParams) {
		super(...aParams);
		this.nbColonneMax =
			ObjetNavigateur_1.Navigateur.ecranL < 1250 &&
			ObjetNavigateur_1.Navigateur.ecranL >= 1199
				? 3
				: 4;
	...
	construire(aParams) {
		...
		this.donnees.navigations[TypeElementWidgetCDC.Appreciations] = {
			ongletNavigation: this.applicationSco.droits.get(
		...
		this.creerObjetsCDC();
		const lWidget = {
			getHtml: this.composeWidgetCDC.bind(this, this.donnees.periodeParDefaut),
	...
	actualiserWidgetCDC(aPeriode) {
		const lWidget = { getHtml: this.composeWidgetCDC.bind(this, aPeriode) };
		$.extend(true, this.donnees, lWidget);
		...
		switch (aTypeElement) {
			...
			case TypeElementWidgetCDC.Appreciations:
				return (
					...
					this.etatUtilisateurSco.estOngletActif(
	...
	jsxNodeClicItemCDC(aClasse, aType, aNode) {
		$(aNode).eventValidation(() => {
			this._surClicItemCDC(
				aClasse.getNumero(),
				aClasse.periode.getNumero(),
				aType,
			);
		});
	}
	...
	composeWidgetCDC(aPeriode) {
		...
		if (lListeClassesAvecServicesDePeriode.count() > 0) {
			...
			H.push(
				IE.jsx.str(
					"div",
					{ "aria-hidden": "true", class: "legende" },
					lItemsLegende.join(""),
				),
			);
		}
		...
		for (const lTypeElement of this.getArrayListeTypesElement()) {
			if (this.estAvecTypeElement(lTypeElement)) {
				...
				H.push(
					IE.jsx.str(
						"th",
						{ scope: "row", class: "cdc_item_enteteligne" },
						IE.jsx.str(
							"div",
							null,
							IE.jsx.str("i", {
						...
						switch (lTypeElement) {
								...
								H.push(
									IE.jsx.str(
										...
										IE.jsx.str(
											"div",
											{
												class: "item-date-conseil-classe",
												"ie-tooltiplabel": lStrDateConseil
													? ObjetTraduction_1.GTraductions.getValeur(
															"accueil.CDC.hint.ConseilProgrammeLe",
															[lStrDateConseil],
														)
													: false,
												"aria-hidden": lStrDateConseil ? false : "true",
											},
											lStrDateConseil,
										),
									),
							...
							case TypeElementWidgetCDC.Appreciations: {
								...
								if (lClasse.auMoinsUnEleveDsServicesAppr) {
									H.push(
										IE.jsx.str(
											"div",
											{
												class: "item-contain",
												tabindex: "0",
												"ie-node": this.jsxNodeClicItemCDC.bind(
													this,
													lClasse,
													TypeElementWidgetCDC.Appreciations,
												),
									...
									H.push(IE.jsx.str("div", null, lContenuCelluleSansEleveAppr));
								}
								H.push("</td>");
								break;
							}
								...
								H.push(
									IE.jsx.str(
										...
										IE.jsx.str(
											"div",
											{
												class: lClassesCellule,
												tabindex: "0",
												"ie-node": lClasse.UniquementServicesSansNote
													? false
													: this.jsxNodeClicItemCDC.bind(
															this,
															lClasse,
															TypeElementWidgetCDC.Devoirs,
														),
												role: "link",
												"data-tooltip": Tooltip_1.Tooltip.Type.default,
												"data-tooltip-text": lHintCelluleDevoir,
												"aria-disabled": lClasse.UniquementServicesSansNote
													? "true"
													: false,
											},
											lContenuCelluleDevoir.join(""),
											this._composeCadenas(
