exports.ObjetMoteurBlog = exports.EGenreEvntBillet = void 0;
...
class ObjetMoteurBlog {
	...
	creerNouveauBlog() {
		...
		const lNouveauBlog = ObjetElement_1.ObjetElement.create({
			...
			listeBillets: undefined,
			estModifiable: undefined,
			estSupprimable: undefined,
			auteur: undefined,
		});
	...
	creerCommentaireParDefaut() {
		const lResult = ObjetElement_1.ObjetElement.create({
			...
			estSupprimable: true,
			etatCommentaire: undefined,
			estAdministrable: undefined,
			estAModerer: undefined,
		});
		return lResult;
	}
		...
		if (aListeDonnees) {
			aListeDonnees.parcourir((aCategorie) => {
				if (!aCategorie.existeNumero()) {
					aCategorie.setLibelle(GlossaireCP_1.TradGlossaireCP.Aucune);
					aCategorie.Genre = 0;
	...
	creerPhotoIndividuEnVignette(aIndividu) {
		const H = [];
		let lSrcPhoto =
			aIndividu.getGenre() === Enumere_Ressource_1.EGenreRessource.Responsable
				? ""
				: ObjetChaine_1.GChaine.creerUrlBruteLienExterne(aIndividu, {
						libelle: "photo.jpg",
					});
		H.push(
			IE.jsx.str(
				...
				IE.jsx.str("img", {
					...
					class: "img-portrait",
					"ie-imgviewer": !!lSrcPhoto,
					alt: aIndividu.getLibelle(),
					"data-libelle": aIndividu.getLibelle(),
				}),
			),
		);
		return H.join("");
	...
	composeCardBillet(aBilletBlog, aParam) {
					...
					HBoutonsActions.push(
						IE.jsx.str(
							...
							IE.jsx.str("ie-btnicon", {
								...
								title: ObjetTraduction_1.GTraductions.getValeur("blog.Publier"),
							}),
						),
					);
				}
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						...
						this.estBlogDontJeSuisModerateur(aBilletBlog.blog)
							? ObjetTraduction_1.GTraductions.getValeur("blog.BilletAPublier")
		...
		lTabContenuBillet.push(
			IE.jsx.str(
				"div",
				{ class: ["ContenuBillet", Divers_css_1.StylesDivers.tinyView] },
			...
			if (lAvecCarrousel) {
				lTabContenuBillet.push(
					IE.jsx.str(
						IE.jsx.fragment,
						null,
						IE.jsx.str("div", { id: aParam.idGalerieCarrousel }),
					),
				);
			}
				...
				if (D.existe()) {
					lTabZoneDocs.push(
						ObjetChaine_1.GChaine.composerUrlLienExterne({
							...
							maxWidth: ObjetNavigateur_1.Navigateur.ecranL - 40,
						}),
					);
				}
			});
			lTabContenuBillet.push(
				IE.jsx.str("div", { class: "zone-docs" }, lTabZoneDocs.join("")),
			);
		}
		...
		if (lNbRedacteurs === 1) {
			...
			lArrInformationsPhoto.push(
				'<i class="icon_eye_open" role="presentation"></i><span>+</span><span>',
		...
		lTabContenuBillet.push(
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						"ie-bouton",
						{ "ie-model": "btnVoirRedacteurs", class: "themeBoutonNeutre" },
						ObjetTraduction_1.GTraductions.getValeur(
		...
		if (this.estBilletCommentable(aBilletBlog)) {
			const lExisteCommentaires = this.existeCommentairesSurBillet(aBilletBlog);
			const lTabZoneBtnCommentaires = [];
			if (lExisteCommentaires) {
				if (aBilletBlog.listeCommentaires.count() > 0) {
					lTabZoneBtnCommentaires.push(
						IE.jsx.str(
							"ie-bouton",
							{
			...
			lTabContenuBillet.push(
				IE.jsx.str(
					...
					lTabZoneBtnCommentaires.join(""),
				),
			);
			if (lExisteCommentaires) {
				lTabContenuBillet.push('<div class="CommentairePlusRecent">');
