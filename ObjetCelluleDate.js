...
class ObjetCelluleDate extends ObjetIdentite_1.Identite {
	...
	_actualiserDate() {
		if (this.cellule) {
			...
			ObjetHtml_1.GHtml.setHtml(this.idContenuWai, this._getChaineDate(true));
		}
	}
	getLabelledBy() {
	...
	_getChaineDate(aPourWAI) {
		if (!this.date) {
			if (aPourWAI) {
				...
				return MethodesObjet_1.MethodesObjet.isObject(this.options.placeHolder)
		...
		if (this.options.avecSelectionSemaine) {
			...
			return ObjetDate_1.GDate.formatDate(this.date, this.options.formatDate);
		}
	}
}
