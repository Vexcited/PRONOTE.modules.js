...
class UtilitairePagePubliqueEtablissement {
	...
	getControleur(aInstance) {
		const lControleur = {
			...
			getWidthTitle: function () {
				...
				let widthTitre = $(this.node).find("section.identiteEtab").innerWidth();
	...
	construire(aParam, aDonneesRequete) {
		...
		this.options.parametresCommun.setDocumentTitle.call(
			this.parametres,
			ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.titlePage"),
		);
	...
	declarerWidgets() {
		return {
			menuDeLaCantine: {
				...
				classWidget: MultipleWidgetMenu ? MultipleWidgetMenu.WidgetMenu : null,
				...
				message: ObjetTraduction_1.GTraductions.getValeur("accueil.pasDeMenu"),
				existeWidget: () => {
					return (
						!this.parametres ||
			...
			agenda: {
				...
				id: GUID_1.GUID.getId(),
				titre: ObjetTraduction_1.GTraductions.getValeur("accueil.agenda.titre"),
			...
			actualites: {
				...
				id: GUID_1.GUID.getId(),
				titre: ObjetTraduction_1.GTraductions.getValeur("accueil.actualites"),
				hint: "",
				message: ObjetTraduction_1.GTraductions.getValeur(
	...
	recupererDonneesWidget(aTabWidgets) {
		...
		new ObjetRequetePagePubliqueEtablissement_1.ObjetRequetePagePubliqueEtablissement(
			...
			.then((aParam) => {
				...
				if (lActualiserUniquementWidget) {
					if (
						aTabWidgets.includes(Enumere_Widget_1.EGenreWidget.menuDeLaCantine)
					) {
		...
		if (this.parametres.urlLogo) {
			H.push(
				...
				ObjetTraduction_1.GTraductions.getValeur(
					"BandeauEspace.LogoEtablissement",
				),
				'" />',
			);
		}
	...
	_construireEdito() {
		return this._construireSectionWidget({
			...
			titre:
				this.parametres && this.parametres.edito
					? this.parametres.edito.titre
					: ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.edito"),
	...
	_construireLiensFooter() {
		...
		if (this.parametres.avecSiteWeb && this.parametres.SiteInternet !== "") {
			H.push(
				...
				'">',
				ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.siteWeb"),
				"</a></div>",
			);
		}
		if (
			...
			H.push(
				...
				ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.contact"),
				"</div>",
			);
		}
		...
		if (this.parametres.avecTwitter) {
			H.push(
				'<ie-btnimage ie-model="btnReseauxSociaux(' +
					...
					ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.twitter") +
					'">' +
					"</ie-btnimage>",
			);
		}
		...
		if (this.parametres.avecYouTube) {
			H.push(
				'<ie-btnimage ie-model="btnReseauxSociaux(' +
					...
					ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.youtube") +
					'">' +
					"</ie-btnimage>",
			);
		}
	...
	_construireCopyright() {
		...
		H.push(
			...
			this.parametres.urlSiteIndexEducation,
			"');\">",
			ObjetTraduction_1.GTraductions.getValeur("pagePubliqueEtab.copyright", [
