...
class UtilitairePagePubliqueEtablissement {
        ...
        $(this.node).on({
          click: function () {
            return new ObjetRequeteMentionsLegales(aInstance)
              .lancerRequete()
              .then((aParams) => {
                let lFenetreMentionsLegales = ObjetFenetre.creerInstanceFenetre(
                  ObjetFenetre_MentionsLegales,
  ...
  instancierWidgets() {
    ...
    if (_estVisible_Informations.call(this, { estPublieUniquement: true })) {
  ...
  construireWidget(aWidget) {
    ...
    GHtml.setHtml(aWidget.id, UtilitaireWidget.composeContenuWidget(aWidget), {
  ...
  recupererDonneesWidget(aTabWidgets) {
    ...
    Requetes.inscrire("PagePubliqueEtablissement", ObjetRequeteConsultation);
...
function _construireSectionWidget(aParam) {
  ...
  if (aParam.estVisible) {
    ...
    H.push('<div class="accueil-widget-content" id="' + aParam.id + '"><div>');
    H.push("</article>");
  }
  return H.join("");
}
...
function _construireLiensUtilesFavoris() {
  ...
  if (_estVisible_LiensUtilesFavoris.call(this)) {
    ...
    const lListeFavoris = this.parametres.lienUtile.listeLiens.getListeElements(
      (aElement) => {
        return aElement.estFavori === true;
      },
    );
