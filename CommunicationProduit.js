...
class CommunicationProduit {
	...
	composeUrlFichier(aPrefix, aNomFichier, aParam) {
		...
		lResult += this.getChaineChiffreeAES(
			JSON.stringify(aParam && aParam.toJSONAll ? aParam.toJSONAll() : aParam),
		);
		lResult += "/" + aNomFichier;
	...
	_envoieRequete() {
		...
		if (!lRequete.enAttente()) {
			...
			if (
				lRequete &&
				lEstRequeteAjax &&
				lRequete.etat === TypeEtatRequeteAjax_1.TypeEtatRequeteAjax.envoye &&
				!this._attenteEnCours &&
				lRequete.Parametres &&
				lRequete.Parametres.sansBlocageInterface
			) {
	...
	_evenementSurReponseRequete(aParametresAppel, ARequete, AReponse, AStatut) {
		...
		switch (lStatut) {
			...
			default: {
				IE.log.addLog(
					"échec requête ajax. requête = " + ARequete + ". statut = " + AStatut,
				);
