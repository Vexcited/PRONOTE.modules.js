...
class ObjetDate {
	...
	dateEntreLesDates(
		...
		return (
			...
			(!aBorneDateFin ||
				aDate <= aBorneDateFin ||
				(aComparaisonTime !== true && this.estJourEgal(aDate, aBorneDateFin)))
		);
	}
	estDateJourAvant(aDate1, aDate2) {
		if (aDate1 && aDate2) {
			...
			return (
				this.estAvantJour(aDate1, aDate2) && !this.estJourEgal(aDate1, aDate2)
			);
		} else {
			return false;
		}
			...
			if (!this._cacheSemainesCalendaires[aSemaine]) {
				...
				if (aSemaine <= 1) {
					this._cacheSemainesCalendaires[aSemaine] =
						((aSemaine - 1 + (this.getSemaineCalendaire() - 1)) % lNbSemaines) +
						1;
				} else if (aSemaine > 0) {
	...
	getDifferenceTime(aDate1, aDate2) {
		const lDureeInMs =
			this.estDateValide(aDate1) && this.estDateValide(aDate2)
				...
				: 0;
		const reste_heures =
			lDureeInMs - Math.floor(lDureeInMs / this.DureeJours) * this.DureeJours;
		const nb_heures = Math.floor(reste_heures / this.DureeHeures);
		const reste_minutes =
			lDureeInMs - Math.floor(lDureeInMs / this.DureeHeures) * this.DureeHeures;
	...
	formatDate(ADate, AFormat) {
		...
		if (!AFormat || !MethodesObjet_1.MethodesObjet.isString(AFormat)) {
			IE.log.addLog(
				"ObjetDate.formatDate, AFormat n'est pas format valide : " + AFormat,
			);
			return "";
		}
		...
		AFormat = AFormat.replace(
			"%Jjj",
			ObjetTraduction_1.GTraductions.getValeur("JoursCourt")[LDay].ucfirst(),
		);
		...
		switch (lFormatDureeEnChaine) {
			...
			case TypeFormatDureeEnChaine_1.TypeFormatDureeEnChaine.fdcSequences: {
				const lNbSequences = Math.round10(
					aDureeEnMillisecondes /
						UtilitaireDuree_1.TUtilitaireDuree.dureeEnMs(this.DureeSequence),
					-2,
				);
	...
	dateEnPlaceAnnuelle(aDate, aEstDateFin = false) {
		const lPlaceAnnuelleJour =
			this.getNbrJoursEntreDeuxDates(IE.Cycles.dateDebutPremierCycle(), aDate) *
			this.PlacesParJour;
	...
	placeEnDate(aNumeroCycle, aPlace, aEstPlaceFin = false, aInstanceCycles) {
		...
		if (
			lNumeroJour > lCycles.cache.datesAfficheesParCycle[lNumeroCycle].length
		) {
	...
	minutesEnNombrePlaces(aDureeEnMinutes) {
		...
		const lDuree1PlaceEnMinutes = UtilitaireDuree_1.TUtilitaireDuree.dureeEnMin(
