...
(function (TypeAffichageContenu) {
	...
	TypeAffichageContenu["PageConsignes"] = "PageConsignes";
...
class ObjetVisuEleve {
	constructor(aNom, aEvenement) {
		...
		this.controleur = {
			...
			Timeline: {
				...
				avecAffichageChrono() {
					return !!(
						aInstance.executionQCM && aInstance.executionQCM.dureeMaxQCM
					);
				},
			},
			Footer: {
					...
					event() {
						...
						switch (lTypePage) {
							...
							case TypeAffichageContenu.PageModalite:
								if (aInstance.executionQCM.consigne) {
									aInstance.typeAffichageCourant =
										TypeAffichageContenu.PageConsignes;
								...
								break;
							case TypeAffichageContenu.PageConsignes:
								aInstance.surEvenement({
									action: TypeActionVisuEleveQCM.Commencer,
								});
								break;
							default: {
								aInstance.surEvenement({
									action: TypeActionVisuEleveQCM.Valider,
								});
								break;
							}
						}
			...
			Question: {
				...
				Associations: {
					modelAssociationItem: {
						event(aIndiceQuestion, aIndiceReponseDestination) {
									...
									ObjetFenetreChoixAssociation,
									{
										...
										evenement(aNumeroBouton, aParametresSelection) {
											if (aNumeroBouton === 1) {
												...
												let lHashReponseBAAssocier = "";
												let lLibelleAssocB = "";
												if (lElementAssociationBAAssocier) {
													...
													if (lEstBesoinDesaffecterReponse) {
														$(
															...
															if (
																...
																$(this).data("hash", "").html("");
																const lIndiceReponse = parseInt(
																	this.id.split("_assoc_")[1],
																);
																$(
																	"#" +
																		aInstance
																			.getIdZoneReponsesDeQuestion(
																				lIndiceReponse,
																			)
																			.escapeJQ(),
																).removeClass("has-asso");
															}
														});
													}
						...
						getLibelle(aIndiceQuestion, aIndiceReponseDestination) {
							return aInstance.estAssociationAvecReponseAssociee(
								aIndiceQuestion,
								aIndiceReponseDestination,
							)
								? ObjetTraduction_1.GTraductions.getValeur("Modifier")
								: ObjetTraduction_1.GTraductions.getValeur(
										"ExecutionQCM.associer",
									);
						},
					},
							...
							if (lIdReponseAssociee) {
								...
								if ($zoneReponseAssociee) {
									...
									$(
										"#" +
											aInstance
												.getIdZoneReponsesDeQuestion(aIndiceReponseDestination)
												.escapeJQ(),
									).removeClass("has-asso");
									$(
										"#" +
	...
	estAssociationAvecReponseAssociee(
		aIndiceQuestion,
		aIndiceReponseDestination,
	) {
		...
		if (lIdReponseAssociee) {
			const $zoneReponseAssociee = $("#" + lIdReponseAssociee.escapeJQ());
	...
	getInfosLibelleComplementaireDeScoreSurPageCorrige(aIndiceQuestion) {
		...
		if (
			...
			if (lEvalNote.question) {
				if (
					lEvalNote.question &&
					((_a = lEvalNote.question.reponsesRepondant) === null || _a === void 0
						? void 0
						: _a.length) &&
					UtilitaireQCM_1.UtilitaireQCM.avecReponseRepondant(
						lEvalNote.question.reponsesRepondant,
					)
				) {
					...
					if (lEvalNote.question.noteEleve) {
						...
						lEstPartiel =
							this.executionQCM.acceptIncomplet &&
							lEvalNote.nbReponsesJustes > 0;
					}
					if (lEstBonneReponse) {
						return {
	...
	actualiser() {
		...
		switch (lTypePage) {
			case TypeAffichageContenu.PageModalite:
				...
				break;
			case TypeAffichageContenu.PageConsignes:
				lHtml.push(this.composePageConsignes());
				break;
			case TypeAffichageContenu.PagePresentationCorrige:
		...
		const lTitreAFocus = $("#" + this.idContenu + " h2").get(0);
		if (lTitreAFocus) {
			ObjetHtml_1.GHtml.setFocus(lTitreAFocus);
		}
		if (
			[
	...
	composePageModalite() {
		const H = [];
		H.push(
			`<section class="${ObjetVisuEleveQCM_css_1.StylesObjetVisuEleveQCM.content}">`,
		);
		H.push(
			'<header><h2 tabindex="0">',
			ObjetTraduction_1.GTraductions.getValeur("ExecutionQCM.APropos"),
			"</h2></header>",
		);
		H.push("<ul>");
		function _creerLI(aNomIcone, aLibelle) {
			return IE.jsx.str(
				"li",
				null,
				IE.jsx.str("i", { role: "presentation", class: aNomIcone }),
				IE.jsx.str("div", null, aLibelle),
			);
		}
		let lTrad = "";
		let lClassesIcone = [];
		...
		if (lTrad !== "") {
			let lLibelleNavigationQCM =
				ObjetTraduction_1.GTraductions.getValeur(lTrad);
			if (lAvecNavigationPossible) {
				lLibelleNavigationQCM += `<span class="${ObjetVisuEleveQCM_css_1.StylesObjetVisuEleveQCM.important}">${ObjetTraduction_1.GTraductions.getValeur("ExecutionQCM.presentation.RS_IlFautCliquerSurValider")}</span>`;
			}
		...
		if (lAfficherNbTentativesPossibles) {
			...
			if (this.executionQCM.nbTentatives) {
				...
				lLibelleNbTentatives += `<span class="${ObjetVisuEleveQCM_css_1.StylesObjetVisuEleveQCM.important}">${ObjetTraduction_1.GTraductions.getValeur("ExecutionQCM.tentatives.xRestants", [lNrRest, this.executionQCM.nbMaxTentative])}</span>`;
			}
	...
	composePageConsignes() {
		return IE.jsx.str(
			"section",
			{ class: ObjetVisuEleveQCM_css_1.StylesObjetVisuEleveQCM.content },
			IE.jsx.str(
				"header",
				null,
				IE.jsx.str(
			...
			IE.jsx.str(
				...
				{
					class: [
						ObjetVisuEleveQCM_css_1.StylesObjetVisuEleveQCM.consigneWrapper,
		...
		if (!lEstAffichageCopieCachee) {
			if (
				...
				if (
					...
					lCartoucheCompetences.push('<li class="competences">');
					if (!lIconeTypeExecQCMAEteAjoute) {
		...
		H.push(
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						"span",
						{ class: "sr-only", "ie-if": "CorrigeQuestion.avecWAICorriger" },
						ObjetTraduction_1.GTraductions.getValeur(
							"ExecutionQCM.Corriger.WAI",
						),
						"\u00A0",
					),
				...
				IE.jsx.str(
					...
					IE.jsx.str("span", {
						"ie-class": (0, jsx_1.jsxFuncAttr)(
							"CorrigeQuestion.getClasseCSSLibelleComplementaire",
							aIndiceQuestion,
						),
						"ie-html": (0, jsx_1.jsxFuncAttr)(
							"CorrigeQuestion.getLibelleComplementaireScoreDeQuestion",
							aIndiceQuestion,
						),
						"ie-if": (0, jsx_1.jsxFuncAttr)(
							"CorrigeQuestion.avecAffichageLibelleComplementaireScoreDeQuestion",
							aIndiceQuestion,
						),
					}),
					IE.jsx.str("span", {
						class: "score",
						"ie-class": (0, jsx_1.jsxFuncAttr)(
							"Question.getClasseScoreReponse",
							aIndiceQuestion,
						),
						"ie-html": (0, jsx_1.jsxFuncAttr)(
							"Question.getHtmlScore",
							aIndiceQuestion,
						),
					}),
					IE.jsx.str(
						...
						{
							"ie-if": (0, jsx_1.jsxFuncAttr)(
								"CorrigeQuestion.avecAffichageCommentaireQuestionAnnotee",
								aIndiceQuestion,
							),
						},
						"* ",
			...
			lArrEnonce.push(
				IE.jsx.str(
					...
					IE.jsx.str("img", {
						...
						onerror:
							"$(this).parent().ieHtml(GTraductions.getValeur('" +
							lCleTrad +
							"')",
					}),
				),
	...
	_composeReponseDeQuestionSingleOuMulti(aParams) {
		...
		const lIdVisuReponse = lIdQuestion + "_visu" + aParams.indiceReponse;
				...
				lContenuCbrVisu.push(
					IE.jsx.str(
						...
						IE.jsx.str("img", {
							...
							src:
								"data:image/png;base64," +
								ObjetChaine_1.GChaine.supprimerRC(aParams.reponse.image),
						}),
					),
				);
			}
		...
		const lNodeInputReponse = (aNode) => {
			return $(aNode).eventValidation(() => {
				if (lTypeElement === "checkbox") {
					if (!$("#" + lIdReponse.escapeJQ()).attr("checked")) {
						$("#" + lIdReponse.escapeJQ()).attr("checked", "checked");
					} else if (lTypeElement === "checkbox") {
						$("#" + lIdReponse.escapeJQ()).removeAttr("checked");
					}
				} else {
					$('input[name="' + lNameGroupeElements + '"]').removeAttr("checked");
					$("#" + lIdReponse.escapeJQ()).attr("checked", "true");
				}
			});
		};
		const lHtml = [];
		lHtml.push(
			IE.jsx.str(
				...
				IE.jsx.str(
					...
					{
						id: lIdVisuReponse,
						"ie-node": lNodeInputReponse.bind(this),
						class: "cbr-visu",
					},
					IE.jsx.str("span", { class: "cbr-coche", "aria-hidden": "true" }),
					aParams.avecReponseMiseEnForme
						? lContenuCbrVisu.join("")
						: IE.jsx.str("span", { class: "lib-text" }, lContenuCbrVisu),
				),
			),
		);
		return lHtml.join("");
	...
	_composeReponsesDeQuestionShortOuNumeric(aQuestion, aIndiceQuestion) {
		const lUniquementReponseNumerique =
			...
			TypeGenreExerciceDeQuestionnaire_1.TypeGenreExerciceDeQuestionnaire
				.GEQ_NumericalAnswer;
		const lTypeChamp = "text";
		const lAttributesInputNumerique = {};
		if (lUniquementReponseNumerique) {
			lAttributesInputNumerique["ie-mask"] = "/[^0-9,.+-]/i";
		}
		...
		lHtml.push(
			IE.jsx.str(
				"div",
				{ class: "fields-wrapper zone-reponse" },
				IE.jsx.str(
					"label",
					{ class: "input-wrap", for: lPrefixeIdQuestion + "_r0" },
					lUniquementReponseNumerique
						? ObjetTraduction_1.GTraductions.getValeur(
								"QCM_Divers.MaReponseNumerique",
							)
						: ObjetTraduction_1.GTraductions.getValeur("QCM_Divers.MaReponse"),
				),
				IE.jsx.str(
					"input",
					Object.assign(
						{ class: "for-qcm", type: lTypeChamp },
						lAttributesInputNumerique,
						{ id: lPrefixeIdQuestion + "_r0", maxlength: "35" },
					),
				),
			),
		);
		...
		if (!!aQuestion.longueurReponseEpellation) {
			...
			lHtml.push(
				IE.jsx.str(
					"div",
					{
						class: "fields-wrapper zone-reponse spell",
						role: "group",
						"aria-labelledby": lPrefixeIdQuestion + "_l",
					},
					IE.jsx.str(
						"p",
						{ id: lPrefixeIdQuestion + "_l" },
						ObjetTraduction_1.GTraductions.getValeur(
							"QCM_Divers.XCaracteresAttendus",
							[aQuestion.longueurReponseEpellation],
						),
					),
					IE.jsx.str(
						"div",
						{ class: "spell-block" },
						IE.jsx.str("div", { class: "spell-contain" }, () => {
							const H = [];
							for (let i = 0; i < aQuestion.longueurReponseEpellation; i++) {
								H.push(
									IE.jsx.str("input", {
										name: lPrefixeIdQuestion + "_r",
										type: "text",
										maxlength: "1",
										"aria-label": ObjetTraduction_1.GTraductions.getValeur(
											"QCM_Divers.PositionLettre",
											[i + 1],
										),
										"ie-node": "Question.Epellation.getNodeInput",
									}),
								);
							}
							return H.join("");
	...
	_composeReponsesDeQuestionMatching(aQuestion, aIndiceQuestion) {
		...
		for (let i = 0; i < aQuestion.listeReponses.count(); i++) {
			...
			const lIdReponseDeQuestion = this.getIdZoneReponsesDeQuestion(i);
			const lIdReponseAssociee = this.getIdZoneReponseAssociee(
				aIndiceQuestion,
				i,
			);
			const lIEModelAssociationItem =
				"Question.Associations.modelAssociationItem(" +
				aIndiceQuestion +
				"," +
				i +
				")";
				...
				IE.jsx.str(
					...
					IE.jsx.str(
						...
						IE.jsx.str(
							...
							IE.jsx.str(
								"div",
								{ id: lIdReponseDeQuestion, class: "asso-question" },
								lLibelleAssocA,
							),
							IE.jsx.str(
								"div",
								{ class: "bt-asso-contain" },
								IE.jsx.str("ie-bouton", {
									class: "small-bt",
									"ie-model": lIEModelAssociationItem,
								}),
							),
						),
						IE.jsx.str(
	...
	_composeReponsesDeQuestionCloze(aQuestion, aIndiceQuestion) {
		...
		if (aQuestion.enonce) {
			...
			if (
				...
				lEnonceAvecElementsGraphiques = lEnonceAvecElementsGraphiques.replace(
					...
					() => {
						return IE.jsx.str("input", {
							"aria-label": ObjetTraduction_1.GTraductions.getValeur(
								"SaisieQCM.TexteARenseigner",
								[lIndice + 1],
							),
							class: "for-qcm",
							type: "text",
							id: lPrefixeIdQuestion + "_r" + lIndice++,
						});
					},
				);
			} else if (
				...
				for (let i = 0; i < aQuestion.listeReponses.count(); i++) {
					let lComboAMettre = "";
					lComboAMettre +=
						'<select class="for-qcm" aria-label="' +
						ObjetTraduction_1.GTraductions.getValeur("SaisieQCM.Choix", [
							i + 1,
						]) +
						'" id="' +
						lPrefixeIdQuestion +
		...
		return IE.jsx.str(
			"div",
			{ class: ["zone-reponse", Divers_css_1.StylesDivers.tinyView] },
			lEnonceAvecElementsGraphiques,
		);
	}
}
...
class ObjetFenetreChoixAssociation extends ObjetFenetre_1.ObjetFenetre {
	...
	jsxModelRadioItemAAssocier(aIndex, aName) {
		return this.getModelRadioItem(aIndex, aName);
	}
	jsxModelRadioItemDejaAssocier(aIndex, aName) {
		return this.getModelRadioItem(aIndex, aName);
	}
	getModelRadioItem(aIndex, aName) {
		return {
			getValue: () => {
				return this.indiceReponseDisponibleSelectionnee === aIndex;
			},
			setValue: () => {
				this.indiceReponseDisponibleSelectionnee = aIndex;
			},
			getName: () => {
	...
	_composeListeReponsesDisponibles(aListeElementsAssociationB) {
		...
		const lReponsesDejaAffectees = [];
		if (aListeElementsAssociationB) {
			let lIndex = 0;
			for (const lElementAssociationB of aListeElementsAssociationB) {
				const lItem = IE.jsx.str(
					"div",
					{ class: "reponse-item" },
					IE.jsx.str(
						"ie-radio",
						{
							"ie-model": lElementAssociationB.estAffecte
								? this.jsxModelRadioItemDejaAssocier.bind(
										this,
										lIndex,
										"ItemDejaAssocier",
									)
								: this.jsxModelRadioItemAAssocier.bind(
										this,
										lIndex,
										"ItemAAssocier",
									),
						...
						UtilitaireQCM_1.UtilitaireQCM.getLibelleAffichageReponseMatching(
							lElementAssociationB.elementAssociationB,
							{ indiceReponse: lIndex },
						),
					),
					lElementAssociationB.elementAssociationB.getGenre() !==
				...
				if (lElementAssociationB.estAffecte) {
					lReponsesDejaAffectees.push(lItem);
				} else {
					lReponsesDisponibles.push(lItem);
				}
				lIndex++;
			}
		}
			...
			lReponsesDisponibles.length
				? IE.jsx.str(
						...
						IE.jsx.str(
							...
							null,
							ObjetTraduction_1.GTraductions.getValeur(
								"ExecutionQCM.ElementsAAssocier",
							),
						),
						IE.jsx.str(
							"div",
			...
			lReponsesDejaAffectees.length
				? IE.jsx.str(
						...
						IE.jsx.str(
							...
							null,
							ObjetTraduction_1.GTraductions.getValeur(
								"ExecutionQCM.ElementsDejaAssocie",
							),
						),
						IE.jsx.str(
							"div",
							{ class: "liste-reponses" },
							lReponsesDejaAffectees.join(""),
						),
					)
				: "",
		);
