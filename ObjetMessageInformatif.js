...
class ObjetMessageInformatif {
	...
	_afficherContenu(aMessage, aDuree, aDetailMessageInformatif, aParams) {
		this._arreterTimerAffichage();
		const lParams = Object.assign({ htmlPied: "", controleur: null }, aParams);
		const lHtml = IE.jsx.str(
			...
			IE.jsx.str(
				...
				IE.estMobile
					? IE.jsx.str("div", {
							class:
								ObjetMessageInformatif_module_css_1.StylesObjetMessageInformatif
									.overlay,
						})
					: "",
				IE.jsx.str(
					...
					IE.jsx.str(
						...
						IE.jsx.str(
							"div",
							{ id: this.IdMessageInformatif, tabindex: "-1", role: "alert" },
							aMessage,
						),
						...
						this.AvecBarreDeProgression
							...
							: "",
						lParams.htmlPied ? IE.jsx.str("div", null, lParams.htmlPied) : "",
					),
				),
			),
		);
		...
		try {
			...
			try {
				document.getElementById(this.IdMessageInformatif).style.outlineWidth =
					"0px";
			} catch (e) {}
		}
	...
	_surEventIOAjax(aParametres) {
		...
		if (aParametres.blocageDeconnexion === true) {
			...
			this.bloquerAfficherAvecDelai(
				ObjetTraduction_1.GTraductions.getValeur("requete.VeuillezPatienter"),
				0,
				aParametres.message || "",
			);
			return;
	...
	_surPatience(aAfficher, aParametres) {
		const lParametres = Object.assign({ delai: 0, message: "" }, aParametres);
		if (aAfficher) {
			this.bloquerAfficherAvecDelai(
				ObjetTraduction_1.GTraductions.getValeur("requete.VeuillezPatienter"),
				lParametres.delai || 0,
