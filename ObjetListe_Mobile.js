exports.ObjetListe_Mobile = void 0;
...
class ObjetListe_Mobile extends _ObjetListe_1.ObjetListe {
			...
			getClassListe: () => {
				...
				if (
					...
					!document.querySelector(
						`#${this.Nom.escapeJQ()} .${ObjetListe_Mobile_css_1.StylesObjetListe_Mobile.liste_scroll}`,
					)
				) {
						...
						"div",
						{
							...
							"ie-class":
								lAvecEntete || this._options.forcerOmbreScrollTop
									? false
									: this.jsxGetClassNodeOmbre.bind(this),
						},
						IE.jsx.str("div", null),
					)
				: "",
	...
	_actualiserZones(aZonesActualisation, aParamsActualiser) {
		...
		if (lElement) {
			ObjetHtml_1.GHtml.setHtml(lElement, this.construireZoneScrollInterne(), {
	...
	_backupScroll(aParams) {
		...
		let lGetterScroll = () => {
			return $(
				"#" +
					this.Nom.escapeJQ() +
					`>.ObjetListe>div.${ObjetListe_Mobile_css_1.StylesObjetListe_Mobile.liste_scroll}`,
			).get(0);
		};
	...
	_getClassCouleurCellule(aParamsCellule) {
		...
		const LCouleurCellule = lDonneesTableau
			? lDonneesTableau.getCouleurCellule(aParamsCellule)
			: null;
		...
		if (this._options.avecCocheCheckBox) {
			return [
				IE.jsx.str(
					...
					IE.jsx.str("ie-checkbox", {
						"ie-model": this.jsxModelCelluleCoche.bind(
							this,
							aParams.ligne,
							aParams.colonne,
							lValeur,
				...
				Object.assign(
					{
						...
						"ie-nodeafter": !this.Donnees.enConstruction_cacheRechercheTexte
							...
							: false,
						"ie-attr": !this.Donnees.enConstruction_cacheRechercheTexte
		...
		for (
			...
			aInfosColonnes.colonnesVisibles.forEach((aNumeroColonne, aIndex) => {
				...
				if (
					...
					lClasses.push(IEHtml_Ripple_css_1.StylesIEHtmlRipple.ieRipple);
				}
	...
	_construireTotal() {
		...
		const H = [];
		const lLignes =
			(lDonneesTableau ? lDonneesTableau.getListeLignesTotal() : null) ||
		...
		lLignes.parcourir((aArticle, aIndexLigne) => {
			...
			lInfosColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
				...
				const lNodeCelluleTotal = (aNode) => {
					const lEventMapCelluleTd = { click: this._surClickCelluleTotal };
	...
	_contruireBouton(aIcon, aLibelle, aAttrs) {
		return IE.jsx.str(
			...
			Object.assign(
				...
				aAttrs,
				{ "aria-label": aLibelle || false, title: aLibelle || false },
			),
			IE.jsx.str("i", { class: aIcon, role: "presentation" }),
		);
	}
	_construireBoutons() {
		const H = [];
		...
		this._getTabBoutonsEnteteOuPiedFD(false).forEach((aInfoBouton) => {
			...
			lHtmlBoutons.push(
				this._contruireBouton(lBouton.class, lBouton.title, {
					"ie-attr": (aNode) => {
		...
		if (lAvecCreation) {
			...
			const lAttr = lAvecInputFile
				? {
						"ie-model": this.jsxModeleSelecFile.bind(this, this, lParams),
						"ie-selecfile": true,
					}
				: {
						"ie-node": (aNode) => {
							const lThis = this;
							$(aNode).eventValidation(function () {
	...
	_construireZoneScroll(aAvecScrollListe) {
		...
		let lResult = IE.jsx.str(
			...
			IE.jsx.str(
				"div",
				{
					...
					"ie-node": this.jsxGetNodePremierElementMobile.bind(this),
				},
		...
		if (this._estRoleTreeGrid()) {
			lResult += this.construireGridWAI(false, false);
		}
		if (aAvecScrollListe) {
			lResult = IE.jsx.str(
				"div",
				{
					class: ObjetListe_Mobile_css_1.StylesObjetListe_Mobile.liste_scroll,
					role: "presentation",
				},
				lResult,
			);
		...
		return (
			...
			this._options.messageContenuVide
				? IE.jsx.str(
						...
						{
							class:
								ObjetListe_Mobile_css_1.StylesObjetListe_Mobile
									.liste_messageVide,
	...
	jsxNodeCellulePere(aLigne, aColonne, aNode) {
		const lThis = this;
	...
	jsxGetClassNodeOmbre(aNode) {
		const lNode = $(
			`#${this.Nom.escapeJQ()} .${ObjetListe_Mobile_css_1.StylesObjetListe_Mobile.liste_scroll}`,
		).get(0);
		if (lNode && lNode.clientHeight < lNode.scrollHeight) {
			aNode.classList.remove("hide");
		} else {
			aNode.classList.add("hide");
		}
		return "";
	}
	jsxModelCelluleCoche(aNumeroLigne, aNumeroColonne, aValeur) {
		return {
			getValue() {
				return (
					aValeur === true ||
					aValeur === ObjetDonneesListe_1.ObjetDonneesListe.EGenreCoche.Verte
				);
			},
			getIndeterminate() {
				return (
					aValeur === ObjetDonneesListe_1.ObjetDonneesListe.EGenreCoche.Grise
				);
			},
			setValue() {},
			getDisabled: () => {
