	...
	_construirePageFinSession(aParametres) {
		...
		lMessage =
			lMessage === null || lMessage === undefined
				...
				: lMessage;
		const lEstPrimaire = !!lApp.estPrimaire;
		const T = [];
		try {
			T.push(lApp.construireEnTetePageFinSession(lParametresFin));
		} catch (e) {}
		T.push(
			`<main class="deconnexion bg-mobile-${!lEstPrimaire ? lApp.nomProduit.toLowerCase() : `primaire`}">`,
		);
		T.push(`<div class="title-content">`);
		T.push(
			`<h3 class="logo_${(lApp.nomProduit).toLowerCase()} ${lEstPrimaire ? ` primaire` : ""} "></h3>`,
		);
		const lModel = () => {
			return {
				event() {
		...
		T.push(
			...
			lGenreErreur !== 7 && !lParametresFin.sansBoutonSeConnecter
				? IE.jsx.str(
						"ie-bouton",
						{
							class: IEHtml_Bouton_css_1.StylesIEHtmlBouton.themeBoutonPrimaire,
							"ie-model": lModel,
						},
		...
		ObjetHtml_1.GHtml.setHtml(lApp.getIdConteneur(), T.join(""));
