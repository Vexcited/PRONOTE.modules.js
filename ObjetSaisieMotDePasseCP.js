...
class ObjetSaisieMotDePasseCP extends ObjetIdentite_1.Identite {
	constructor(...aParams) {
		...
		this.options = {
			...
			libelleNewMDP: ObjetTraduction_1.GTraductions.getValeur(
				"saisieMDP.NewMotDePasse",
			),
			libelleConfirmNewMDP: ObjetTraduction_1.GTraductions.getValeur(
				"saisieMDP.Confirmation",
			),
			...
			libelleMDPActuelIncorrect: ObjetTraduction_1.GTraductions.getValeur(
				"saisieMDP.MotActuelIncorrect",
			),
			libelleConfirmationIncorrecte: ObjetTraduction_1.GTraductions.getValeur(
	...
	_getOptionsValidationMdp() {
		return { avecMdpDifferent: this.options.avecMdpNouveauDifferentDeActuel };
	}
	_construireHtml() {
		const H = [];
		...
		if (this.options.avecReglesMDP) {
			H.push(
				IE.jsx.str(
					...
					IE.jsx.str("div", { class: "validateur", id: this.ids.validateur }),
				),
			);
		}
		H.push("</div>");
