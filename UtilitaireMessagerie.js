						...
						(H) => {
							...
							H.push(
								(0, tag_1.tag)(
									...
									lMessage.estHTML
										? lMessage.contenu
										: ObjetChaine_1.GChaine.replaceRCToHTML(lMessage.contenu),
							...
							if (lMessage.listeMessagesPourContexte) {
								...
								for (J = lNbMessagsContexte - 1; J >= 0; J--) {
									const lMessagePourContexte =
										lMessage.listeMessagesPourContexte.get(J);
									H.push(
										(0, tag_1.tag)("div", { class: "visu_transfert" }, (H) => {
											H.push(
												UtilitaireMessagerie.getLibelleParticipants(
...
UtilitaireMessagerie.getLibelleParticipants = function (
	...
	if (aMessage.estMessageTransferant) {
		...
		if (aMessage.brouillon) {
					...
					H.push(
						'<span class="participants_public"' +
							(aMessage.hint_droite && !IE.estMobile
								? ObjetHtml_1.GHtml.composeAttr(
										"ie-hint",
										ObjetChaine_1.GChaine.replaceRCToHTML(aMessage.hint_droite),
									)
								: "") +
							" ie-ellipsis >" +
			...
			if (aPourVisuTransfert) {
				H.push(
					'<div class="regular m-left-l">' + aMessage.libelleDate + "</div>",
				);
			}
		}
		if (aPourVisuTransfert) {
...
UtilitaireMessagerie.afficherFenetreDestinatairesDeMessage = async function (
	aNumeroMessage,
	aEstPublicParticipant,
	aEstDestinatairesReponse,
) {
	...
	const lHtml = IE.jsx.str("ul", { class: "liste-destinataires" }, (aTab) => {
			...
			aTab.push(
				IE.jsx.str(
					...
					lListeDest.avecConnection
						? UtilitaireMessagerie.contruirePublicConnecte(aDest, lLibelleSupp)
	...
	if (lListePublics) {
		...
		lListePublics.parcourir((aPublic) => {
			...
			if (lListePublics.avecConnection) {
				lTab.push(
					UtilitaireMessagerie.contruirePublicConnecte(aPublic, lLibelleSupp),
				);
			} else {
		...
		if (lNb > 0) {
			...
			if (aEstPublicParticipant) {
				lStrParticipants = ObjetTraduction_1.GTraductions.getValeur(
					"Messagerie.NParticipants",
					[lNb],
				);
			} else if (lNb > 1) {
...
UtilitaireMessagerie.contruirePublicConnecteStatutConnexion = function (
	...
	return aPublic.refusMess
		? ObjetTraduction_1.GTraductions.getValeur("Messagerie.RefuseDiscussions")
...
UtilitaireMessagerie.construireImageEtiquette = function (aEtiquette) {
	...
	if (lIcone) {
		lResult.icone = IE.jsx.str("i", { class: lIcone, role: "presentation" });
	}
	if (
	...
	if (
		aEtiquette.getGenre() ===
			TypeOrigineCreationEtiquetteMessage_1.TypeOrigineCreationEtiquetteMessage
				.OCEM_Pre_Brouillon ||
		aEtiquette.getGenre() ===
			TypeOrigineCreationEtiquetteMessage_1.TypeOrigineCreationEtiquetteMessage
				.OCEM_Pre_Poubelle
...
UtilitaireMessagerie.getNbNonlusDEtiquette = function (
	...
	if (aListeMessagerie && aEtiquette) {
		aListeMessagerie.parcourir((D) => {
			if (
				UtilitaireMessagerie.estDiscussionVisibleSelonEtiquette(D, aEtiquette)
			) {
...
UtilitaireMessagerie.selectionnerListePublics = async (aParams) => {
	...
	if (
		...
		lDonnees = await new ObjetRequeteListePublics_1.ObjetRequeteListePublics(
			lParams.instance,
		).lancerRequete({
			pourMessagerie: true,
		...
		if (
			...
			lParams.genreRessource === Enumere_Ressource_1.EGenreRessource.Responsable
		) {
	...
	if (
		...
		lGenreCumul =
			ObjetFenetre_SelectionPublic_1.TypeGenreCumulSelectionPublic.respDelegues;
	}
	...
	if (
		...
		if (
			...
			MethodesObjet_1.MethodesObjet.isBoolean(lListeRessources.get(0).estAbsent)
		) {
			lListeCumuls.addElement(
				new ObjetElement_1.ObjetElement(
					ObjetTraduction_1.GTraductions.getValeur(
...
UtilitaireMessagerie.getLibelleRaccourciMessPrimEleve = () => {
	...
	if (lListeDest && lListeDest.count() > 0) {
		return lListeDest.count() > 1
			? ObjetTraduction_1.GTraductions.getValeur("Messagerie.EcrireUnMessage")
			: ObjetTraduction_1.GTraductions.getValeur(
...
UtilitaireMessagerie.creerDiscussionAvecMairie = (aObjetFenetre, aInstance) => {
	...
	lFenetreMessage.setDonnees({
		ListeRessources:
			GEtatUtilisateur.Identification.ressource.destinatairePersonnelsMairie,
