...
class ObjetListeElements {
	...
	getIndiceParParsingLibelle(aLibelle) {
		...
		for (let I = 0; I < N; I++) {
			...
			if (
				ComparateurChaines_1.ComparateurChaines.compare(lLibelle, aLibelle) < 0
			) {
				lIndice++;
			} else {
				break;
	...
	listeIdentiqueParElementsOrdonnes(aListeElements) {
		...
		this.parcourir((aElement, aIndice) => {
			...
			if (
				lElement &&
				aElement &&
				lElement.egalParNumeroEtGenre(aElement.getNumero(), aElement.getGenre())
			) {
				return;
			}
			lResult = false;
	...
	toJSON() {
		let I,
			lNb,
			lAvecSerialisation = !!(
				this.serialisateurJSON && this.serialisateurJSON.methodeSerialisation
			),
			lResult = [],
			...
			if (lElement) {
				...
				if (lExisteElement || lAvecSerialisation) {
					...
					if (
						lAvecSerialisation &&
						this.serialisateurJSON &&
						typeof this.serialisateurJSON.methodeSerialisation === "function"
					) {
						const lGarderElement = this.serialisateurJSON.methodeSerialisation(
							lElement,
							lJSONElement,
							I,
						);
