...
class ObjetSaisie extends ObjetIdentite_1.Identite {
		...
		return $.extend(true, super.getControleur(aInstance), {
			...
			btnValidationCB: {
				event: function (aEvent) {
					...
					aInstance.surValidation(true, -1, { validationMultiSelection: true });
				},
	...
	setParametres(
		...
		this.setOptionsObjetSaisie({
			mode: AMode,
			longueur:
				ALongueur === null || ALongueur === undefined ? undefined : ALongueur,
			hauteur: aHauteur,
			avecBouton:
		...
		if (
			...
			if (lEstMultiSelection) {
				...
				$(
					...
					ObjetHtml_1.GHtml.getElement(lIdDescendant) ? lIdDescendant : null,
				);
			} else if (
				ASelection >= -1 &&
				!lParams.validationMultiSelection &&
	...
	_remplirListe() {
		...
		if (ObjetHtml_1.GHtml.elementExiste(this.NomListe)) {
			...
			if (lEnteteListe && lEnteteListe.html) {
				$("#" + this.NomListe.escapeJQ() + " .ObjetSaisie_entete_liste").ieHtml(
					lEnteteListe.html,
					{ controleur: lEnteteListe.controleur, instance: this },
				);
			}
			if (lPiedListe && lPiedListe.html) {
				$("#" + this.NomListe.escapeJQ() + " .ObjetSaisie_pied_liste").ieHtml(
					lPiedListe.html,
					{ controleur: lPiedListe.controleur, instance: this },
				);
			}
			if (this.cache.scrollInfiniActif) {
				let lTop = 0,
				...
				switch (this._options.mode) {
					case Enumere_Saisie_1.EGenreSaisie.Combo: {
						...
						if (
							...
							!this._options.multiSelection &&
							!((_a = ObjetHtml_1.GHtml.getElement(this.getNom())) === null ||
							_a === void 0
								? void 0
			...
			Object.assign(
				{
					...
					element:
						this.Selection >= 0 ? this.ListeElements.get(this.Selection) : null,
		...
		this.cache.blocs.forEach((aBloc, aNumeroBloc) => {
			const lVisible = aSurFermeture
				...
				: aNumeroBloc <=
					...
					aNumeroBloc >= aNumeroBlocVisible - this._options.nbBlocsScrollInfini;
