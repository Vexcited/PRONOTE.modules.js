...
class ObjetListeEspace extends _ObjetListe_1.ObjetListe {
	...
	_getInfosZonesColonnes(aCache) {
		...
		aCache.avecScrollHMultiZonePrevu =
			this._options.scrollHorizontal && this._options.scrollHorizontal !== true;
	...
	_construireAffichage() {
		...
		return IE.jsx.str(
			"div",
			{
				...
				class:
					ObjetListeEspaceMobile_css_1.StylesObjetListeEspaceMobile.ObjetListe,
		...
		return IE.jsx.str(
			"div",
			{
				...
				"ie-tooltiplabel": lTriInactif
					? ObjetTraduction_1.GTraductions.getValeur("liste.wai.TrierColonne")
				...
				for (
					...
					if (
						...
						const lAvecChaineHtml = !!(
							lDescripteurTitre.getLibelleHtml || lDescripteurTitre.libelleHtml
						);
						...
						for (let lILigneSpan = 0; lILigneSpan < lNbLigne; lILigneSpan++) {
							for (
								...
								if (lILigneSpan > 0 || lIColonneSpan > 0) {
									...
									const lIdColBidon =
										lGetIdTitre(iLigne + lILigneSpan, lIndiceColVisibleSpan) +
										"_wai";
						...
						const lAttrTitre = lAvecTri
							...
							: false;
						const lIdTooltip =
							lAriaLabelTitre || lFuncTooltipLabel ? `${lId}_tooltip_t` : "";
						let lHtmlTooltip = "";
						if (lIdTooltip) {
							const lAttrs = {
								id: lIdTooltip,
										...
										if (!lChaineHtml) {
											if (lDescripteurTitre.estCoche) {
												lContenuTitre += IE.jsx.str("ie-checkbox", {
													"ie-model": this.jsxGetModelCocheTitreClassique.bind(
														this,
														this,
														I,
													),
													"ie-attr": this.jsxGetAttrCocheTitreClassique.bind(
														this,
														I,
													),
												});
													...
													lContenuTitre += IE.jsx.str(
														"span",
														{
															class: [
																...
																lAvecTitleExistant
																	? IEHtmlComposants_1.StylesIEHtmlComposants
																			.ellipsisCancelTitle
																	: "",
															],
															style: {
		...
		return IE.jsx.str(
			"div",
			{
				...
				style:
					...
					(lPaddingRight > 0 ? "padding-right:" + lPaddingRight + "px;" : ""),
				role: "presentation",
			},
			IE.jsx.str(
				"div",
	...
	jsxDragCurseurTriDraggable(aColonne, aNumeroTri, aIndiceBloc, aNode) {
		...
		return {
			...
			drag(aParamsDrag) {
				...
				let lLeft =
					aParamsDrag.pos.x - lRectContainer.left - aParamsDrag.rect.width / 2;
		...
		return {
			...
			over(aParamsDrop) {
				...
				const lEstDerniereLigne =
					lParamsCellule.ligne ===
					lThis._cache.lignesVisibles[lThis._cache.lignesVisibles.length - 1];
				if (lThis.Donnees.options.dragNDropLigneInsertion) {
					...
					let lTop =
						...
						ObjetPosition_1.GPosition.getClientRect(lJConteneur.get(0)).top -
						1;
