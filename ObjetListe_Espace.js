...
class ObjetListeEspace extends _ObjetListe_1.ObjetListe {
								...
								if (
									...
									while (
										lNumeroColonneTriRecherche <
											aInstance._cache.listeCorrespondancesColonnes.length &&
									...
									if (
										aInstance._cache.colonnesTri[lNumeroColonneTriRecherche] &&
										aInstance._estColonneVisible(lNumeroColonneTriRecherche) &&
										ObjetHtml_1.GHtml.getElement(
											aInstance.getIdAriaTitreDeColonne(
												lNumeroColonneTriRecherche,
											),
						...
						start(aParamsDrag) {
							...
							Object.assign(
								aParamsDrag.data,
								{
									estObjetListe: true,
									instance: aInstance,
									avecSuppression: aInstance._avecSuppressionSelectionCourante(
							...
							if (lData) {
								if (lData.horsZoneSuppression) {
									...
									ObjetHtml_1.GHtml.setFocus(
										aInstance.getIdCellule(lData.colonne, lData.ligne, true),
									);
								}
							}
						},
				...
				dropCelluleDroppable(aLigne, aColonne) {
					...
					return {
						...
						over(aParamsDrop) {
							...
							aInstance._parcourirCellulesVoileBleu(lParamsCellule, (aNode) => {
								...
								lJConteneur.ieHtmlAppend(
									[
										IE.jsx.str(
											"div",
											{
												id: aInstance.ids.dragInsertion + lParamsCellule.ligne,
	...
	_gererModificationsColonnes() {
		...
		this._cache.parametrageColonnes.forEach((aColonne) => {
			...
			this._cache.listeCorrespondancesColonnes[lOriginalIds.indexOf(lDecl.id)] =
				lNumeroColonne;
		});
		if (
					...
					IE.jsx.str(
						"div",
						{
							...
							class: [
								...
								this._options.forcerOmbreScrollBottom ? " ombre-bottom" : "",
							],
										...
										IE.jsx.str(
											...
											(aTab) => {
												this._cache.infosZonesColonnes.forEach(
													(aInfosZoneColonnes) => {
														aTab.push(
															this._construireContenuListe(aInfosZoneColonnes),
														);
													},
												);
											},
										...
										aTabGrid.push(
											IE.jsx.str(
												"div",
												{
													...
													"aria-describedby": this.ids.WAIDescribeTreeGridTotal,
												},
												this.ListeTitres
													? IE.jsx.str("div", {
															role: "rowgroup",
											...
											IE.jsx.str(
												...
												(aTab) => {
													this._cache.infosZonesColonnes.forEach(
														(aInfosZoneColonnes) => {
															aTab.push(
																this._construireLigneTotal(aInfosZoneColonnes),
															);
														},
													);
												},
	...
	_entrerDateCalendrier(aParams) {
		this.fenetreDate = ObjetFenetre_1.ObjetFenetre.creerInstanceFenetre(
			...
			{ pere: this, evenement: this._evenementFenetreDate.bind(this, aParams) },
		);
			...
			while (
				...
				this._cache.infosZonesColonnes.forEach((aInfosZoneColonnes) => {
					...
					aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
						...
						const lNombreLignesEnFusion = this._chercherLignesFusionVertical(
							lParamsCellule,
							iIndiceLigne,
							lRangeLignes,
							lFusions,
	...
	_nodeTableTitrePourTri(aNode, aInfosZonesColonnes) {
		...
		$(
			"#" +
				(
					this.ScrollH.getIdZone(aInfosZonesColonnes.idScrollTri) + "_conteneur"
				).escapeJQ(),
		).css("top", lHeightNode - Math.round(this._options.heightTri) + 1 + "px");
		const lIdSurvol =
				this.ids.survolTitre.escapeJQ() + aInfosZonesColonnes.indiceBloc,
			lIdCurseurTriSurvol =
				this.ids.curseurTriSurvol.escapeJQ() + aInfosZonesColonnes.indiceBloc;
	...
	_getLigneElementParParseur(aParseur, aColonne) {
		...
		this._cache.lignesVisibles.every((aLigne) => {
			...
			if (
				lValeur &&
				ComparateurChaines_1.ComparateurChaines.compare(lValeur, aParseur) >= 0
			) {
				return false;
			}
			return true;
		...
		const lEstColonneExportee = (aNumeroColonne) => {
			return (
				...
				(!this._cache.declarationsColonnes[aNumeroColonne] ||
					!this._cache.declarationsColonnes[aNumeroColonne].ignorerExportCSV)
			);
		};
				...
				for (
					...
					if (lEstColonneExportee(lNumeroColonne)) {
						lParamsCellule = this._getParamsCellule(
							lNumeroColonne,
							lNumeroLigne,
							{ surEdition: false, surProportion: false, surExportCSV: true },
						);
	...
	_construireTaillesGrid(aInfosZoneColonnes) {
		...
		aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne, aIndex) => {
			...
			if (
				...
				(aInfosZoneColonnes.estBlocFixe && !aInfosZoneColonnes.dernierBloc)
			) {
				lWidthCol += 1;
			}
			if (
			...
			lResult.tabWidth.push({ width: lWidthCol, colonne: aNumeroColonne });
		...
		return IE.jsx.str(
			"div",
			{
				id:
					...
					aIndiceTri,
				tabindex: "-1",
				"ie-nodeafter": (0, jsx_1.jsxFuncAttr)("liste.getNodeAfterCurseurTri", [
					aIndiceTri,
					aInfosZoneColonnes.indiceBloc,
				]),
		...
		if (this.ListeTitres) {
			for (iLigne = 0; iLigne < this.ListeTitres.length; iLigne++) {
				...
				if (
					...
					(!aInfosZoneColonnes.dernierBloc && !aInfosZoneColonnes.estBlocFixe)
				) {
					for (
						...
						if (
							!this._colonneEnFusionAvecColonnePrecedente(lTitresLigne, I) &&
							(this._estColonneVisible(I) ||
								this._colonneEnFusionAvecColonnePrecedente(lTitresLigne, I + 1))
						) {
						...
						T.push(
							IE.jsx.str(
								"div",
								{
									...
									class: lCss,
									"ie-class": aAvecTri
										? (0, jsx_1.jsxFuncAttr)("liste.getClassTriGridTitre", [I])
										: false,
									style:
	...
	_getJeuxCouleurNonSelection(aParamsCellule) {
		...
		if (
			...
			lCouleurs =
				this._cache.couleursAlternanceParLigne[aParamsCellule.ligne] === true
	...
	_chercherLignesFusionVertical(
		...
		for (
			...
			if (
				...
				this._cache.lignesFusionParColonne[lCle].lignesCumuls[lLigneSuivante] =
					true;
	...
	_construireCopySVG() {
		return IE.jsx.str(
			...
			IE.jsx.str(
				...
				IE.jsx.str("rect", { x: "0", y: "0", width: "100%", height: "100%" }),
			),
		);
	}
		...
		return IE.jsx.str(
			...
			(T) => {
				if (lCacheRef.avecConstructionDynamiqueContenu) {
					T.push(
						IE.jsx.str("div", {
							"ie-node": (0, jsx_1.jsxFuncAttr)("liste.nodeGabaritRefresh", [
								true,
								aInfosZoneColonnes.indiceBloc,
							]),
	...
	construireContenuListeInterneLignes(aInfosZoneColonnes, aIndiceRange) {
		...
		for (
			...
			if (
				...
				this.Donnees.avecSeparateurLigneHautFlatdesign(
					lParamCelluleLigne,
					this._getParamsCellule(-1, this._cache.lignesVisibles[iIndice - 1]),
				)
			) {
				T.push(
					(0, tag_1.tag)("hr", {
			...
			aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
				...
				this._cache.collectionFuncConstructCellule[
					...
					indiceColonneVisible:
						lAriaColIndex - 1 + (aInfosZoneColonnes.indiceColVisibleDebut || 0),
				...
				if (
					...
					lAttrIEDrop = (0, jsx_1.jsxFuncAttr)("liste.dropCelluleDroppable", [
						lParamsCellule.ligne,
						lParamsCellule.colonne,
					]);
				}
				...
				T.push(
					IE.jsx.str(
						"div",
						{
							...
							"ie-droppable": !this.Donnees.enConstruction_cacheRechercheTexte
								? lAttrIEDrop
								: "",
	...
	_construireContenuListe(aInfosZoneColonnes) {
		...
		T.push(
			...
			this._options.borduresContenu_top > 0
				...
				: "",
			this._options.borduresContenu_right > 0 && aInfosZoneColonnes.dernierBloc
				? ObjetStyle_1.GStyle.composeCouleurBordure(
						lCouleurBorduresContenu,
					...
					for (let I = 0; lTitresLigne && I < lTitresLigne.length; I++) {
						if (
							...
							if (lEstTriColonne) {
								...
								const lCarTri =
									lNumeroTri >= 0 && this._triCourant.colonne[lNumeroTri] === I
						...
						T.push(
							IE.jsx.str(
								"td",
								{
									...
									colspan:
										lNombreColonnesFusion > 1 ? lNombreColonnesFusion : false,
								},
								IE.jsx.str(
									"div",
									{
									...
									IE.jsx.str("tr", { class: "trTotal" }, (aTab) => {
										lColonnesVisibles.forEach((aNumeroColonne) => {
											...
											const lSansBordureDroite =
												this._cache.colonnesSansBordureDroit[aNumeroColonne];
											...
											if (lAvecBorduresTotal) {
												lStyleTotal = ObjetStyle_1.GStyle.composeCouleurBordure(
													"#000",
													1,
													(lSansBordureDroite
														? 0
													...
													IE.jsx.str(
														"div",
														{
															style: ObjetStyle_1.GStyle.composeWidth(
																lTaillesColonnes[aNumeroColonne].px * lRatio,
															),
														},
	...
	_avecLigneTotal() {
		const lListeTotal = this.Donnees
			...
			: null;
		return (
			this._options.avecLigneTotal || (lListeTotal && lListeTotal.count() > 0)
		);
	}
		...
		lLignes.parcourir((aArticle, aIndexLigne) => {
			...
			aInfosZoneColonnes.colonnesVisibles.forEach((aNumeroColonne) => {
				...
				if (this.Donnees) {
					...
					lSansBordureDroite =
						this._cache.colonnesSansBordureDroit[lFusions[aNumeroColonne].fin];
					lDerniereColonne =
						aInfosZoneColonnes.dernierBloc &&
						aInfosZoneColonnes.indiceColonneFin ===
				...
				T.push(
					IE.jsx.str(
						"div",
						{
							...
							style:
								this._composePaddingCellule() +
								(lTaille > 0 ? ObjetStyle_1.GStyle.composeWidth(lTaille) : "") +
								(lListeBordures > 0
									? ObjetStyle_1.GStyle.composeCouleurBordure(
											this._options.couleursListe.editable.getBordure(false),
											1,
				...
				Object.assign(
					{
						role: "checkbox",
						"aria-checked": "true",
						"aria-label": ObjetTraduction_1.GTraductions.getValeur("WAI.Coche"),
						class: lValeur,
	...
	_getHeightListeHorsContenu() {
		let lResult =
			...
			this._options.borduresContenu_bottom;
		lResult +=
			this._options.borduresContenu_top + this._options.borduresContenu_bottom;
		if (
			this._cache.avecScrollHorizontal &&
			this._cache.reserverPlaceScrollHorizontal
		) {
	...
	_gererRollover(aAfficher, aElement, aLigneColonne) {
		if (
			...
			this._cache.infosZonesColonnes.forEach((aBloc) => {
				...
				$(
					"#" + this.ids.rolloverContenuColonne.escapeJQ() + aBloc.indiceBloc,
				).hide();
				$(
					"#" + this.ids.rolloverContenuCellule.escapeJQ() + aBloc.indiceBloc,
				).hide();
			});
		}
	}
	...
	_surKeyUpCellule(aEvent) {
		...
		if (
			lInstance.Donnees &&
			lInstance._options.skin === _ObjetListe_1.ObjetListe.skin.flatDesign &&
					...
					Math.max(
						...
						this.ScrollV.TailleZone +
							...
							(this._avecBoutonsListeHautScroll()
								? (this._options.tailleBoutons + 1) * this._cache.boutons.length
								: 0) -
							this._options.borduresContenu_bottom,
					),
				);
	...
	_evenementFenetreDate(aParams, aGenreBouton, aDate) {
		if (aGenreBouton === -1) {
			if (aParams.ligne === null || aParams.ligne === undefined) {
				...
				this._surEditionFin({ colonne: aParams.colonne, ligne: aParams.ligne });
			}
		} else {
			const lValeur =
				aGenreBouton === 1
			...
			switch (aEvent.which) {
				case ToucheClavier_1.ToucheClavier.FlecheGauche: {
					...
					if (lCols) {
						...
						if (lIndice > 0) {
							(_a = ObjetHtml_1.GHtml.getElement(
								this.getIdCelluleTotal(lCols[lIndice - 1], lNav.ligne, true),
							)) === null || _a === void 0
								? void 0
								: _a.focus();
						}
	...
	liaisonWAITitreColonne(aActiver, aPourContenu, aNumeroColonne) {
		...
		if (aActiver) {
			...
			const lTreeGridTitre = ObjetHtml_1.GHtml.getElement(lIdsConteneurTitre);
			if (lTreeGridTitre) {
				lTreeGridTitre.setAttribute("aria-owns", lElementTitre ? lIdTitre : "");
			}
		} else {
			const lTreeGridTitre = ObjetHtml_1.GHtml.getElement(lIdsConteneurTitre);
			if (lTreeGridTitre) {
