...
class ObjetTreeView extends ObjetIdentite_1.Identite {
	...
	recupererDonnees() {}
	setParametres() {
		this.couleurs = (0, AccessApp_1.getApp)().getCouleur().liste;
		this.largeurDeploiement = 11;
	...
	_construireLigneDeNode(aElementConteneur, aNode) {
		...
		if (this.donneeTreeView.getAvecDeploiement(aNode)) {
			const lDOMi = document.createElement("i");
			ObjetHtml_1.GHtml.setClass(
				lDOMi,
				ObjetTreeView_css_1.StylesObjetTreeView.iconeDeploiement +
					" AvecMain " +
					(aNode.deploye ? "icon_fleche_num_bas" : "icon_fleche_num"),
			);
			$(lDOMi).mousedown(
				{ instance: this, node: aNode },
				this._surMouseDownDeploiement,
			);
			ObjetHtml_1.GHtml.setTexte(lDOMi, " ");
			aNode.DOM.deploiement = lDOMi;
			lTdDeploiement.appendChild(lDOMi);
		}
	...
	_surDragDebut(aNode) {
		...
		this._posSourisX = ObjetNavigateur_1.Navigateur.pointerX;
		this._posSourisY = ObjetNavigateur_1.Navigateur.pointerY;
		...
		const lPositionX = Math.min(
			...
			ObjetNavigateur_1.Navigateur.clientL -
				ObjetPosition_1.GPosition.getWidth(lDraggable),
		);
		...
		ObjetNavigateur_1.Navigateur.debDeplacement(this, lDraggable);
		this._objetDrag = {
	...
	_surDraggableAfficher(AAfficher) {
		if (
			...
			ObjetNavigateur_1.Navigateur.SourisBouton
		) {
			if (
				AAfficher ||
				(this._posSourisX - ObjetNavigateur_1.Navigateur.pointerX) *
	...
	_setSelectionNode(aNode) {
		...
		if (this.donneeTreeView.avecMultiSelection()) {
			...
			if (this.nodeSelection) {
				if (ObjetNavigateur_1.Navigateur.CtrlTouche) {
			...
			if (this.donneeTreeView.surSuppressionConfirmation(aNode)) {
				...
				(0, AccessApp_1.getApp)()
					...
					.afficher({
						...
						message:
							this.donneeTreeView.getMessageSuppressionConfirmation(aNode),
	...
	_surDeploiement(aEvent) {
		...
		if (aEvent.data.instance.donneeTreeView.getAvecDeploiement(lNode)) {
			...
			ObjetHtml_1.GHtml.modifierClass(
				...
				lNode.deploye ? "icon_fleche_num" : "icon_fleche_num_bas",
				lNode.deploye ? "icon_fleche_num_bas" : "icon_fleche_num",
			);
			if (lNode.DOM.image) {
				ObjetHtml_1.GHtml.setClass(
					lNode.DOM.image,
	...
	_surMouseUpLigne(aEvent) {
		if (
			ObjetNavigateur_1.Navigateur.estSourisBoutonDroit() &&
	...
	_surKeyDownTreeView(event) {
		...
		let lNode;
		if (
			ToucheClavier_1.ToucheClavierUtil.estEventSupprimer(event) &&
			!lInstance._enEdition
		) {
			lInstance.surSuppression(lInstance.nodeSelection);
		} else if (ObjetNavigateur_1.Navigateur.isToucheFlecheBas()) {
		...
		} else if (ObjetNavigateur_1.Navigateur.isToucheFlecheHaut()) {
			lNode = lInstance.donneeTreeView.selectionFocusSuivant(
				lInstance.nodeSelectionFocus,
				false,
			);
		...
		} else if (ObjetNavigateur_1.Navigateur.isToucheSelection()) {
