...
class ObjetFenetre_EvenementPeriodicite extends ObjetFenetre {
	constructor(...aParams) {
		...
		this.id = {
			inputHeureDebut: GUID.getId(),
			inputHeureFin: GUID.getId(),
			ctnHeure: GUID.getId(),
		};
	}
	construireInstances() {
	...
	setDonnees(aParametres) {
		...
		this.periodicite = this.SaisieAgenda.evenement.estPeriodique
			? MethodesObjet.dupliquer(this.SaisieAgenda.evenement.periodicite)
			: this.periodicite;
		if (
		...
		_initListePeriodicite.call(this);
		_initInputsParDefault.bind(this)();
		...
		return $.extend(true, super.getControleur(aInstance), {
				...
				setValue: function (aIndice) {
					...
					if (
						...
						aInstance.periodicite.intervalle = parseInt(
							GHtml.getValue(aInstance.Nom + "_editJ_" + 0),
						);
					}
					if (
						aInstance.periodicite.type ===
						...
						aInstance.periodicite.intervalle = parseInt(
							GHtml.getValue(aInstance.Nom + "_editS_" + 0),
						);
					}
					if (
						aInstance.periodicite.type ===
						...
						aInstance.periodicite.intervalle = parseInt(
							GHtml.getValue(aInstance.Nom + "_editM_" + 0),
						);
					}
				},
			},
			typeMensuel: {
				...
				setValue: function (aIndice) {
					...
					if (
						...
						aInstance.periodicite.intervalle = parseInt(
							GHtml.getValue(aInstance.Nom + "_editM_" + 1),
						);
					}
				},
			},
			...
			ComboMensuelle: {
				init: function (aIndice, aCombo) {
					aCombo.setOptionsObjetSaisie({
						labelWAICellule:
							aIndice === 1
								? GTraductions.getValeur("Fenetre_SaisieAgenda.WAI.position")
								: GTraductions.getValeur("Fenetre_SaisieAgenda.WAI.jour"),
			...
			comboSelectTypePeriodicite: {
				init(aCombo) {
					aCombo.setOptionsObjetSaisie({
						...
						labelWAICellule: GTraductions.getValeur("WAI.SelectionPeriodicite"),
					});
				...
				getAttr() {
					return {
						"aria-description": GTraductions.getValeur(
			...
			inputMois: {
				...
				exitChange(aIndice) {
					...
					aInstance.periodicite.intervalle = parseInt(lValue);
				},
				getAttr(aIndice) {
					let lResult;
					if (aIndice === 0 && !!aInstance.inputMoisMensuelAbsolue) {
						lResult = {
							"aria-label": `${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteToutesLes")} ${aInstance.inputMoisMensuelAbsolue}  ${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteMois")}`,
						};
					} else if (!!aInstance.inputMoisMensuelRelatif) {
						lResult = {
							"aria-label": `${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteToutesLes")} ${aInstance.inputMoisMensuelRelatif}  ${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteMois")}`,
						};
					}
					return lResult;
				},
			...
			inputJours: {
				...
				exitChange() {
					const lValue = _verifValueRangeInput(
				...
				getAttr() {
					if (!!aInstance.inputJourQuotidien) {
						const lResult = {
							"aria-label": `${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteToutesLes")} ${aInstance.inputJourQuotidien}  ${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteJours")}`,
						};
						return lResult;
					} else {
						return "";
			...
			inputSemaines: {
				...
				getAttr() {
					if (!!aInstance.inputSemaineHebdomadaire) {
						const lResult = {
							"aria-label": `${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteToutesLes")} ${aInstance.inputSemaineHebdomadaire}  ${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteSemaines")}`,
						};
						return lResult;
					} else {
						return "";
			...
			inputDate: {
				...
				exitChange() {
					...
					aInstance.periodicite.jourDuMois = parseInt(lValue);
				},
				getAttr() {
					if (!!aInstance.inputDateMensuelAbsolue) {
						const lResult = {
							"aria-label": `${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteLe")} ${aInstance.inputDateMensuelAbsolue}`,
						};
						return lResult;
					} else {
						return "";
	...
	composeContenu() {
		...
		T.push(
			`\n        <section class="field-contain">\n            <section class=" bloc-horaire">\n                <article class="ctn-switch">\n                    <ie-switch ie-textleft ie-model="switchHoraire"><span class="def-txt">${GTraductions.getValeur("Fenetre_SaisieAgenda.AvecHoraire")}</span></ie-switch>\n                </article>\n                <article class="ctn-time ${IE.estMobile && this.periodicite.sansHoraire ? "opacity-low" : ""}" id="${this.id.ctnHeure}">\n                    <article>\n                        <input type="time" aria-label="${GTraductions.getValeur("Fenetre_SaisieAgenda.WAI.heureDebut")}" ie-model="inputTimeHoraire.heureDebut" class="round-style"></input>\n                    </article>\n                    <article>\n                        <input type="time" aria-label="${GTraductions.getValeur("Fenetre_SaisieAgenda.WAI.heureFin")}" ie-model="inputTimeHoraire.heureFin" class="round-style"></input>\n                    </article>\n                </article>\n            </section>\n        </section>\n    `,
		);
		T.push(
			'<section class="field-contain">',
			'<article class="bloc-date">',
			'<span class="label-gauche">',
			GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteDu"),
			"</span>",
			'<article id="',
			this.getInstance(this.identDateDebut).getNom(),
			'"></article>',
			'<span class="label-gauche">',
			...
			"</span>",
			'<article id="',
			this.getInstance(this.identDateFin).getNom(),
			'"></article>',
			"</article>",
			"</section>",
		);
		...
		T.push(`\n              <section ie-display="blocQuotidienne" class="bloc-quotidienne flex-between">\n                  <span class="m-right-l def-txt">${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteTousLes")}</span>\n                  <article>\n                      ${this._composeSaisieJour(0)}
                      <p class="m-left-l def-txt like-span">${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteJours")}</p>\n                  </article>\n              </section>\n    `);
		T.push(`\n              <section ie-display="blocHebdomadaire" class="bloc-hebdomadaire">\n                  <article class="ctn-haut flex-center">\n                      <span  class="m-right-l def-txt">${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteToutesLes")}</span>\n                      <article>\n                          ${this._composeSaisieSemaine(0)}
                          <p class="m-left-l def-txt like-span">${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteSemaines")}</p>\n                      </article>\n                  </article>\n                  <article class="ctn-liste-jours">\n    `);
		for (
	...
	composeBas() {
		...
		if (this.SaisieAgenda.evenement.estPeriodique) {
			if (IE.estMobile) {
				T.push(
					`<div class="compose-bas">\n                    <ie-btnicon class="icon_trash avecFond i-medium" title="${GTraductions.getValeur("Fenetre_SaisieAgenda.FenetrePeriodiciteSupprimerPeriodicite")}" ie-model="supprimerPeriodicite"></ie-btnicon>\n                </div>`,
				);
			} else {
				T.push(
	...
	_composeSaisieJour(aGenreEdit) {
		const T = [];
		const lStyle = GStyle.composeWidth(IE.estMobile ? 20 : 35);
		T.push(
			'<input id="',
	...
	_composeSaisieSemaine(aGenreEdit) {
		const T = [];
		const lStyle = GStyle.composeWidth(IE.estMobile ? 17 : 35);
		T.push(
			'<input id="',
			...
			lStyle,
			'" ',
			'class="input-nbr round-style m-right-l"',
			'maxlength="2" ',
...
function _initListePeriodicite() {
	const lArr = [
	...
	lArr.map((aIndex) => {
		lListe.add(new ObjetElement(GTraductions.getValeur(aIndex)));
	});
...
function _composeLibelleDescription() {
	...
	switch (this.periodicite.type) {
		case ObjetFenetre_EvenementPeriodicite.radioPeriodicite.quotidienne:
			if (this.periodicite.intervalle > 1) {
				lHtml.push(
					...
					GTraductions.getValeur("Fenetre_SaisieAgenda.ALieuTousLesNJours", [
				...
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.ALieuTousLesJours"),
					"</div>",
				);
			}
			break;
		case ObjetFenetre_EvenementPeriodicite.radioPeriodicite.hebdomadaire:
			if (this.periodicite.intervalle > 1) {
				...
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.ALieuToutesLesSemaines"),
					"</div>",
				);
			}
			if (this.periodicite.joursDeLaSemaine.length > 0) {
				...
				lHtml.push(
					...
					GTraductions.getValeur("Fenetre_SaisieAgenda.LeJourSem", [
				...
				for (
					...
					lHtml.push(
						'<div class="InlineBlock">',
						GTraductions.getValeur("Fenetre_SaisieAgenda.JourSemSuite", [
							this.SaisieAgenda.listeJoursDeLaSemaine
		...
		case ObjetFenetre_EvenementPeriodicite.radioPeriodicite.mensuelAbsolue:
			if (this.periodicite.intervalle > 1) {
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.ALieuTousLesNMois", [
						this.periodicite.intervalle,
					]),
					"</div>",
				);
				...
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.ALieuTousLesMois"),
					"</div>",
				);
			}
			lHtml.push(
				'<div class="InlineBlock PetitEspaceDroit">',
				GTraductions.getValeur("Fenetre_SaisieAgenda.LeJourDuMois", [
		...
		case ObjetFenetre_EvenementPeriodicite.radioPeriodicite.mensuelRelatif:
			lHtml.push(
				'<div class="InlineBlock PetitEspaceDroit">',
				GTraductions.getValeur("Fenetre_SaisieAgenda.LeMoisRelatif", [
			...
			if (this.periodicite.intervalle > 1) {
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.tousLesNMoisRelatif", [
						this.periodicite.intervalle,
					]),
					"</div>",
				);
				...
				lHtml.push(
					'<div class="InlineBlock PetitEspaceDroit">',
					GTraductions.getValeur("Fenetre_SaisieAgenda.tousLesMoisRelatif"),
					"</div>",
				);
			}
			break;
	...
	lHtml.push(
		'<div class="InlineBlock PetitEspaceDroit">',
		GTraductions.getValeur("Fenetre_SaisieAgenda.ACompterDU", [
			GDate.formatDate(this.periodicite.DateDebut, "%JJ/%MM/%AA"),
		]),
		"</div>",
	);
	lHtml.push(
		'<div class="InlineBlock PetitEspaceDroit">',
		GTraductions.getValeur("Fenetre_SaisieAgenda.Jusquau", [
			GDate.formatDate(this.periodicite.DateFin, "%JJ/%MM/%AA"),
		]),
		"</div>",
	);
	if (!this.periodicite.sansHoraire) {
		lHtml.push(
			'<div class="InlineBlock PetitEspaceDroit">',
			GTraductions.getValeur("Fenetre_SaisieAgenda.DeH"),
			"</div>",
		);
		lHtml.push(
			'<div class="InlineBlock PetitEspaceDroit">',
			GDate.formatDate(this.periodicite.heureDebut, "%hh%sh%mm"),
			"</div>",
		);
		lHtml.push(
			'<div class="InlineBlock PetitEspaceDroit">',
			GTraductions.getValeur("Fenetre_SaisieAgenda.AH"),
			"</div>",
		);
		lHtml.push(
			'<div class="InlineBlock PetitEspaceDroit">',
			GDate.formatDate(this.periodicite.heureFin, "%hh%sh%mm"),
			"</div>",
		);
	}
...
function _initValeurInputParType(aParams) {
	if (!!aParams) {
...
function _verifValueRangeInput(aValue, aMax) {
...
function _surValidation() {
	...
	this.SaisieAgenda.genreEvt =
		this.SaisieAgenda.genreEvt === EGenreEvtAgenda.nonPeriodique
			? EGenreEvtAgenda.surNouvelleSerie
			: this.SaisieAgenda.genreEvt;
	this.SaisieAgenda._actualiserBlocPeriodicite();
	this.fermer();
}
function _setHeureEvent() {
	const lHeureDebutHeure = parseInt(this.valueInputHeureDebut.substring(0, 2));
	...
	if (
		...
		if (lPeriodiciteApres.avecEvtPersoDansSerie) {
			if (
				...
				lPeriodiciteAvant.avecJourOuvres === lPeriodiciteApres.avecJourOuvres &&
