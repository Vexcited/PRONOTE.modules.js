...
class _InterfaceMobile extends ObjetInterface_1.ObjetInterface {
	constructor(...aParams) {
		super(...aParams);
		this.applicationPoduit = (0, AccessApp_1.getApp)();
		this.largeur = "100%";
		this.hauteur = "100%";
	...
	construireStructureAffichage() {
		...
		if (this.idMenuOnglets !== undefined) {
			lZones.push(
				`<div id="${this.getNomInstance(this.idMenuOnglets)}" class="side-menu-mobile disable-dark-mode ie-draggable-x" ie-draggable="getDragMenu"></div>`,
			);
		}
		const lAfficherFooter =
			(!ObjetNavigateur_1.Navigateur.isAndroid &&
				!ObjetNavigateur_1.Navigateur.isIOS) ||
			(ObjetNavigateur_1.Navigateur.isIOS &&
				ObjetNavigateur_1.Navigateur.versionIOS &&
				(ObjetNavigateur_1.Navigateur.versionIOS[0] > 10 ||
					(ObjetNavigateur_1.Navigateur.versionIOS[0] === 10 &&
						ObjetNavigateur_1.Navigateur.versionIOS[1] > 3))) ||
			(ObjetNavigateur_1.Navigateur.isAndroid &&
				ObjetNavigateur_1.Navigateur.versionAndroid &&
				ObjetNavigateur_1.Navigateur.versionAndroid[0] >= 6);
		lZones.push(
			'<div id="',
			this.getNomInstance(this.identEnteteMobile),
			...
			'<div id="',
			this.getNomInstance(this.identPageMobile),
			'" class="objetInterfaceMobile_identPage"></div>',
			"</div>",
			this.IdentBandeauPied >= 0 && lAfficherFooter
				? IE.jsx.str("div", {
						class: "footer-mobile",
						"ie-display": "afficherMasquerFooter",
						id: this.getNomInstance(this.IdentBandeauPied),
					})
				: "",
		);
		lZones.push("</div>");
	...
	initEntete(aInstance) {
		if (this.idMenuOnglets !== undefined) {
			aInstance.setParametres({
				idPanel: this.getNomInstance(this.idMenuOnglets),
			});
		}
	}
	...
	evenementSurOnglet(
		...
		if (
			...
			const lOnglet =
				GEtatUtilisateur.listeOnglets && aGenreOnglet
					? GEtatUtilisateur.listeOnglets.getElementParGenre(aGenreOnglet)
					: null;
			...
			if (!lInstance.applicationPoduit.estAppliMobile) {
				setTimeout(() => {
					lInstance.applicationPoduit
						...
						.then(
							lInstance._evenementPrecedentSuivantDeconnexion.bind(lInstance),
						);
				}, 10);
			} else {
