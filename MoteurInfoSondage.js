exports.MoteurInfoSondage = exports.EGenreEvntMenuCtxBlocInfoSondage = void 0;
...
class MoteurInfoSondage {
				...
				if (aParam.composante && aParam.composante.listePiecesJointes) {
					aParam.composante.listePiecesJointes.parcourir((aPJ) => {
						if (aPJ.avecMiniaturePossible) {
							...
							aPJ.miniature = TypeGenreMiniature_1.TypeGenreMiniature.GM_600;
	...
	composeDescriptif(aParam) {
		...
		if (lActu) {
			...
			if (aParam.avecTitre) {
				H.push(
					IE.jsx.str("h2", { class: "titre-info-sondage" }, lActu.getLibelle()),
				);
			}
			H.push("<p>");
			...
			if (lEstModeDiffusion && !lActu.estModele) {
				...
				if (
					...
					if (lNbrPartage > 0) {
						...
						H.push(IE.jsx.str("p", { class: "partage-info-sondage" }, lStr));
					}
				}
			}
	...
	composeDescriptifLecture(aActu, aEstModeReception) {
		...
		const lStr = aActu.estSondage
			? ObjetTraduction_1.GTraductions.getValeur("infoSond.sondage")
			: ObjetTraduction_1.GTraductions.getValeur("infoSond.information");
		...
		for (let i = 0, lNbr = aActu.listeQuestions.count(); i < lNbr; i++) {
			...
			if (aActu.estSondage) {
				if (lParam.avecLibelleQuestion && this.estComposanteQuestion(lParam)) {
			...
			if (lListePJ.count()) {
				...
				if (lArrDocs.length > 0) {
					H.push(
						lArrDocs.length > 1
							? ObjetTraduction_1.GTraductions.getValeur(
									"infoSond.DocumentsJoints",
								)
							: ObjetTraduction_1.GTraductions.getValeur(
									"infoSond.DocumentJoint",
								),
						" : ",
						lArrDocs.join(", "),
					);
	...
	composeReponseTextuelle(aParam, aId) {
		...
		H.push(
			...
			ObjetTraduction_1.GTraductions.getValeur("actualites.SaisirReponse"),
			'" ',
		...
		if (lComposante) {
			...
			const lNombreCoche = lEstAvecMax
				? lComposante.reponse && lComposante.reponse.valeurReponse
					...
					: 0
				: 0;
			for (let I = 0, lNbr = lComposante.listeChoix.count(); I < lNbr; I++) {
		...
		if (aParam.composante.reponse.estRepondant === false) {
			...
			if (aParam.estAffEditionActualite) {
				if (aParam.composante.avecAR) {
					H.push(
						`<p>${ObjetTraduction_1.GTraductions.getValeur("actualites.Edition.AvecAR")}</p>`,
					);
				}
			} else {
				H.push(
					'<ie-checkbox ie-model="cbAR">',
					ObjetTraduction_1.GTraductions.getValeur("accueil.prisConnaissance"),
					"</ie-checkbox>",
				);
			}
		}
	...
	avecInfoPublic(aActualite) {
		...
		if (lEstEspaceParent || lEstEspaceEntreprise) {
			...
			const lEstActuDUtilisateur =
				aActualite.public.getNumero() === lUtilisateurConnecte.getNumero() &&
			...
			if (lElement.public) {
				...
				if (!lCumulPere) {
					const lATitreIndividuel =
						lElement.public.getNumero() === lUtilisateurConnecte.getNumero() &&
	...
	aToutRepondu(aActualite) {
		...
		for (
			...
			lResult =
				...
				this.utilitaires.genreReponse.estGenreSansAR(lQuestion.genreReponse);
		}
		return lResult;
	}
	getDerniereReponse(aActualite) {
		...
		aActualite.listeQuestions.parcourir((aQuestion) => {
			if (
				...
				!this.utilitaires.genreReponse.estGenreSansAR(aQuestion.genreReponse)
			) {
				if (!lResult) {
					lResult = aQuestion;
				} else {
	...
	formatterListeActusPourBlocs(aParam) {
		for (let i = 0, lNbr = aParam.listeActualites.count(); i < lNbr; i++) {
			...
			lActu.avecReponse =
				UtilitaireInfoSondage_1.TUtilitaireInfoSondage.avecReponse.bind(lActu);
			lActu.aToutRepondu =
				UtilitaireInfoSondage_1.TUtilitaireInfoSondage.aToutRepondu.bind(lActu);
			lActu.getDate =
	...
	initialiserNouveauItem(aParam) {
		...
		lInformation.getDate =
			UtilitaireInfoSondage_1.TUtilitaireInfoSondage.getDate.bind(lInformation);
		return lInformation;
	}
	...
	afficherInfoNonDispoSurMobile() {
		...
		Toast_1.Toast.afficher({ msg: lMessage, type: Toast_1.ETypeToast.info });
	}
	initCommandesMenuCtxDInfoSond(aMenu, aActualite, aParam) {
		if (aParam.avecEditionActualite) {
			this.initCommandesMenuCtxDInfoSondDiffusion(aMenu, aActualite, aParam);
		} else {
			this.initCommandesMenuCtxDInfoSondReception(aMenu, aActualite, aParam);
		}
	}
			...
			if (lAvecDroitSaisie) {
				...
				if (lEstCtxModele) {
					...
					if (lAvecGestionDuPartage) {
						aMenu.add(
							...
							},
							{
								...
								Numero: EGenreEvntMenuCtxBlocInfoSondage.gestionPartageModele,
								typeAffEnModeMixte:
					...
					if (!IE.estMobile && aActualite.estSondage) {
						aMenu.add(
							ObjetTraduction_1.GTraductions.getValeur(
								"actualites.exporterModeleSondage",
							),
							lAvecDroitSaisie,
							(aLigneMenu) => {
								aParam.evenementMenuContextuel(
	...
	initCommandesMenuCtxDInfoSondReception(aMenu, aActualite, aParam) {
		if (!(0, AccessApp_1.getApp)().getModeExclusif()) {
			if (aActualite.lue) {
				aMenu.add(
					ObjetTraduction_1.GTraductions.getValeur("actualites.MarquerNonLu"),
					aActualite.lue,
					() => {
				...
				aMenu.add(
					ObjetTraduction_1.GTraductions.getValeur("actualites.MarquerLu"),
					!aActualite.lue,
					() => {
	...
	verifierEtatModification(aParam) {
		...
		const lTest =
			...
			!ObjetDate_1.GDate.estJourEgal(lDonnee.dateFin, lDonneeOrigine.dateFin) ||
...
function _getStrPublication(aInfos) {
	return aInfos.estPubliee
		? aInfos.estEnCours
			? ObjetTraduction_1.GTraductions.getValeur("actualites.Publie")
			: aInfos.estFutur
				? ObjetTraduction_1.GTraductions.getValeur("infoSond.publicationFutur")
				: ObjetTraduction_1.GTraductions.getValeur("infoSond.publicationPassee")
