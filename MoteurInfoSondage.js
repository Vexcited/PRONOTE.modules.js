...
class MoteurInfoSondage {
		...
		for (let i = 0, lNbr = aActu.listeQuestions.count(); i < lNbr; i++) {
			...
			if (aActu.estSondage) {
				...
				if (this.estComposanteQuestTextuelle(lParam) && lQuestion.reponse) {
					...
					const lArrTextes = SyntheseVocale.getTextesDUneChaineHTML(lHtmlChoix);
					...
					if (lQuestion.reponse.estRepondant === false) {
						H.push(
							GTraductions.getValeur("actualites.XAPrisConnaissanceLeX", [
								...
								GDate.formatDate(lQuestion.reponse.reponduLe, "%JJ/%MM/%AAAA"),
							]),
						);
					} else {
	...
	composeReponseTextuelle(aParam, aId) {
		...
		H.push(
			...
			aId,
			'_0" ',
			GHtml.composeAttr("ie-model", "surSaisieReponseTextuelle", aParam.indice),
				...
				if (!lEstCtxModele) {
					...
					if (lDroitPublicationPageEtablissement) {
						if (aActualite.publicationPageEtablissement) {
							aMenu.add(
								GTraductions.getValeur("actualites.Edition.DepublierPageEtab"),
								lAvecDroitSaisie && lEstAuteurActu,
								() => {
							...
							aMenu.add(
								GTraductions.getValeur("actualites.Edition.PublierPageEtab"),
...
function _evntRelanceActu(aActualite, aParams, aLigneMenu, aGenreAction) {
	if (aGenreAction === EGenreAction.Valider) {
		aParams.evenementMenuContextuel(aActualite, EGenreEvntActu.SurMenuCtxActu, {
			cmd: aLigneMenu,
		});
	}
}
