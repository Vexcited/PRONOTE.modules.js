...
class ObjetChaine {
  ...
  getLongueurChaineDansDiv(AChaine, ATaille, AGras, aNonToTexte) {
    ...
    if (this.__cacheCanvas.actif) {
      ...
      if (!this.__cacheCanvas.fontSize[ATaille]) {
        if (ATaille && typeof ATaille === "string" && ATaille.endsWith("rem")) {
          ...
          let lVal = UtilitaireCss_1.UtilitaireCss.chercherAttributReglesCss(
            ".Texte" + ATaille,
            "fontSize",
          );
          if (!lVal) {
    ...
    lChaine =
      lChaine && lChaine.replace ? lChaine.replace(/ /g, "&nbsp;") : lChaine;
    E.innerHTML = lChaine;
      ...
      for (
        ...
        if (!L) {
          L = this.LongueurCaracteres[ATaille][G][C] =
            this.getLongueurChaineDansDiv(lChaine.charAt(I), ATaille, AGras);
        }
        LLongueur += L === 0 ? 10 : L;
      }
    }
  ...
  composerUrlLienExterne(aParam) {
    ...
    if (
      ...
      (lParam.documentJoint.getEtat() === Enumere_Etat_1.EGenreEtat.Creation ||
  ...
  estFichierImageAvecMiniaturePossible(aNomFichier) {
    if (aNomFichier) {
      ...
      return (
        !!lExt &&
        !["webp"].includes(lExt) &&
        Enumere_FormatDocJoint_1.EFormatDocJointUtil.getGenreDeFichier(lExt) ===
  ...
  _getClasseIconeDocumentJoint(aDocJoint) {
    ...
    if (lGenreFichier !== Enumere_FormatDocJoint_1.EFormatDocJoint.Inconnu) {
      ...
      if (
        ...
        aDocJoint.getGenre() === Enumere_DocumentJoint_1.EGenreDocumentJoint.Url
      ) {
