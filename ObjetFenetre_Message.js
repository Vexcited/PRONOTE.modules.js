...
class ObjetFenetre_Message extends ObjetFenetre_1.ObjetFenetre {
			...
			chipsFichierCloud: {
				eventBtn: function (aIndice) {
					...
					if (lElement) {
						...
						aInstance.message.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
					}
				},
			},
			textareaSansTiny: {
	...
	_getListeTypeDestinatairesPossiblesPourEleve() {
		...
		if (
			this.param.genreRessource === Enumere_Ressource_1.EGenreRessource.Eleve
		) {
	...
	static creerFenetreDiscussion(aInstance, aDonnees, aOptions) {
		...
		if (
			...
			lDonnees.genreRessource = Enumere_Ressource_1.EGenreRessource.Enseignant;
	...
	_composeChampDestinataires() {
		...
		if (!this.optionsFenetre.avecChoixDestinataires) {
			H.push(
				...
				'">',
				"<div>",
				ObjetTraduction_1.GTraductions.getValeur("Messagerie.DestinatairesMsg"),
				"</div>",
				'<div id="',
	...
	_composeZoneContenuMsg() {
		...
		if (lAvecTiny || this._autorisePJ()) {
			...
			return IE.jsx.str(
				"div",
				{ role: "group", class: "flex-contain cols", "aria-labelledby": lId },
				IE.jsx.str(
					"span",
	...
	_evntSelecteurPJ() {
		if (this.message.contenu !== "" || this.message.listeFichiers.count() > 0) {
			this.message.setEtat(Enumere_Etat_1.EGenreEtat.Modification);
		}
		this.$refreshSelf();
	}
